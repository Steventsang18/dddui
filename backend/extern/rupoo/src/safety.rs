...
impl SafetyContext {
    ...
    /// Snapshot of the tool names that require user approval. Useful for hosts
    /// that need to pre-classify tool calls (e.g. suppress duplicate event
    /// emission when an interactive approval gate is installed).
    pub fn approval_required_tools(&self) -> HashSet<String> {
        self.approval_required_tools.clone()
    }
}
#[cfg(test)]
mod tests {
