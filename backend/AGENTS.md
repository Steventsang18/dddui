...
See also the standing discipline in the root `AGENTS.md` / memory `rosecore-verification-blindspot-g6`: self-consistent-all-green ≠ correct — verify outward (against a real agent) AND against the old/reference implementation, not just against your own happy path.
...
- All request/response types defined in `roseui-api-types`
- `roseui-api-types` must NOT depend on axum/tower or any HTTP framework
- Use `roseui_common::ApiError` only at API/HTTP boundaries such as routes and middleware. Service/domain code must prefer crate-owned errors (`ConversationError`, `TeamError`, etc.) and map them to `ApiError` in route modules.
...
- Repository traits in `roseui-db`, prefixed with `I`
- Concrete implementations prefixed with `Sqlite`
- Row models in `roseui-db/src/models/`
...
- All assembly happens in `roseui-app`'s `build_*_state()` functions
...
New subprocess spawn sites must go through `roseui_runtime`'s spawn Builder — never raw `tokio::process::Command`. See [ARCHITECTURE.md § Runtime Infrastructure](./ARCHITECTURE.md#runtime-infrastructure) for the correct constructor and details.
...
1. Request/response types → `roseui-api-types/src/{domain}.rs`
2. Handler function → `crates/roseui-{domain}/src/routes.rs`
3. Business logic → `crates/roseui-{domain}/src/service.rs`
4. Register route in `domain_routes()` function
5. Add test → `crates/roseui-{domain}/tests/` or `crates/roseui-app/tests/`

### Add Migration

1. Next number → `ls crates/roseui-db/migrations/`
2. Create `NNN_descriptive_name.sql` with `IF NOT EXISTS`

### Add WebSocket Event

1. Event type → `roseui-api-types`
...
> ⚠️ **When to run what:**
> - During development: only test the crate you're working on → `cargo test -p roseui-<crate>`
...
> - `cargo clippy -p roseui-<crate>` and `cargo test -p roseui-<crate>` typically complete in under 1 minute.

### During Development (fast feedback loop)

```bash
cargo test -p roseui-<crate>                          # Test the crate you changed
cargo clippy -p roseui-<crate> -- -D warnings         # Lint the crate you changed
```
...
cargo clippy -p roseui-<crate1> -p roseui-<crate2> -- -D warnings              # Lint affected crates
cargo test -p roseui-<crate1> -p roseui-<crate2>                               # Test affected crates
```
