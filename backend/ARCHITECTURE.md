...
RoseCore is the backend server for RoseUi, built with Rust (Axum + Tokio + SQLite).
It provides HTTP REST APIs and WebSocket real-time events for the RoseUi desktop client.
...
│  tion    │          │          │                 │
├──────────┴──────────┴──────────┴────────────────┤
│   roseui-auth          roseui-realtime           │
│  (JWT, CSRF, middleware) (WebSocket, events)     │
├─────────────────────────────────────────────────┤
│  roseui-db    roseui-api-types   roseui-runtime  │
│ (repositories) (API contracts)  (runtime/process)│
├─────────────────────────────────────────────────┤
│       roseui-common          roseui-assets       │
│  (error types, enums, crypto)  (embedded data)   │
└─────────────────────────────────────────────────┘
```

Dependencies flow strictly downward. Domain crates must not depend on roseui-app,
and roseui-common has zero internal dependencies.
...
|-------|----------------|
| `roseui-common` | Shared error types (ApiError), enums, ID generation, crypto utilities, timestamps, pagination |
| `roseui-api-types` | All HTTP/WebSocket request and response types — the single source of truth for API contracts |
| `roseui-db` | SQLite database layer, defines Repository traits and implementations |
| `roseui-assets` | Embedded static assets (agent metadata, prompts) |
| `roseui-runtime` | Managed Node, subprocess spawning, PATH enhancement |
...
| Crate | Responsibility |
|-------|----------------|
| `roseui-auth` | JWT authentication, password hashing, CSRF protection, cookie management, auth middleware |
| `roseui-realtime` | WebSocket connection management, event broadcasting (BroadcastEventBus), message routing |
...
| Crate | Responsibility |
|-------|----------------|
| `roseui-conversation` | Conversation management, messaging, confirmations, streaming responses |
| `roseui-channel` | Multi-channel integration (WeChat, DingTalk, Lark), plugin system, pairing sessions |
| `roseui-team` | Team collaboration, task scheduling, mailbox system |
| `roseui-cron` | Scheduled job execution, cron expressions, event triggering |
| `roseui-file` | File operations, watching, snapshots, git operations, compression |
| `roseui-office` | Office document handling (Excel, PPT, Word), preview, conversion |
| `roseui-system` | System settings, provider management, version checking, model fetching |
| `roseui-mcp` | MCP protocol integration, OAuth, multi-platform adapters |
| `roseui-ai-agent` | Agent lifecycle management, worker task queues, ACP/auxiliary skills |
| `roseui-extension` | Extension registry, hub management, skill discovery and installation |
| `roseui-shell` | Shell command execution, speech-to-text |
| `roseui-assistant` | Assistant configuration and management |

### Composition

| Crate | Responsibility |
|-------|----------------|
| `roseui-app` | Top-level binary entry point, assembles all crates into the Axum server |
...
Every domain crate follows a consistent internal organization. Using roseui-conversation as a reference:

### Standard Directory Structure

```
crates/roseui-conversation/src/
...
All response types are defined in `roseui-api-types` — the single source of truth for API contracts.
...
All database access goes through trait abstractions defined in `roseui-db`:
...
|------|----------|---------|---------|
| Row models | `roseui-db/src/models/` | Database row mapping | `ConversationRow` |
| Params objects | `roseui-db/src/repository/` | Database write parameters | `UpdateConversationParams` |
| Request/response types | `roseui-api-types` | API contracts and shared DTOs | `CreateConversationRequest`, `ConversationResponse` |

**The service layer may directly use types from `roseui-api-types`.** This crate contains
pure data structure definitions with no HTTP framework dependencies, essentially serving as a shared DTO layer.

⚠️ **Critical constraint: `roseui-api-types` must not depend on axum, tower, or any HTTP framework.
...
- **Repository (roseui-db):** Pure database operations, no business logic
...
- Migration files are located in `crates/roseui-db/migrations/`
...
DbError (database layer)
  ↓ From trait implementation (roseui-db/src/error.rs)
...
`roseui-app` defines `AppServices`, which holds all shared dependencies centrally:
...
- **Domain crates do not construct their own dependencies** — they only define what they need (RouterState), `roseui-app` handles assembly
...
- Validity: 24 hours
- Payload: `user_id`, `username`, `iat`, `exp`, `iss` ("roseui"), `aud` ("roseui-webui")
- Secret source priority: environment variable → database → random generation (64 bytes, getrandom)
- Token extraction priority: `Authorization: Bearer` header → `roseui-session` cookie
- Supports token blacklist (SHA-256 hash, DashMap storage)

### CSRF Protection

Uses the Double Submit Cookie pattern:
- Cookie name: `roseui-csrf-token` (not HttpOnly — JavaScript must read it)
...
|--------|----------|--------|----------|---------|
| `roseui-session` | ✅ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
| `roseui-csrf-token` | ❌ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
...
| E2E tests | `crates/roseui-app/tests/` | In-memory SQLite | Full HTTP request chain verification |
...
`roseui-app/tests/common/mod.rs` provides shared test utilities:
...
**Step 3: If database access is needed, add to roseui-db**
...
**Step 4: If API types are needed, add to roseui-api-types**

Define request/response types in `roseui-api-types` to keep API contracts centrally managed.

**Step 5: Wire into roseui-app**

1. Add dependency in `roseui-app/Cargo.toml`:
   ```toml
   roseui-my-feature.workspace = true
   ```
...
- [ ] Repository trait defined in roseui-db, implementation uses Sqlite prefix
- [ ] API types defined in roseui-api-types
...
`crates/roseui-runtime/src/node_runtime/`. Packaged builds activate Node
...
`fn main()` calls `roseui_runtime::enhance_process_path()` **before** the
...
`roseui_runtime::Builder::agent(program)` (for long-running agent CLIs
whose stdio the caller owns) or `roseui_runtime::Builder::clean_cli(program)`
