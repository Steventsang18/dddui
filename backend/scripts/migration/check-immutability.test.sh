...
init_case_repo() {
    ...
    mkdir -p "$dir/crates/roseui-db/migrations"
    (
        cd "$dir"
        git init -q -b main
        git config user.email test@example.com
        git config user.name "Migration Test"
        printf '%s\n' '-- 001 initial' > crates/roseui-db/migrations/001_initial_schema.sql
        printf '%s\n' '-- 002 data fix' > crates/roseui-db/migrations/002_data_fix.sql
        printf '%s\n' '-- auxiliary sql' > crates/roseui-db/migrations/manual_fixture.sql
        git add crates/roseui-db/migrations
...
printf '%s\n' '-- modified' >> "$modified_repo/crates/roseui-db/migrations/001_initial_schema.sql"
...
rm "$deleted_repo/crates/roseui-db/migrations/002_data_fix.sql"
...
printf '%s\n' '-- modified auxiliary sql' >> "$auxiliary_repo/crates/roseui-db/migrations/manual_fixture.sql"
...
printf '%s\n' '-- 003 new migration' > "$added_repo/crates/roseui-db/migrations/003_new_change.sql"
run_in_repo "$added_repo" 0 "Migration immutability check passed" \
    env AIONCORE_MIGRATION_BASE_REF=main bash "$script"

duplicate_repo="$(init_case_repo duplicate)"
printf '%s\n' '-- duplicate 002 migration' > "$duplicate_repo/crates/roseui-db/migrations/002_duplicate_change.sql"
run_in_repo "$duplicate_repo" 1 "Duplicate database migration versions are not allowed" \
    env AIONCORE_MIGRATION_BASE_REF=main bash "$script"

override_repo="$(init_case_repo override)"
printf '%s\n' '-- modified with explicit override' >> "$override_repo/crates/roseui-db/migrations/001_initial_schema.sql"
