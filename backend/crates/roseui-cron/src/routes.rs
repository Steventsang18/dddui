...
async fn system_resume(
    ...
    let is_internal = headers.get("x-roseui-internal").and_then(|value| value.to_str().ok()) == Some("1");
    if !is_internal {
...
async fn create_conversation_cron(
    ...
    let conversation_id = header_value(&headers, "x-roseui-conversation-id")?;
    let Json(req) = body.map_err(ApiError::from)?;
    let resp = state
        .cron_service
...
fn trusted_header_user_id(headers: &HeaderMap, current_user: &CurrentUser) -> Result<String, ApiError> {
    let header_user_id = header_value(headers, "x-roseui-user-id")?;
...
mod tests {
    ...
    fn header_value_rejects_missing_or_blank_required_header() {
        ...
        assert!(matches!(
            header_value(&headers, "x-roseui-user-id"),
            Err(ApiError::BadRequest(message)) if message.contains("x-roseui-user-id")
        ));

        headers.insert("x-roseui-user-id", "   ".parse().unwrap());
    ...
    fn header_value_trims_required_header() {
        let mut headers = HeaderMap::new();
        headers.insert("x-roseui-user-id", " user_1 ".parse().unwrap());

        assert_eq!(header_value(&headers, "x-roseui-user-id").unwrap(), "user_1");
    }

    #[test]
    fn trusted_header_user_id_rejects_user_mismatch() {
        let mut headers = HeaderMap::new();
        headers.insert("x-roseui-user-id", "user_b".parse().unwrap());
        let current_user = CurrentUser {
            id: "user_a".to_owned(),
            username: "alice".to_owned(),
            user_type: roseui_db::UserType::Aionpro,
    ...
    fn trusted_header_user_id_accepts_authenticated_user() {
        let mut headers = HeaderMap::new();
        headers.insert("x-roseui-user-id", "user_a".parse().unwrap());
        let current_user = CurrentUser {
            id: "user_a".to_owned(),
            username: "alice".to_owned(),
            user_type: roseui_db::UserType::Aionpro,
    ...
    fn conversation_error_maps_through_boundary_mapper() {
        let err: ApiError =
            CronError::Conversation(roseui_conversation::ConversationError::NotFound { id: "conv-1".into() }).into();
