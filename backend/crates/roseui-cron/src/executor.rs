...
use roseui_ai_agent::task_manager::IWorkerTaskManager;
use roseui_api_types::{AssistantConversationRequest, CreateConversationRequest};
use roseui_common::{
    AgentType, ProviderWithModel, WorkspacePathValidationError, now_ms, validate_workspace_path_availability,
};
use roseui_conversation::{
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<roseui_db::models::ConversationRow>, CronError> {
    ...
    pub async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<roseui_db::models::ConversationAssistantSnapshotRow>, CronError> {
        let Some(user_id) = self
            .conversation_repo
            .owner_user_id(conversation_id)
            .await
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: roseui_common::now_ms(),
        };
...
mod tests {
    ...
    use roseui_ai_agent::AgentStreamEvent;
    use roseui_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use roseui_ai_agent::protocol::events::{FinishEventData, TextEventData};
    use roseui_ai_agent::types::{BuildTaskOptions, SendMessageData};
    use roseui_api_types::{AgentModeResponse, ConfigOptionConfirmation, SetConfigOptionResponse, WebSocketMessage};
    use roseui_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use roseui_db::{
    ...
    fn sample_job() -> CronJob {
        CronJob {
            ...
            agent_config: Some(CronAgentConfig {
                ...
                model: None,
                config_options: None,
                workspace: Some(ensure_named_workspace_path("roseui-cron-sample-job-workspace")),
            }),
    ...
    async fn prepare_scheduled_rejects_cross_user_existing_conversation() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("roseui-cron-cross-user-conversation-workspace")
            }),
        ));
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(roseui_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    async fn build_conversation_extra_preserves_agent_workspace() {
        ...
        assert_eq!(
            extra["workspace"],
            ensure_named_workspace_path("roseui-cron-sample-job-workspace")
        );
    }
    ...
    async fn execute_inner_uses_conversation_workspace_when_job_workspace_missing() {
        ...
        assert_eq!(
            ...
            ensure_named_workspace_path("roseui-cron-existing-conversation-workspace")
        );
    ...
    async fn execute_inner_persists_agent_workspace_when_conversation_workspace_missing() {
        ...
        assert_eq!(
            value["workspace"],
            ensure_named_workspace_path("roseui-cron-agent-workspace")
        );
    }
    ...
    async fn execute_inner_inserts_right_side_user_message_for_cron_prompt() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("roseui-cron-existing-conversation-workspace")
            }),
        ));
    ...
    fn make_executor_for_busy_tests() -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            ) -> Result<(), roseui_ai_agent::AgentError> {
                Ok(())
            }
            fn kill_and_wait(
                &self,
            ...
            fn collect_idle(&self, _: roseui_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            ...
            async fn owner_user_id(&self, _id: &str) -> Result<Option<String>, roseui_db::DbError> {
                Ok(None)
            }
            async fn create(&self, _row: &roseui_db::models::ConversationRow) -> Result<(), roseui_db::DbError> {
                Ok(())
            }
            async fn update(
                &self,
            ...
            async fn insert_message(
                &self,
                _user_id: &str,
                _message: &roseui_db::models::MessageRow,
            ) -> Result<(), roseui_db::DbError> {
                Ok(())
            }
        ...
        impl roseui_realtime::EventBroadcaster for StubBroadcaster {
        ...
        let stub_broadcaster: Arc<dyn roseui_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn roseui_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn roseui_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl RecordingAgent {
        fn new(conversation_id: &str, mode: &str, initialized: bool) -> Self {
            ...
            Self {
                conversation_id: conversation_id.to_owned(),
                workspace: ensure_named_workspace_path("roseui-cron-agent-workspace"),
                event_tx,
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), roseui_ai_agent::AgentSendError> {
        ...
        async fn cancel(&self) -> Result<(), roseui_ai_agent::AgentError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), roseui_ai_agent::AgentError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, roseui_ai_agent::AgentError> {
        ...
        async fn set_config_option(
            ...
            option_id: &str,
            value: &str,
        ) -> Result<SetConfigOptionResponse, roseui_ai_agent::AgentError> {
            if option_id != "mode" {
                return Err(roseui_ai_agent::AgentError::bad_request(format!(
    ...
    impl IWorkerTaskManager for FailingTaskManager {
        ...
        async fn get_or_build_task(
            ...
            Err(roseui_ai_agent::AgentError::bad_gateway(self.message.clone()))
        }
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(
            ...
            Ok(Some(roseui_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "user1".into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: roseui_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<roseui_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<roseui_db::models::MessageRow> {
    ...
    impl roseui_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        ) -> Result<ConversationArtifactRow, roseui_db::DbError> {
            self.operations
                .lock()
                .unwrap()
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn roseui_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    fn make_executor_with_task_manager_repo_and_broadcaster(
        task_manager: Arc<dyn IWorkerTaskManager>,
        repo: Arc<dyn IConversationRepository>,
        broadcaster: Arc<dyn roseui_realtime::EventBroadcaster>,
    ) -> JobExecutor {
        struct StubSkillResolver;
    ...
    #[async_trait::async_trait]
    impl roseui_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get_for_user(
            &self,
            _user_id: &str,
            _conversation_id: &str,
        ) -> Result<Option<roseui_db::models::AcpSessionRow>, roseui_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &roseui_db::CreateAcpSessionParams<'_>,
        ) -> Result<roseui_db::models::AcpSessionRow, roseui_db::DbError> {
            Err(roseui_db::DbError::Init("stub".into()))
        }
        ...
        async fn delete_for_user(&self, _user_id: &str, _conversation_id: &str) -> Result<bool, roseui_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state_for_user(
            ...
            _user_id: &str,
            _conversation_id: &str,
        ) -> Result<Option<roseui_db::PersistedSessionState>, roseui_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state_for_user(
            ...
            _user_id: &str,
            _conversation_id: &str,
            _params: &roseui_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, roseui_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl roseui_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(&self) -> Result<Vec<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            Ok(Vec::new())
        }
        async fn list_all_for_user(
            &self,
            _user_id: &str,
        ) -> Result<Vec<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            self.list_all().await
        }
        async fn get(&self, _id: &str) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            Ok(None)
        }
        async fn get_for_user(
            &self,
            _user_id: &str,
            id: &str,
        ) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            self.get(id).await
        }
        ...
        async fn upsert(
            &self,
            _params: &roseui_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<roseui_db::models::AgentMetadataRow, roseui_db::DbError> {
            Err(roseui_db::DbError::Init("stub".into()))
        }
        async fn upsert_for_user(
            &self,
            _user_id: &str,
            params: &roseui_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<roseui_db::models::AgentMetadataRow, roseui_db::DbError> {
        ...
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &roseui_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            Ok(None)
        }
        async fn apply_handshake_for_user(
            ...
            _user_id: &str,
            id: &str,
            params: &roseui_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
        ...
        async fn update_availability_snapshot(
            &self,
            _id: &str,
            _params: &roseui_db::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
            Ok(None)
        }
        async fn update_availability_snapshot_for_user(
            ...
            _user_id: &str,
            id: &str,
            params: &roseui_db::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<roseui_db::models::AgentMetadataRow>, roseui_db::DbError> {
        ...
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, roseui_db::DbError> {
            Ok(false)
        }
        ...
        async fn delete(&self, _id: &str) -> Result<bool, roseui_db::DbError> {
            Ok(false)
        }
        async fn delete_for_user(&self, _user_id: &str, id: &str) -> Result<bool, roseui_db::DbError> {
