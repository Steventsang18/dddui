...
use roseui_db::{ITeamRepository, SqliteTeamRepository, init_database_memory};
use roseui_team::{TaskBoard, TaskStatus, TaskUpdate};

async fn setup() -> (TaskBoard, roseui_db::Database) {
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteTeamRepository::new(db.pool().clone()));
    repo.create_team(&roseui_db::models::TeamRow {
        id: "t1".to_owned(),
