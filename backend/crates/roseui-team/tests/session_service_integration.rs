...
use roseui_ai_agent::task_manager::AgentFactory;
use roseui_ai_agent::types::BuildTaskOptions;
use roseui_ai_agent::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
...
use roseui_common::{AgentKillReason, AgentType, PaginatedResult, ProviderWithModel};
...
use roseui_team::session::SpawnAgentRequest;
use roseui_team::{
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root =
            std::env::temp_dir().join(format!("roseui-team-fake-workspaces-{}", roseui_common::generate_id()));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    ) -> Result<TeamConversationCreateResult, roseui_team::TeamError> {
        if request.assistant_id.is_some() && request.agent_type.is_some() {
            return Err(roseui_team::TeamError::InvalidRequest(
                "assistant-backed team conversations must not provide agent_type".into(),
            ));
        }
        let id = roseui_common::generate_id();
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, roseui_team::TeamError> {
    ...
    async fn conversation_assistant_id(&self, conversation_id: &str) -> Result<Option<String>, roseui_team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(
        ...
        _user_id: &str,
        team_id: &str,
    ) -> Result<String, roseui_team::TeamError> {
        if self
    ...
    async fn patch_runtime_config(
        ...
        let user_id = self
            ...
            .await?
            .ok_or_else(|| roseui_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        self.repo
            .update(
                &user_id,
                conversation_id,
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(roseui_common::now_ms()),
                    project_id: None,
                    folder_id: None,
                },
            )
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), roseui_team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, roseui_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| roseui_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
    ...
    async fn warmup_agent_process(
        ...
        let row = self.repo.get(user_id, conversation_id).await?.ok_or_else(|| {
            roseui_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
        })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = roseui_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                roseui_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, roseui_team::TeamError> {
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), roseui_team::TeamError> {
        let user_id = self
            .repo
            .owner_user_id(&row.conversation_id)
            .await?
            .ok_or_else(|| roseui_team::TeamError::InvalidRequest("conversation not found".into()))?;
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, roseui_team::TeamError> {
        let Some(user_id) = self.repo.owner_user_id(conversation_id).await? else {
            return Ok(None);
        };
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<roseui_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &roseui_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams_for_restore(&self) -> Result<Vec<roseui_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<roseui_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, user_id: &str, id: &str) -> Result<Option<roseui_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team_for_restore(&self, id: &str) -> Result<Option<roseui_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, user_id: &str, id: &str, params: &roseui_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = roseui_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, user_id: &str, row: &roseui_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<roseui_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, user_id: &str, row: &roseui_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<roseui_db::models::TeamTaskRow>, DbError> {
    ...
    async fn update_task(
        ...
        team_id: &str,
        task_id: &str,
        params: &roseui_db::UpdateTaskParams,
    ) -> Result<(), DbError> {
        self.inner.update_task(user_id, team_id, task_id, params).await
    }
    async fn list_tasks(&self, user_id: &str, team_id: &str) -> Result<Vec<roseui_db::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<roseui_ai_agent::AgentInstance> {
    ...
    ) -> Result<roseui_ai_agent::AgentInstance, AgentError> {
    ...
    fn collect_idle(&self, idle_threshold_ms: roseui_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    ...
    use roseui_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use roseui_ai_agent::protocol::events::AgentStreamEvent;
    use roseui_ai_agent::types::SendMessageData;
    use roseui_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), roseui_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(roseui_ai_agent::AgentInstance::Mock(Arc::new(
...
fn test_acp_build_options(conversation_id: String, workspace: String) -> BuildTaskOptions {
    BuildTaskOptions::new(AgentSessionContext {
        conversation: ConversationContext {
            conversation_id,
            user_id: "user1".into(),
            agent_type: roseui_common::AgentType::Acp,
            source: None,
        },
        workspace: WorkspaceContext {
            path: workspace.clone(),
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self, _user_id: &str) -> Result<Vec<roseui_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _user_id: &str, _id: &str) -> Result<Option<roseui_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: roseui_db::CreateProviderParams<'_>,
    ) -> Result<roseui_db::models::Provider, DbError> {
    ...
    async fn update(
        ...
        _user_id: &str,
        _id: &str,
        _params: roseui_db::UpdateProviderParams<'_>,
    ) -> Result<roseui_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
fn setup_with_factory_metadata_team_repo_and_conversation_repo(
    ...
    let backend_binary_path = Arc::new(std::path::PathBuf::from("/tmp/dodiddoneui-test"));
...
async fn recovery_creates_system_run_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(
            "user1",
            &roseui_db::models::MailboxMessageRow {
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&roseui_db::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
            ...
            created_at: roseui_common::now_ms(),
            updated_at: roseui_common::now_ms(),
            project_id: None,
            folder_id: None,
        })
        .await
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("roseui-team-user-workspace-{}", roseui_common::generate_id()));
...
async fn create_team_side_branch_backfills_project_binding_when_injected() {
    ...
    .await
    .unwrap();
    let store: Arc<dyn roseui_db::IProjectStore> = Arc::new(roseui_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    svc.with_project_service(Arc::new(roseui_project::ProjectService::new(
        store,
        std::env::temp_dir().join("roseui-team-bind-test-conversations"),
    )));

    let workspace_dir = std::env::temp_dir().join(format!("roseui-team-bind-{}", roseui_common::generate_id()));
    std::fs::create_dir_all(&workspace_dir).unwrap();
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(roseui_team::TeamError::TeamNotFound(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("roseui-team-workspace-{}", roseui_common::generate_id()));
...
async fn add_agent_uses_team_temp_workspace_when_team_and_leader_workspaces_are_unusable() {
    ...
    conv_repo
        .patch_extra(
            ...
            serde_json::json!({ "workspace": "/tmp/roseui-team-missing-leader-workspace" }),
        )
        .unwrap();
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, roseui_team::TeamError::TeamNotFound(_)));
