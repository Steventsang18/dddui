...
async fn sb1_bridge_config_generation() {
    use roseui_team::{TeamMcpStdioConfig, TeamMcpStdioServerSpec};
    ...
    let config = TeamMcpStdioConfig {
        ...
        slot_id: "lead-1".into(),
        binary_path: "/bin/dodiddoneui".into(),
    };

    let spec = TeamMcpStdioServerSpec::from_config("/bin/dodiddoneui", &config);
