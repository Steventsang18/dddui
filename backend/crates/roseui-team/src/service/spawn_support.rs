...
use roseui_db::models::AgentMetadataRow;
use roseui_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use std::sync::Arc;
...
mod tests {
    ...
    use roseui_db::models::{AgentMetadataRow, AssistantDefinitionRow, AssistantOverlayRow, Provider};
    use roseui_db::{
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: roseui_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
    ...
    async fn add_agent_rejects_assistant_missing_from_team_selectable_catalog() {
        ...
        let err = svc
            .add_agent(
                "user1",
                &created.id,
                roseui_api_types::AddAgentRequest {
                    name: "Worker".into(),
                    role: "teammate".into(),
                    backend: None,
