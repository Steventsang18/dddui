...
use roseui_db::models::{MailboxMessageRow, TeamRow, TeamTaskRow};
use roseui_db::{DbError, ITeamRepository, UpdateTaskParams, UpdateTeamParams};
use std::sync::Mutex;
...
pub(crate) mod workspace_harness {
    ...
    use roseui_common::{AgentKillReason, AgentType, PaginatedResult, now_ms};
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(
            &self,
            _user_id: &str,
            _row: &roseui_db::models::MailboxMessageRow,
        ) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(
            ...
            _team_id: &str,
            _task_id: &str,
            _params: &roseui_db::UpdateTaskParams,
        ) -> Result<(), DbError> {
            Ok(())
        }
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<roseui_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: roseui_ai_agent::types::BuildTaskOptions,
        ...
        fn collect_idle(&self, _idle_threshold_ms: roseui_common::TimestampMs) -> Vec<String> {
            vec![]
        }
    }
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![roseui_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
