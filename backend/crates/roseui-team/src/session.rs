...
use roseui_common::{AgentKillReason, generate_id};
...
pub struct WakeInput {
    ...
    /// as left bubbles (matches RoseUi `TeammateManager.wake()`). These are
...
pub struct TeamSession {
    ...
    /// agent's conversation as left bubbles (RoseUi parity: see
...
impl TeamSession {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let auth_token = roseui_common::generate_id();
...
mod tests {
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<roseui_db::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &roseui_db::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    async fn stdio_spec_uses_fixed_name_and_binary_path() {
        ...
        assert_eq!(spec.command, "/tmp/dodiddoneui-test");
    ...
    async fn shutdown_without_active_run_opens_system_lifecycle_run() {
        ...
        assert_eq!(payload.source, roseui_api_types::TeamRunSource::SystemLifecycle);
    ...
    async fn message_to_runtime_starting_member_returns_blocked_runtime_starting() {
        ...
        assert_eq!(
            ...
            Some(roseui_api_types::TeamSlotBlockedReason::RuntimeStarting)
        );
        session.stop();
    }
