...
//! backends. It lives in `roseui-common` (below every consumer: `roseui-session`,
//! `roseui-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `roseui-ai-agent::manager::acp::stderr_error_extractor` (git
