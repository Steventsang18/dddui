...
use roseui_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use roseui_common::{TimestampMs, now_ms};
use roseui_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "roseui";
...
impl McpOAuthService {
    ...
    pub async fn logout(&self, user_id: &str, server_url: &str) -> Result<(), McpError> {
        match self.token_repo.delete(user_id, server_url).await {
            ...
            Err(roseui_db::DbError::NotFound(_)) => {
    ...
    async fn handle_callback_connection(
        ...
        let response = "HTTP/1.1 200 OK\r\n\
            ...
            <p>You can close this window and return to RoseUi.</p>\
            </body></html>";
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(
            ...
            _: &str,
            _: &str,
        ) -> Result<Option<roseui_db::models::OAuthTokenRow>, roseui_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<roseui_db::models::OAuthTokenRow, roseui_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str, _: &str) -> Result<(), roseui_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self, _: &str) -> Result<Vec<String>, roseui_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, _: &str, url: &str) -> Result<(), roseui_db::DbError> {
            Err(roseui_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(
            ...
            Ok(Some(roseui_db::models::OAuthTokenRow {
