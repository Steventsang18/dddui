//! Integration tests for file-based MCP Agent adapters (Opencode, Aionrs, Roseui).
//!
...
//!
//! For Roseui, we use a mock repository since it reads from the DB.
...
use roseui_mcp::{RoseuiAdapter, McpAgentAdapter, McpServerTransport};

// ===========================================================================
// Roseui adapter (DB-backed)
// ===========================================================================

mod roseui {
    use super::*;
    use roseui_db::models::McpServerRow;
    use roseui_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: &str,
            _s: &str,
            _lc: Option<roseui_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = RoseuiAdapter::new(repo.clone());
...
mod opencode {
    use super::*;
    use roseui_mcp::OpencodeAdapter;
