...
use roseui_db::SqliteMcpServerRepository;
use roseui_mcp::{DetectedServer, McpAgentAdapter, McpError, McpServerTransport, McpSyncService};
...
async fn make_service(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> McpSyncService {
    ...
    let repo: Arc<dyn roseui_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
