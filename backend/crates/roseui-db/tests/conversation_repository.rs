...
async fn setup() -> (SqliteConversationRepository, roseui_db::Database) {
    let db = init_database_memory().await.unwrap();
    let repo = SqliteConversationRepository::new(db.pool().clone());
    (repo, db)
}

async fn create_user_2(db: &roseui_db::Database) {
    sqlx::query(
...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        id: roseui_common::generate_prefixed_id("conv"),
...
fn make_message(conv_id: &str, content: &str) -> MessageRow {
    let now = roseui_common::now_ms();
    MessageRow {
        id: roseui_common::generate_prefixed_id("msg"),
        conversation_id: conv_id.to_string(),
        msg_id: Some(roseui_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: &str, artifact_id: &str) -> roseui_db::ConversationArtifactRow {
    roseui_db::ConversationArtifactRow {
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("roseui-pinned");
    c1.source = Some("roseui".to_string());
    c1.pinned = true;
    c1.pinned_at = Some(roseui_common::now_ms());
    repo.create(&c1).await.unwrap();

    let mut c2 = make_conversation("telegram-pinned");
    c2.source = Some("telegram".to_string());
    c2.pinned = true;
    c2.pinned_at = Some(roseui_common::now_ms());
    repo.create(&c2).await.unwrap();

    let mut c3 = make_conversation("roseui-unpinned");
    c3.source = Some("roseui".to_string());
    c3.pinned = false;
    repo.create(&c3).await.unwrap();

    // Filter: source=roseui AND pinned=true
    let result = repo
        .list_paginated(
            USER_ID,
            &ConversationFilters {
...
async fn concurrent_insert_message_does_not_require_sequence_allocation() {
    ...
    let db = roseui_db::init_database(&dir.path().join("messages.db")).await.unwrap();
    let repo = SqliteConversationRepository::new(db.pool().clone());
...
async fn anchor_rejects_legacy_artifact_rows() {
    ...
    assert!(matches!(err, roseui_db::DbError::NotFound(_)));
}
...
async fn pin_and_unpin_conversation() {
    ...
    // Pin
    let pin_time = roseui_common::now_ms();
    repo.update(
        &conv.user_id,
        &conv.id,
        &ConversationRowUpdate {
