...
async fn seed_legacy_assistant_identity(pool: &sqlx::SqlitePool) {
    ...
    for (definition_id, assistant_key, agent_backend, source_ref) in [
        ("def-aionrs", "roseui-assistant", "aionrs", "roseui-assistant"),
...
async fn migration_013_normalizes_legacy_cron_agent_identity() {
    ...
    insert_legacy_cron(
        ...
        "conv_aionrs",
        "aionrs",
        r#"{"backend":"provider-1","name":"Aion","assistant_id":"roseui-assistant","model_id":"gpt-5"}"#,
    )
    .await;
    insert_legacy_cron(
        &pool,
    ...
    .await
    .unwrap();
    assert_eq!(aionrs.get::<String, _>("assistant_id"), "roseui-assistant");
