//! RosePro adoption coverage gate.
//!
//! `adopt_system_default_data` re-owns the local default user's data to the
//! machine's first external (RosePro) account by convention: any table with a
...
const GLOBAL_TABLES: &[(&str, &str)] = &[
    ...
    // Local single-user knowledge base (wiki). Not user-private data and must
    // not be re-owned by RosePro adoption — the wiki is machine-local by design.
    ("wiki_pages", "machine-local knowledge base, not user-scoped"),
    ("wiki_tags", "machine-local knowledge base, not user-scoped"),
    ("wiki_page_tags", "machine-local knowledge base, not user-scoped"),
    ("wiki_links", "machine-local knowledge base, not user-scoped"),
    ("wiki_edges", "machine-local knowledge base, not user-scoped"),
    ("wiki_fts", "FTS5 index over wiki_pages, machine-local"),
    ("wiki_fts_config", "FTS5 shadow table, machine-local"),
    ("wiki_fts_data", "FTS5 shadow table, machine-local"),
    ("wiki_fts_docsize", "FTS5 shadow table, machine-local"),
    ("wiki_fts_idx", "FTS5 shadow table, machine-local"),
];
...
async fn every_table_is_classified_for_aionpro_adoption() {
    ...
    assert!(
        ...
        "NEW TABLE(S) WITHOUT AN ROSEPRO ADOPTION DECISION: {unclassified:?}\n\
         ...
         See crates/roseui-db/tests/adoption_coverage.rs."
    );
...
async fn seed_default_user_project(pool: &sqlx::SqlitePool) -> (String, String) {
    ...
    let (project, entry) = store
        .create_project_with_workspace_entry(
            ...
            &folder.folder_id,
            "adopt-ws",
            roseui_db::ProjectKind::Standard,
        )
        .await
        .unwrap();
...
async fn adoption_fires_once_then_new_local_data_stays_local() {
    ...
    // RoseUi keeps working locally: NEW data lands on the default user.
    sqlx::query(
        "INSERT INTO conversations (id, user_id, name, type, extra, status, created_at, updated_at)
