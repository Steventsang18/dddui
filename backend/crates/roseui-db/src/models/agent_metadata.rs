...
pub struct AgentMetadataRow {
    ...
    /// Native mode id that RoseUi's legacy `yolo` / `yoloNoSandbox`
    ...
    /// (`roseui-common/src/enums.rs`), a hardcoded per-backend table whose
    /// default arm is the literal `"yolo"` — see Team spawn
    /// (`roseui-team/src/service/spawn_support.rs`) and cron
    /// (`roseui-cron/src/service.rs`). So clearing this column silently hands
