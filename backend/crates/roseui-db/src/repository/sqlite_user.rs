...
impl IUserRepository for SqliteUserRepository {
    ...
    async fn create_user(&self, username: &str, password_hash: &str) -> Result<User, DbError> {
        let id = roseui_common::generate_prefixed_id("user");
        let now = roseui_common::now_ms();
    ...
    async fn adopt_system_default_data(&self, owner_id: &str) -> Result<u64, DbError> {
        ...
        // RoseUi conversations were repeatedly re-owned). A stamped marker
        ...
        // adoption-coverage classification test in roseui-db/tests.
        let mut moved: u64 = 0;
...
mod tests {
    ...
    async fn ensure_external_user_is_idempotent_and_has_no_password() {
        ...
        let projection = ExternalUserProjection {
            username: Some("RosePro User".to_string()),
        ...
        assert!(repo.find_by_username("RosePro User").await.unwrap().is_none());
    }
