...
use roseui_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
fn ensure_test_workspace_path() -> String {
    ensure_named_workspace_path("roseui-conversation-extended-test-project")
}
...
async fn t10_1_same_workspace() {
    let (svc, _repo, _b) = setup().await;
    let shared_workspace = ensure_named_workspace_path("roseui-conversation-extended-shared-workspace");
    let other_workspace = ensure_named_workspace_path("roseui-conversation-extended-other-workspace");
...
async fn t10_2_no_associated() {
    let (svc, _repo, _b) = setup().await;
    let workspace = ensure_named_workspace_path("roseui-conversation-extended-unique-workspace");
