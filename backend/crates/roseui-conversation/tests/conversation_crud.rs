...
use roseui_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
use roseui_conversation::skill_resolver::SkillResolver;
use roseui_conversation::{ConversationError, ConversationService};
use roseui_db::{SqliteConversationRepository, init_database_memory};
...
impl IWorkerTaskManager for NoopTaskManager {
    fn get_task(&self, _: &str) -> Option<roseui_ai_agent::AgentInstance> {
        None
    }
    async fn get_or_build_task(
        &self,
        _: &str,
        _: roseui_ai_agent::types::BuildTaskOptions,
...
async fn setup() -> (ConversationService, Arc<TestBroadcaster>, Arc<dyn IWorkerTaskManager>) {
    ...
    let agent_metadata_repo: Arc<dyn roseui_db::IAgentMetadataRepository> =
        Arc::new(roseui_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn roseui_db::IAcpSessionRepository> =
        Arc::new(roseui_db::SqliteAcpSessionRepository::new(db.pool().clone()));
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("roseui-conversation-crud-test-project");
...
async fn t4_4_extra_merge_preserves_existing_keys() {
    let (svc, _, task_mgr) = setup().await;
    let dir = std::env::temp_dir().join("roseui-conversation-crud-extra-merge");
...
async fn full_lifecycle_create_get_update_delete() {
    let (svc, broadcaster, task_mgr) = setup().await;
    let updated_workspace = std::env::temp_dir().join("roseui-conversation-crud-updated-workspace");
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use roseui_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(roseui_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn roseui_db::IAgentMetadataRepository> =
        Arc::new(roseui_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn roseui_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = roseui_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
