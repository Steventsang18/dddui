...
use roseui_ai_agent::protocol::events::{ErrorEventData, TipType};
use roseui_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use roseui_api_types::{AgentErrorCode, WebSocketMessage};
...
use roseui_db::IConversationRepository;
...
impl StreamRelay {
    ...
    async fn consume_inner(
        ...
        info!(
            target: "roseui_feedback_diagnostics",
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(roseui_ai_agent::protocol::events::FinishEventData::default()),
...
mod tests {
    ...
    use roseui_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    use roseui_db::DbError;
    ...
    impl SkillResolver for RecordingSkillResolverForRelay {
        ...
        async fn resolve_skills(&self, _names: &[String]) -> Vec<roseui_extension::ResolvedAgentSkill> {
            Vec::new()
        }
        ...
        async fn link_workspace_skills(
            ...
            _rel_dirs: &[&str],
            _skills: &[roseui_extension::ResolvedAgentSkill],
        ) -> usize {
            0
        }
    }
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(roseui_realtime::BroadcastEventBus::new(64));
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use roseui_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn acp_dialect_signal_is_not_forwarded_or_persisted() {
        use roseui_ai_agent::protocol::events::{AcpDialectSignalData, AcpDialectSignalKind};
    ...
    fn stream_relay_emits_feedback_runtime_terminal_log() {
        ...
        assert!(captured.contains("roseui_feedback_diagnostics"), "{captured}");
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            roseui_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: roseui_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
            },
        ))
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(roseui_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use roseui_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use roseui_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(roseui_realtime::BroadcastEventBus::new(64));
        let adapter =
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, roseui_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, roseui_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _user_id: &str, _id: &str) -> Result<Option<roseui_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn owner_user_id(&self, _id: &str) -> Result<Option<String>, DbError> {
            Ok(Some("user-1".into()))
        }
        async fn create(&self, _row: &roseui_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(
            &self,
            _user_id: &str,
            _id: &str,
            _updates: &roseui_db::ConversationRowUpdate,
        ) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &roseui_db::ConversationFilters,
        ) -> Result<roseui_common::PaginatedResult<roseui_db::models::ConversationRow>, DbError> {
            Ok(roseui_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<roseui_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<roseui_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            ...
            _user_id: &str,
            _conv_id: &str,
            _params: &roseui_db::MessagePageParams,
        ) -> Result<roseui_db::MessagePageResult, DbError> {
            Ok(roseui_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(
            ...
            _conversation_id: &str,
            id: &str,
            updates: &roseui_db::MessageRowUpdate,
        ) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<roseui_common::PaginatedResult<roseui_db::MessageSearchRow>, DbError> {
            Ok(roseui_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
