...
use roseui_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
use roseui_extension::{AssistantRuleDispatcher, ExtensionError};
...
#[path = "service_test/session_event_persist_test.rs"]
mod session_event_persist_test;
...
impl IConversationRepository for MockRepo {
    async fn get(&self, user_id: &str, id: &str) -> Result<Option<ConversationRow>, roseui_db::DbError> {
    ...
    async fn create(&self, row: &ConversationRow) -> Result<(), roseui_db::DbError> {
        self.rows.lock().unwrap().push(row.clone());
        Ok(())
    }

    async fn update(&self, user_id: &str, id: &str, updates: &ConversationRowUpdate) -> Result<(), roseui_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .iter_mut()
            .find(|r| r.user_id == user_id && r.id == id)
            .ok_or_else(|| roseui_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, user_id: &str, id: &str) -> Result<(), roseui_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| !(r.user_id == user_id && r.id == id));
        if rows.len() == len_before {
            return Err(roseui_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, roseui_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, roseui_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, roseui_db::DbError> {
        Ok(vec![])
    }
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| roseui_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn insert_message(&self, _user_id: &str, message: &MessageRow) -> Result<(), roseui_db::DbError> {
    ...
    async fn update_message(
        ...
        let message = messages
            ...
            .ok_or_else(|| roseui_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, _user_id: &str, conv_id: &str) -> Result<(), roseui_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, roseui_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(&self) -> Result<Vec<roseui_db::StaleRuntimeMessageRow>, roseui_db::DbError> {
        let messages = self.messages.lock().unwrap();
        let rows = self.rows.lock().unwrap();
        Ok(messages
            .iter()
            ...
            .filter_map(|message| {
                ...
                Some(roseui_db::StaleRuntimeMessageRow {
                    user_id,
    ...
    async fn list_artifacts(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<ConversationArtifactRow>, roseui_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, roseui_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, roseui_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("roseui-conversation-service-test-project");
...
fn unique_test_workspace_path(label: &str) -> PathBuf {
    let workspace = std::env::temp_dir()
        .join(format!("roseui-conversation-service-test-{label}"))
...
async fn insert_conversation_with_type(repo: &Arc<MockRepo>, user_id: &str, agent_type: AgentType) -> ConversationRow {
    let id = format!(
        ...
        roseui_common::generate_short_id()
    );
...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(resp.source, Some(ConversationSource::Roseui));
    assert!(!resp.pinned);
    ...
    assert_eq!(events[0].data["source"], "roseui");
}

// ── Project-bind side branch tests ─────────────────────────────────

async fn make_injected_project_service(temp_root: &std::path::Path) -> std::sync::Arc<roseui_project::ProjectService> {
    ...
    .await
    .unwrap();
    let store: std::sync::Arc<dyn roseui_db::IProjectStore> =
        std::sync::Arc::new(roseui_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    std::sync::Arc::new(roseui_project::ProjectService::new(
        store,
...
async fn create_auto_provisions_workspace_under_date_partition() {
    ...
    let workspace_root = temp.path().join("roseui-data");
...
async fn create_rejects_unavailable_workspace_with_trailing_whitespace_in_request() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let dir = std::env::temp_dir().join(format!("roseui-test-{}", roseui_common::generate_short_id()));
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, roseui_api_types::ConversationRuntimeStateKind::Idle);
...
async fn update_extra_merge() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "roseui-conversation-update-extra-merge-{}",
        roseui_common::generate_short_id()
    ));
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            roseui_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
fn assert_conversation_runtime_context(options: &BuildTaskOptions, user_id: &str, conversation_id: &str) {
    assert!(
        options
            .context
            .runtime_env
            .contains(&("ROSEUI_USER_ID".to_owned(), user_id.to_owned())),
        "runtime env should include ROSEUI_USER_ID"
    );
    assert!(
        options
            .context
            .runtime_env
            .contains(&("ROSEUI_CONVERSATION_ID".to_owned(), conversation_id.to_owned())),
        "runtime env should include ROSEUI_CONVERSATION_ID"
    );
    assert_eq!(
...
async fn send_message_injects_configured_runtime_helper_context() {
    ...
    let svc = svc.with_runtime_helper_context(
        "/Applications/RoseUi/dodiddoneui".to_owned(),
    ...
    assert!(
        options[0].context.runtime_env.contains(&(
            ROSEUI_HELPER_BIN_ENV.to_owned(),
            "/Applications/RoseUi/dodiddoneui".to_owned()
        )),
        "runtime env should include ROSEUI_HELPER_BIN"
    );
    assert!(
        options[0]
            .context
            .runtime_env
            .contains(&(ROSEUI_BASE_URL_ENV.to_owned(), "http://127.0.0.1:51234".to_owned())),
        "runtime env should include ROSEUI_BASE_URL"
    );
}
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", roseui_common::generate_short_id());
    repo.update(
        "user_1",
        &conv.id,
        &ConversationRowUpdate {
...
async fn send_message_records_agent_availability_feedback_on_send_failure() {
    ...
    create_req.source = Some(ConversationSource::Roseui);
...
async fn warmup_injects_runtime_token_for_mcp_team_conversation() {
    ...
    assert!(
        options[0]
            ...
            .runtime_env
            .iter()
            .any(|(key, value)| key == ROSEUI_RUNTIME_TOKEN_ENV && !value.is_empty()),
        "MCP Team conversations should receive ROSEUI_RUNTIME_TOKEN for CLI fallback"
    );
}
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = roseui_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(roseui_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
...
async fn seed_aionrs_conversation_with_snapshot(
    ...
    let row = ConversationRow {
        id: format!("aionrs-seed-{}", roseui_common::generate_short_id()),
        user_id: "user_1".into(),
