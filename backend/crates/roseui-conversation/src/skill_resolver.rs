...
/// Production adapter backed by `roseui_extension::skill_service`.
...
impl ExtensionSkillResolver {
    pub fn new(paths: Arc<roseui_extension::SkillPaths>, skill_repo: Arc<dyn ISkillRepository>) -> Self {
...
impl SkillResolver for ExtensionSkillResolver {
    async fn auto_inject_names(&self) -> Vec<String> {
        match roseui_extension::list_available_skills_with_repo(&self.paths, self.skill_repo.as_ref()).await {
            Ok(items) => {
                let mut names: Vec<String> = items
                    .into_iter()
                    .filter(|item| {
                        item.source == roseui_extension::SkillSource::Builtin
                            && item
                                .relative_location
                                .as_deref()
    ...
    async fn resolve_skills_for_user(&self, user_id: &str, names: &[String]) -> Vec<ResolvedAgentSkill> {
        ...
        match roseui_extension::materialize_skills_for_agent_with_repo_for_user(
            &self.paths,
    ...
    async fn link_workspace_skills(&self, workspace: &Path, rel_dirs: &[&str], skills: &[ResolvedAgentSkill]) -> usize {
        ...
        match roseui_extension::link_workspace_skills(workspace, rel_dirs, skills).await {
            Ok(n) => n,
            Err(e) => {
                tracing::warn!(
...
mod tests {
    use super::*;
    use roseui_db::{SqliteSkillRepository, UpsertSkillParams};
    ...
    async fn extension_resolver_reads_auto_inject_names_from_skill_catalog() {
        ...
        let paths = Arc::new(roseui_extension::SkillPaths {
        ...
        roseui_extension::sync_skill_catalog_into_repo(paths.as_ref(), repo.as_ref())
            .await
            .unwrap();
