...
use roseui_ai_agent::shared_kernel::{ConfigKey, ConfigValue, ModeId, ModelId, PersistedSessionState};
use roseui_ai_agent::types::BuildTaskOptions;
use roseui_api_types::{AcpBuildExtra, AionrsBuildExtra, TeamSessionBinding};
...
impl<'a> SessionContextBuilder<'a> {
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&roseui_db::models::AcpSessionRow>,
...
fn decode_persisted_session_state(state: roseui_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    fn workspace_validation_failure_logs_redacted_runtime_check() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let raw_path = "/tmp/roseui-secret-workspace-token-12345";
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            ("codex", serde_json::json!({ "workspace": "/tmp/roseui-codex-history" })),
            (
                "openclaw-gateway",
