...
use roseui_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a roseui_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    async fn resolve_message_attachments(
        ...
        let upload_root = std::env::temp_dir().join("roseui");
        project
    ...
    /// `delete()` removes the conversation row. Used by `roseui-app` to wire
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<roseui_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(roseui_api_types::ConversationAssistantIdentityResponse {
    ...
    /// to `roseui`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::Roseui);
        ...
        let row = roseui_db::models::ConversationRow {
            id: id.clone(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(roseui_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert_for_user(
                user_id,
                &roseui_db::UpsertAssistantPreferenceParams {
                    assistant_definition_id: &snapshot.assistant_definition_id,
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let user_msg = roseui_db::models::MessageRow {
...
impl ConversationService {
    ...
    fn runtime_token_for_build(
        ...
        // CLI (`dodiddoneui config` / `diagnose`) can authenticate in RosePro
    ...
    /// workspaces are constrained to RoseUi's generated path; custom
...
fn classify_repo_mcp_status(row: &roseui_db::models::McpServerRow, support: McpSupportPolicy) -> ConversationMcpStatus {
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::Roseui).unwrap(), "roseui");
    ...
    fn response_with_type(agent_type: roseui_common::AgentType) -> ConversationResponse {
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_roseui_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
