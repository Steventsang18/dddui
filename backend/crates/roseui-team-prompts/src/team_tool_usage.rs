use roseui_api_types::{TeamToolDescriptor, TeamToolRole, TeamToolTransport};
...
pub fn build_team_tool_usage(role: TeamPromptRole, transport: TeamToolTransport) -> String {
    ...
    let descriptors = roseui_api_types::team_tool_descriptors_for_role(tool_role);
    match transport {
...
use \"$ROSEUI_HELPER_BIN\" team capabilities or \"$ROSEUI_HELPER_BIN\" team help to inspect the Team contract,\n\
...
use the Team CLI fallback through \"$ROSEUI_HELPER_BIN\" team ... commands to continue Team coordination.\n\n\
...
fn render_cli_usage(role: TeamPromptRole, descriptors: &[TeamToolDescriptor]) -> String {
    let mut text = String::from(
        "You MUST use DoDidDoneUi Team CLI for ALL team coordination:\n\
\"$ROSEUI_HELPER_BIN\" team ...\n\n\
Run \"$ROSEUI_HELPER_BIN\" team capabilities when you need command names,\n\
stdin JSON schema, required fields, enum values, permissions, examples,\n\
or error meanings.\n\n\
Run \"$ROSEUI_HELPER_BIN\" team help when you need a short readable guide.\n\n\
...
mod tests {
    ...
    fn cli_teammate_usage_excludes_lead_only_tools() {
        ...
        assert!(usage.contains("\"$ROSEUI_HELPER_BIN\" team"));
    ...
    fn mcp_lead_usage_includes_lead_tools() {
        ...
        assert!(usage.contains("\"$ROSEUI_HELPER_BIN\" team capabilities"));
