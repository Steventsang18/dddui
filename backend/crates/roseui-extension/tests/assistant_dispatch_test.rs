...
use roseui_api_types::{ApiResponse, AssistantSource, SkillImportLimitsResponse};
use roseui_auth::CurrentUser;
use roseui_db::{UserStatus, UserType};
use roseui_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use roseui_extension::error::ExtensionError;
use roseui_extension::external_paths::ExternalPathsManager;
use roseui_extension::skill_routes::{SkillRouterState, skill_routes};
use roseui_extension::skill_service::SkillPaths;
use axum::Extension;
use axum::body::Body;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(roseui_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
