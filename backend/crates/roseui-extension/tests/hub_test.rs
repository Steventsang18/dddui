...
use roseui_extension::hub::{HubIndexManager, HubInstaller};
use roseui_extension::registry::ExtensionRegistry;
use roseui_extension::state::ExtensionStateStore;
use roseui_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![roseui_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: roseui_extension::types::ExtensionSource::Env,
    }];
