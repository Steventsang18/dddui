//! Filesystem side of RoseUi → RosePro adoption.
//!
//! DB adoption (`adopt_system_default_data`) re-owns the local default user's
//! rows to the first external (RosePro) user. Because per-user data now lives
...
pub async fn adopt_user_filesystem(
    ...
    let Ok(adopter_dir) = roseui_common::user_dir_name(adopter_user_id) else {
...
async fn rewrite_adopted_skill_paths(
    ...
    for row in rows {
        ...
        let params = roseui_db::UpsertSkillParams {
            name: &row.name,
...
mod tests {
    use super::*;
    use roseui_db::{SqliteSkillRepository, UpsertSkillParams, init_database_memory};
    ...
    async fn seed_user(db: &roseui_db::Database) -> String {
        use roseui_db::IUserRepository;
        roseui_db::SqliteUserRepository::new(db.pool().clone())
