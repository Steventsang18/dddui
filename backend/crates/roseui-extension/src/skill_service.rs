...
use roseui_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `roseui-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`ROSEUI_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../roseui-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "ROSEUI_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    /// Root data directory (~/.roseui/).
    pub data_dir: PathBuf,
    /// User-created skills directory (~/.roseui/skills/).
    pub user_skills_dir: PathBuf,
    /// Per-job cron skills directory (~/.roseui/cron/skills/).
    ...
    /// Assistant-level rules directory (~/.roseui/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.roseui/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.roseui/`) and
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = roseui_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            roseui_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
/// Startup sync for RosePro machines: built-in skills only.
///
...
async fn sync_builtin_skills_into_repo(paths: &SkillPaths, repo: &dyn ISkillRepository) -> Result<(), ExtensionError> {
    ...
    // owned by the job owner and maintained by roseui-cron, never synced
    // here as global rows.
...
mod tests {
    use super::*;
    use roseui_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    use std::io::Write;
