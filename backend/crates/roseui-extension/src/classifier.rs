...
//!
//! These traits live in `roseui-extension` (not `roseui-assistant`) so
//! `skill_routes` can depend on them without pulling `roseui-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `roseui-assistant::AssistantService`.

use roseui_api_types::AssistantSource;
...
///
/// Implemented by `roseui_assistant::AssistantService`; depended on by
