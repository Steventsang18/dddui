...
use roseui_api_types::{BedrockConfig, FetchModelsAnonymousRequest, FetchModelsRequest, FetchModelsResponse};
use roseui_common::decrypt_string;
...
mod tests {
    ...
    use roseui_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];
    const TEST_USER_ID: &str = "user-1";

    async fn setup() -> (ModelFetchService, roseui_db::Database) {
    ...
    async fn create_provider(db: &roseui_db::Database, platform: &str, base_url: &str, api_key: &str) -> String {
