...
use roseui_db::{IUserRepository, UserStatus, UserType};
...
/// and injected into agent subprocess environments as `ROSEUI_RUNTIME_TOKEN`.
pub const RUNTIME_TOKEN_HEADER: &str = "x-roseui-runtime-token";
/// Header carrying the acting user id asserted by the helper CLI.
pub const RUNTIME_USER_ID_HEADER: &str = "x-roseui-user-id";
/// Header carrying the conversation id the helper CLI runs inside.
pub const RUNTIME_CONVERSATION_ID_HEADER: &str = "x-roseui-conversation-id";
...
/// service; `roseui-auth` must not depend on `roseui-ai-agent` directly.
...
pub struct AuthState {
    ...
    /// (`dodiddoneui config` / `diagnose`), which cannot carry a JWT or cookies.
...
///
/// Flow:
/// 1. Extract bearer token from `Authorization` header or `roseui-session` cookie
...
pub async fn auth_middleware(
    ...
    if state.identity_mode == AuthIdentityMode::RosePro && user.user_type != UserType::Aionpro {
