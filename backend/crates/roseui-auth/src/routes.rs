...
use roseui_common::constants::COOKIE_MAX_AGE_DAYS;
use roseui_db::{DbError, IUserRepository, UserStatus, UserType, models::User};
...
const BOOTSTRAP_SECRET_HEADER: &str = "x-dodiddoneui-bootstrap-secret";
...
pub struct AuthRouterState {
    ...
    /// Optional on-disk adoption side-effect (RoseUi → RosePro upgrade).
...
async fn refresh_handler(
    ...
    if state.aionpro_mode && user.user_type != roseui_db::UserType::Aionpro {
        return Err(user_context_required());
    }
