...
use roseui_db::{ExternalUserProjection, IUserRepository, UserStatus, UserType, models::User};
...
pub enum ProvisionError {
    ...
    Db(#[from] roseui_db::DbError),
    #[error("token signing failed: {0}")]
    Token(#[from] crate::AuthError),
}

/// Port for the on-disk side of RoseUi → RosePro adoption. When DB adoption
...
/// in the composition layer over the skill filesystem so `roseui-auth` stays
...
impl AuthProvisionService {
    ...
    pub async fn ensure_external_user(
        ...
        // Upgrade path (RoseUi -> RosePro over the same database): while this
