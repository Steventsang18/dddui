...
use roseui_db::{IUserRepository, SqliteUserRepository, UserStatus, init_database_memory};
...
fn json_post_with_bootstrap_secret(uri: &str, body: &str, secret: &str) -> Request<Body> {
    Request::builder()
        ...
        .uri(uri)
        .header("content-type", "application/json")
        .header("x-dodiddoneui-bootstrap-secret", secret)
        .body(Body::from(body.to_owned()))
        .unwrap()
}
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find_map(|cookie| {
            cookie
                .split(',')
                .find(|part| part.trim_start().starts_with("roseui-session="))
                .and_then(|part| part.trim_start().strip_prefix("roseui-session="))
...
async fn t4_1_login_success() {
    ...
    assert!(set_cookie.contains("roseui-session="));
