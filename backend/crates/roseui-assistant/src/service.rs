...
use roseui_common::{generate_prefixed_id, now_ms};
use roseui_db::{
...
use roseui_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionError};
use serde_json;
...
fn assistant_error_is_busy(error: &AssistantError) -> bool {
    matches!(error, AssistantError::Internal(message) if roseui_db::message_indicates_busy(message))
}
...
fn assistant_error_is_unique(error: &AssistantError) -> bool {
    match error {
        ...
        AssistantError::Internal(message) => roseui_db::message_indicates_unique_violation(message),
        _ => false,
    }
}
...
pub struct AssistantService {
    ...
    /// Defaults to `~/.roseui/` but can be overridden for tests.
...
impl AssistantService {
    ...
    /// directory.
    ///
    /// There is no implicit `~/.roseui` fallback on purpose: an earlier
    /// version had one, and dev builds silently wrote rule files to the
    /// release directory while the db lived under `~/.roseui-dev/`,
    ...
    /// Bootstrap for RosePro machines: identical to
    ...
    pub async fn import_for_user(
        ...
        for entry in req.assistants {
            ...
            match self.repo.create_for_user(user_id, &params).await {
                ...
                Err(roseui_db::DbError::Conflict(_)) => {
    ...
    ///   override when `ROSEUI_BUILTIN_ASSISTANTS_PATH` is set).
    ...
    fn user_rules_dir_for_user(&self, user_id: &str) -> PathBuf {
        let dir = roseui_common::user_dir_name(user_id).unwrap_or_else(|_| user_id.to_owned());
...
impl AssistantService {
    ...
    fn definition_to_detail_response(
        ...
        definition: &AssistantDefinitionRow,
        state: Option<&AssistantOverlayRow>,
        preference: Option<&roseui_db::AssistantPreferenceRow>,
        rules_content: &str,
        projection: &AssistantRuntimeProjection,
    ) -> Result<AssistantDetailResponse, AssistantError> {
        let builtin_default = self.builtin_listing_default(definition);
...
mod tests {
    ...
    struct Fixture {
        ...
        override_repo: Arc<dyn IAssistantOverrideRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<roseui_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
        _db: roseui_db::Database,
    }

    #[derive(Clone, Default)]
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<roseui_api_types::AgentManagementRow>>>,
    }
    ...
    struct FixtureOpts {
        ...
        agent_rows: Vec<roseui_api_types::AgentManagementRow>,
    }
    ...
    async fn create_test_user(db: &roseui_db::Database, username: &str) -> String {
        let repo = roseui_db::SqliteUserRepository::new(db.pool().clone());
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: roseui_api_types::AgentManagementStatus,
    ) -> roseui_api_types::AgentManagementRow {
        roseui_api_types::AgentManagementRow {
            id: id.into(),
            ...
            agent_source: roseui_api_types::AgentSource::Builtin,
            agent_source_info: roseui_api_types::AgentSourceInfo::default(),
            enabled: true,
            installed: true,
            ...
            behavior_policy: roseui_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(roseui_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(roseui_api_types::AgentSnapshotCheckKind::Manual),
    ...
    fn mk_uninstalled_agent_row(id: &str, backend: &str) -> roseui_api_types::AgentManagementRow {
        let mut row = mk_agent_row(id, backend, roseui_api_types::AgentManagementStatus::Unchecked);
        row.installed = false;
    ...
    async fn bootstrap_skips_dirty_generated_assistant_definitions() {
        let fx = fixture().await;
        {
            ...
            *rows = vec![
                mk_agent_row("agent-dirty", "dirty", roseui_api_types::AgentManagementStatus::Online),
                mk_agent_row("agent-valid", "valid", roseui_api_types::AgentManagementStatus::Online),
            ];
        }
    ...
    async fn builtin_listing_uses_manifest_default_enabled_and_sort_order() {
        ...
        let mut butler = mk_builtin("roseui-assistant", "Butler");
        ...
        let butler_resp = list.iter().find(|a| a.id == "roseui-assistant").unwrap();
        assert!(butler_resp.enabled, "butler defaults to enabled");
        assert_eq!(butler_resp.sort_order, 0);
        let butler_idx = list.iter().position(|a| a.id == "roseui-assistant").unwrap();
    ...
    async fn list_maps_generated_definition_to_generated_source() {
        ...
        fx.agent_rows
            ...
            .push(mk_agent_row(
                "agent-claude",
                "claude",
                roseui_api_types::AgentManagementStatus::Online,
            ));
        fx.definition_repo
            .upsert(&UpsertAssistantDefinitionParams {
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        ...
        assert_eq!(bare.agent_status, roseui_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_unchecked_agent() {
        let mut unchecked_row = mk_agent_row(
            "agent-cursor",
            "cursor",
            roseui_api_types::AgentManagementStatus::Unchecked,
        );
        ...
        assert_eq!(bare.agent_status, roseui_api_types::AgentManagementStatus::Unchecked);
        assert!(bare.team_selectable);
    ...
    async fn list_hides_existing_generated_assistant_when_agent_is_uninstalled_until_installed() {
        ...
        uninstalled_row.status = roseui_api_types::AgentManagementStatus::Offline;
        let fx = fixture_with_options(FixtureOpts {
...

        {
            ...
            rows[0].status = roseui_api_types::AgentManagementStatus::Unchecked;
        }
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = roseui_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        // Engines like Rose CLI carry their identity in `agent_type` and leave
        ...
        agent_row.agent_type = roseui_common::AgentType::Aionrs;
    ...
    async fn aionrs_assistant_resolves_agent_status_via_agent_type_not_backend() {
        ...
        aionrs_row.agent_type = roseui_common::AgentType::Aionrs;
    ...
    async fn bootstrap_places_new_generated_assistants_before_existing_assistants() {
        let fx = fixture_with_options(FixtureOpts {
            ...
            agent_rows: vec![
                ...
                mk_agent_row("agent-codex", "codex", roseui_api_types::AgentManagementStatus::Online),
            ],
            ..Default::default()
        })
        .await;
    ...
    async fn bootstrap_reactivates_soft_deleted_builtin_definition_by_source_ref() {
        let mut builtin = mk_builtin("roseui-assistant", "RoseUi Butler");
        builtin.rule_file = Some("rules/roseui-assistant.{locale}.md".into());
        let fx = fixture_with_builtins(vec![builtin]).await;

        let original = fx
            .definition_repo
            .get_by_assistant_id("roseui-assistant")
            .await
            .unwrap()
        ...
        assert_eq!(restored.rule_resource_ref.as_deref(), Some("roseui-assistant"));
