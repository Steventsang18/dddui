...
impl crate::traits::IFileService for FileService {
    ...
    async fn copy_files_to_workspace(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut fail = |fp: &str, reason: &str| {
                failed.push(roseui_api_types::CopyFailure {
                    path: fp.to_owned(),
    ...
    async fn create_temp_file(&self, file_name: &str) -> Result<String, FileError> {
        ...
        tokio::task::spawn_blocking(move || {
            let tmp_dir = std::env::temp_dir().join("roseui");
    ...
    async fn create_upload_file(
        ...
        tokio::task::spawn_blocking(move || {
            let mut dir = std::env::temp_dir().join("roseui");
...
mod tests {
    ...
    fn list_workspace_files_sync_skips_directory_symlinks() {
        let dir = tempfile::tempdir().unwrap();
        let skill_dir = dir.path().join("builtin-skills/auto-inject/roseui-skills");
        fs::create_dir_all(&skill_dir).unwrap();
        fs::write(skill_dir.join("SKILL.md"), "---\ndescription: test\n---\nbody").unwrap();

        let workspace = dir.path().join("workspace/.claude/skills");
        fs::create_dir_all(&workspace).unwrap();
        std::os::unix::fs::symlink(&skill_dir, workspace.join("roseui-skills")).unwrap();

        let files = list_workspace_files_sync(&dir.path().join("workspace")).unwrap();

        assert!(
            files.iter().all(|f| f.name != "roseui-skills"),
    ...
    impl roseui_realtime::EventBroadcaster for NullBroadcaster {
