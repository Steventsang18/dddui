...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "roseui-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "roseui";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@roseui.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/roseui-snapshot-{hash}` (bare-style git dir).
