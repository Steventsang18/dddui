...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn roseui_db::IChannelRepository> = Arc::new(roseui_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(roseui_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn roseui_db::IClientPreferenceRepository> =
        Arc::new(roseui_db::SqliteClientPreferenceRepository::new(pool));
