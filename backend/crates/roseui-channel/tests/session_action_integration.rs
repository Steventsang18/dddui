...
use roseui_common::{generate_id, now_ms};
use roseui_db::models::AssistantUserRow;
use roseui_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use roseui_realtime::EventBroadcaster;

use roseui_channel::action::{ActionExecutor, MessageResult};
use roseui_channel::channel_settings::ChannelSettingsService;
use roseui_channel::pairing::PairingService;
use roseui_channel::session::SessionManager;
use roseui_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn roseui_db::IClientPreferenceRepository> =
        Arc::new(roseui_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
// roseui-db sqlite_channel.rs::update_session_conversation_persists.
