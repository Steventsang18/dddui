//! `roseui-session` — server-authoritative SessionState control plane (feature
//! 002). Domain layer.
//!
//! ## What this crate is
//! It spawns the claude CLI directly (via 001 `roseui-process`, no ACP/SDK),
...
//! `roseui-conversation` + WebSocket — all Non-Goal here, untouched. This crate
...
//!
//! ## Layering
//! Domain layer, parallel to `roseui-ai-agent`. Depends ONLY on
//! `roseui-process` (001) + `roseui-common`. NO production caller this
