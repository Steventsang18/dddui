...
//! the upper `roseui-ai-agent` Domain crate; `roseui-session` must stay
...
pub struct AcpConnection {
    ...
    command: roseui_common::CommandSpec,
}

impl AcpConnection {
    pub fn new(spawner: Arc<dyn Spawner>, command: roseui_common::CommandSpec) -> Self {
...
pub fn acp_capabilities() -> Capabilities {
    Capabilities {
        ...
        emits: SignalSet {
            ...
            // prompt response (no idle-timeout in DoDidDoneUi anyway, post-007).
            heartbeat: false,
            tool_lifecycle: true,
...
impl AcpSessionBackend {
    ...
    async fn bound_session(&self) -> Result<String, BackendError> {
        ...
        // (roseui-agent-rest INIT_TIMEOUT_SECS). A cold start / untrusted project slows
...
struct AcpWakeRecipe {
    ...
    command: Option<roseui_common::CommandSpec>,
    config: SessionConfig,
}
...
async fn handle_open_response(
    ...
    // protocols/design/dodiddoneui-opencode-acp-configoptions-model-mode-prompt.md.
    let mut disc = discovered.lock().unwrap_or_else(|e| e.into_inner());
...
async fn handle_reverse_rpc(
    ...
    match method {
        "session/request_permission" => {
            ...
            // card can show the approver WHAT they are approving (RoseUi issue
...
fn enrich_acp_error_message(raw: &str, stderr_tail: Option<&str>) -> String {
    ...
    if let Some(tail) = stderr_tail
        && let Some(cause) = roseui_common::error_extract::extract_error_message(tail)
    {
...
mod tests {
    ...
    fn parse_permission_metadata_extracts_claude_title_prefix_server() {
        ...
        let params = json!({
            ...
            "toolCall": { "title": "mcp__roseui-team__team_members", "rawInput": {} },
        });
        let meta = parse_permission_metadata(Some(&params)).expect("metadata present");
        assert_eq!(meta["server_name"], "roseui-team");
    ...
    fn parse_permission_metadata_extracts_codex_raw_input_server() {
        ...
        let params = json!({
            ...
            "toolCall": { "title": "mcp__other__x", "rawInput": { "server_name": "roseui-team-guide" } },
        });
        let meta = parse_permission_metadata(Some(&params)).expect("metadata present");
        assert_eq!(meta["server_name"], "roseui-team-guide", "rawInput.server_name wins");
    }
    ...
    async fn acp_answer_permission_echoes_real_offered_option_id_by_kind() {
        ...
        // RoseUi issue #3779: the card must show what is being approved — the
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        let command = roseui_common::CommandSpec {
            command: "hermes".into(),
            args: vec!["acp".into()],
            env: Vec::new(),
            cwd: None,
