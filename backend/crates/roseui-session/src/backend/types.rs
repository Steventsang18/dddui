...
impl BackendError {
    ...
    /// before.
    pub(crate) fn from_spawn(context: &'static str, e: roseui_process::ProcessError) -> Self {
        match e {
            roseui_process::ProcessError::WorkspaceUnavailable(path) => Self::WorkspaceUnavailable(path),
...
/// How a session is (re)opened. `session_id` is the STABLE DoDidDoneUi-minted
...
mod tests {
    ...
    fn from_spawn_keeps_workspace_unavailable_class() {
        let err = BackendError::from_spawn(
            "codex spawn failed",
            roseui_process::ProcessError::workspace_unavailable("/gone/workspace"),
        );
        assert_eq!(err, BackendError::WorkspaceUnavailable("/gone/workspace".into()));

        let err = BackendError::from_spawn("codex spawn failed", roseui_process::ProcessError::internal("boom"));
        assert_eq!(
            err,
