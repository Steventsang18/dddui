...
/// (roseui-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `ROSEUI_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("ROSEUI_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
/// its `McpServerConfig` map). Deliberately crate-local — `roseui-session` stays
/// self-contained (no `roseui-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// blob). Kept as `serde_json::Value` so `roseui-session` need not depend on
...
pub struct SessionConfig {
    ...
    pub spawn_env: Vec<roseui_common::EnvVar>,
