...
impl WorkerTaskManagerImpl {
    ...
    fn refresh_runtime_token_for_new_task(&self, options: &mut BuildTaskOptions) {
        ...
        options
            .context
            .runtime_env
            .retain(|(key, _)| key != ROSEUI_RUNTIME_TOKEN_ENV);
        options
            .context
            .runtime_env
            .push((ROSEUI_RUNTIME_TOKEN_ENV.to_owned(), issue.token));
    }
}
...
/// Wired up by `roseui-app` so deleting a conversation tears down its
...
mod tests {
    ...
    fn make_team_options_with_runtime_token(conversation_id: &str, runtime_token: &str) -> BuildTaskOptions {
        ...
        let team = roseui_api_types::TeamSessionBinding {
            team_id: "team-1".into(),
            ...
            move |opts: BuildTaskOptions| {
                ...
                async move {
                    if let Some((_, token)) = opts
                        ...
                        .runtime_env
                        .iter()
                        .find(|(key, _)| key == ROSEUI_RUNTIME_TOKEN_ENV)
                    {
