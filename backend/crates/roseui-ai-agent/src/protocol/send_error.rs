        ...
        match err {
            AgentError::WorkspacePathRuntimeUnavailable(path) => Self {
                stream_error: AgentStreamErrorData {
                    ...
                    ownership: Some(AgentErrorOwnership::Roseui),
            ...
            AgentError::Internal(_) => Self::new(
                ...
                AgentErrorCode::RoseuiInternalError,
                AgentErrorOwnership::Roseui,
                Some(detail),
                true,
                true,
                resolution(
            ...
            AgentError::Forbidden(_) => Self::new(
                ...
                AgentErrorCode::RoseuiPermissionError,
                AgentErrorOwnership::Roseui,
                Some(detail),
                false,
                true,
            ...
            let classified = acp_agent_internal_texts(message, data.as_ref())
                ...
                .find_map(|text| {
                    ...
                    classify_agent_lifecycle(&lower)
                        ...
                        .or_else(|| classify_roseui_state(&lower))
...
fn classify_roseui_state(lower: &str) -> Option<ClassifiedError> {
    if lower.contains("conversation is already processing") {
        return Some(ClassifiedError {
            message: "The current response is still running",
            code: AgentErrorCode::RoseuiConversationBusy,
            ownership: AgentErrorOwnership::Roseui,
            retryable: true,
            feedback_recommended: false,
...
mod tests {
    use super::*;
    use roseui_api_types::{AgentErrorResolutionKind, AgentErrorResolutionTarget};
    use serde_json::json;
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_roseui_error() {
        let err =
        ...
        assert_eq!(err.ownership(), Some(AgentErrorOwnership::Roseui));
        assert_eq!(
    ...
    fn classifies_agent_setup_failures() {
        ...
        for managed_binary_missing in [
            "expected managed Claude ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\RoseUi\\roseui\\runtime\\acp\\claude-agent-acp\\0.58.1\\win32-x64\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe",
            "expected managed Codex ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\RoseUi\\roseui\\runtime\\acp\\codex-acp\\1.1.2\\win32-x64\\node_modules\\@openai\\codex-win32-x64\\vendor\\x86_64-pc-windows-msvc\\bin\\codex.exe",
        ] {
            assert_classification(
    ...
    #[test]
    fn classifies_roseui_conversation_busy_after_agent_and_provider_checks() {
        assert_classification(
            "Conflict: Conversation is already processing a message",
            AgentErrorCode::RoseuiConversationBusy,
            AgentErrorOwnership::Roseui,
