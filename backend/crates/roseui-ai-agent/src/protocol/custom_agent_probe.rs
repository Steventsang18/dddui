...
use roseui_api_types::TryConnectCustomAgentResponse;
use roseui_common::{CommandSpec, EnvVar};
use roseui_runtime::{NodeRuntimeProgressReporter, ResolvedCommand, ensure_runtime_command_with_reporter};
...
mod tests {
    ...
    async fn probe_returns_fail_cli_when_command_missing() {
        let resp = try_connect_custom_agent("roseui-definitely-does-not-exist-xyz", &[], &HashMap::new(), None).await;
        match resp {
