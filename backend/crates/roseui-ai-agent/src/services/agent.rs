...
//! `roseui-conversation::ConversationService`, which dispatches through
...
use roseui_api_types::{AgentLogoEntry, AgentManagementRow, ProviderHealthCheckRequest, ProviderHealthCheckResponse};
use roseui_db::IProviderRepository;
...
impl AgentService {
    ...
    pub async fn set_agent_overrides(
        ...
        user_id: &str,
        id: &str,
        req: roseui_api_types::SetAgentOverridesRequest,
        ...
        if (command_override.is_some() || has_env_override) && is_internal_aion_cli_row(&row) {
            return Err(AgentError::bad_request("Internal Rose CLI does not support overrides"));
        }
    ...
    pub async fn get_agent_overrides(
        ...
        user_id: &str,
        id: &str,
    ) -> Result<roseui_api_types::AgentOverridesResponse, AgentError> {
        let row = self
            .registry
            .repo_handle()
            .get_for_user(user_id, id)
        ...
        let env_override = row
            .env_override
            .as_deref()
            .and_then(|s| serde_json::from_str::<Vec<roseui_api_types::AgentEnvEntry>>(s).ok())
            .unwrap_or_default();

        Ok(roseui_api_types::AgentOverridesResponse {
...
fn is_bridge_launched_row(row: &roseui_db::AgentMetadataRow) -> bool {
    let Some(raw) = row.agent_source_info.as_deref() else {
        return false;
    };
    let Ok(info) = serde_json::from_str::<roseui_api_types::AgentSourceInfo>(raw) else {
        return false;
    };
...
fn is_internal_aion_cli_row(row: &roseui_db::AgentMetadataRow) -> bool {
