...
use roseui_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use roseui_db::models::RemoteAgentRow;
use roseui_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
...
impl RemoteAgentService {
    ...
    pub async fn create(
        ...
        let row = self
            .repo
            .create(roseui_db::CreateRemoteAgentParams {
                user_id,
                name: &req.name,
    ...
    pub async fn update(
        ...
        let row = self.repo.update(user_id, id, params).await.map_err(|e| match e {
            roseui_db::DbError::NotFound(msg) => AgentError::not_found(msg),
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    let device_id = roseui_common::generate_prefixed_id("dev");
...
fn db_err(e: roseui_db::DbError) -> AgentError {
