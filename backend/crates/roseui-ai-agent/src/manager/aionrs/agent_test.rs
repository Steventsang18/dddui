...
#[test]
fn resolve_roseui_config_discards_standalone_max_token_settings() {
...
"#,
    ...
    let embedded = resolve_roseui_config(&cli_args).unwrap();
    assert_eq!(embedded.max_tokens, None);
    assert_eq!(embedded.compat.default_max_tokens_for_model("gpt-test"), None);
}

#[test]
fn resolve_roseui_config_keeps_builtin_provider_max_token_policy() {
    let project = tempfile::tempdir().unwrap();
    fs::write(
        project.path().join(".aionrs.toml"),
        r#"
...
fn aionrs_final_input_dump_value_contains_raw_split_input_and_context() {
    ...
    let context = AionrsFinalInputDumpContext {
        ...
        skills: vec!["roseui-config".to_owned()],
        mcp_servers,
        runtime_env: vec![("ROSEUI_RAW".to_owned(), "raw-env-value".to_owned())],
    };
    ...
    assert_eq!(value["resolved_context"]["skills"][0], "roseui-config");
    assert_eq!(
