...
pub(crate) fn normalize_requested_mode(metadata: &AgentMetadata, mode: &str) -> String {
    ...
    // RoseUi persists the legacy aliases `yolo` / `yoloNoSandbox` while
...
mod tests {
    use super::*;
    use roseui_api_types::AgentHandshake;
    use roseui_common::AgentType;

    fn metadata_with_yolo_id(yolo_id: Option<&str>) -> AgentMetadata {
        use roseui_api_types::{AgentSource, AgentSourceInfo, BehaviorPolicy};
        AgentMetadata {
            id: "test".into(),
            icon: None,
            name: "Test".into(),
    ...
    fn uses_meta_resume_true_when_policy_flag_set() {
        use roseui_api_types::BehaviorPolicy;
