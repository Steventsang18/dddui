...
impl AgentRegistry {
    ...
    pub async fn run_slow_probe_recheck(&self) {
        ...
        for id in pending {
            ...
            let params = roseui_db::UpdateAgentAvailabilitySnapshotParams {
...
fn decode_row(
    ...
    let team_capable = behavior_policy.supports_team
        || roseui_common::constants::is_team_capable(backend_str, handshake.agent_capabilities.as_ref());
    ...
    if is_internal_aion_cli && command_override.is_some() {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring command override for internal Rose CLI agent"
        );
    }
    ...
    if is_internal_aion_cli && env_override.as_ref().is_some_and(|entries| !entries.is_empty()) {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring environment overrides for internal Rose CLI agent"
        );
    }
...
/// unavailable rows go to `info!` so the default dodiddoneui.log surfaces
...
/// agent's runtime environment or RoseUi-internal wiring. Case-insensitive.
pub(crate) fn is_blocked_override_env_key(key: &str) -> bool {
    let upper = key.to_ascii_uppercase();
    if upper.starts_with("ROSEUI_") {
        return true;
    }
    matches!(
        upper.as_str(),
...
mod tests {
    use super::*;
    use roseui_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database_memory};
    ...
    async fn codex_yolo_id_maps_to_agent_full_access() {
        ...
        // Legacy RoseUi yolo aliases resolve to Codex's native
    ...
    async fn pi_builtin_uses_stable_acp_adapter_and_requires_pi_cli() {
        ...
        // gives it the `$ROSEUI_HELPER_BIN team ...` command surface instead of
    ...
    /// (Rose CLI is `agent_source = internal` with no `command`).
    ...
    async fn list_all_filters_out_unavailable_rows() {
        ...
        // Rose CLI (internal, no spawn command) is always available.
        assert!(
    ...
    fn decode_row_applies_command_override() {
        use roseui_db::AgentMetadataRow;
