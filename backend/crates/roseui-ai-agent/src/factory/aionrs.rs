...
use roseui_api_types::{
    ...
    SessionMcpTransport, TEAM_MCP_SERVER_NAME, TeamMcpStdioConfig, WIKI_MCP_SERVER_NAME, WikiMcpStdioConfig,
};
use roseui_common::ProviderWithModel;
use roseui_db::IMcpServerRepository;
use roseui_db::models::McpServerRow;
use roseui_realtime::EventBroadcaster;
use roseui_runtime::ensure_runtime_command_with_reporter;
...
use crate::manager::aionrs::sanitize_session_messages;
use crate::manager::rupoo::RupooAgentManager;
...
pub(super) async fn build(
    ...
    let api_key = roseui_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)
...
pub(crate) fn resolve_bedrock_config(json: Option<&str>) -> Option<aion_config::config::BedrockConfig> {
    let bc: roseui_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
...
fn resolve_mcp_servers(overrides: &AionrsBuildExtra) -> HashMap<String, McpServerConfig> {
    ...
    if let Some(cfg) = overrides.wiki_mcp_stdio_config.as_ref() {
        map.extend(wiki_mcp_to_config(cfg));
    }
    if let Some(cfg) = overrides.team_mcp_stdio_config.as_ref() {
        map.extend(team_mcp_to_config(cfg));
    }
    map
}

fn wiki_mcp_to_config(cfg: &WikiMcpStdioConfig) -> HashMap<String, McpServerConfig> {
    let mut env = HashMap::new();
    env.insert("ROSEUI_WIKI_DB_PATH".into(), cfg.db_path.clone());

    let server = McpServerConfig {
        transport: TransportType::Stdio,
        command: Some(cfg.binary_path.clone()),
        args: Some(vec!["mcp-wiki-stdio".into()]),
        env: Some(env),
        url: None,
        headers: None,
        deferred: Some(false),
    ...
    HashMap::from([(WIKI_MCP_SERVER_NAME.to_owned(), server)])
}
...
mod tests {
    ...
    impl IMcpServerRepository for MockMcpRepo {
        ...
        async fn find_by_id(&self, user_id: &str, id: &str) -> Result<Option<McpServerRow>, roseui_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        async fn find_by_name(&self, user_id: &str, name: &str) -> Result<Option<McpServerRow>, roseui_db::DbError> {
            Ok(self
                .rows
                .iter()
