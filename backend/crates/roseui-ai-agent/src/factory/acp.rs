...
use roseui_api_types::{AgentMetadata, SessionMcpServer, SessionMcpTransport};
use roseui_common::CommandSpec;
use roseui_db::IMcpServerRepository;
use roseui_db::models::McpServerRow;
use roseui_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
use roseui_runtime::{ensure_runtime_command, ensure_runtime_command_with_reporter};
        ...
        && matches!(backend_label, "claude" | "codex")
    {
        let instance = crate::session_agent::build_session_instance(
            ...
            crate::session_agent::SessionBuildInputs {
                ...
                // The ROSEUI_* conversation runtime context the legacy path
...
async fn resolve_catalog_metadata(
    ...
    config: &roseui_api_types::AcpBuildExtra,
    user_id: &str,
...
async fn resolve_agent_command_spec(
    meta: &roseui_api_types::AgentMetadata,
    user_id: &str,
    workspace: &str,
    conversation_id: &str,
    ...
    let mut args: Vec<String> = resolved
        ...
        .collect();
    let launch_args = if meta.agent_source == roseui_api_types::AgentSource::Builtin
        && meta.agent_source_info.bridge_binary.as_deref() == Some("npx")
        && let Some(backend) = meta.backend.as_deref()
    {
        roseui_runtime::pin_registry_npx_args(backend, &meta.args)
    ...
    let mut env: Vec<roseui_common::EnvVar> = meta
        .env
        .iter()
        ...
        .collect();
    env.extend(resolved.env.iter().map(|(name, value)| roseui_common::EnvVar {
...
mod tests {
    use super::*;
    use roseui_api_types::AcpBuildExtra;
    use roseui_db::{
    ...
    use roseui_runtime::{ManagedResourcesMode, init as init_runtime, set_managed_resources_mode};
    ...
    impl BundledRuntimeModeGuard {
        fn install(root: &Path) -> Self {
            unsafe { std::env::set_var("ROSEUI_BUNDLED_MANAGED_RESOURCES", root) };
    ...
    impl Drop for BundledRuntimeModeGuard {
        fn drop(&mut self) {
            unsafe { std::env::remove_var("ROSEUI_BUNDLED_MANAGED_RESOURCES") };
    ...
    async fn resolve_agent_command_spec_flattens_bare_npx_command() {
        ...
        let mut meta = roseui_api_types::AgentMetadata {
            id: "agent-1".into(),
            icon: None,
            name: "Test ACP".into(),
            name_i18n: None,
            ...
            agent_source: roseui_api_types::AgentSource::Custom,
            ...
            env: vec![roseui_api_types::AgentEnvEntry {
                name: "K".into(),
                value: "V".into(),
                description: None,
            }],
            native_skills_dirs: None,
            behavior_policy: roseui_api_types::BehaviorPolicy::default(),
            yolo_id: None,
            sort_order: 0,
            team_capable: false,
            ...
            handshake: roseui_api_types::AgentHandshake::default(),
        ...
        meta.agent_source = roseui_api_types::AgentSource::Builtin;
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self, user_id: &str) -> Result<Vec<McpServerRow>, roseui_db::DbError> {
            if self.fail {
                Err(roseui_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.iter().filter(|row| row.user_id == user_id).cloned().collect())
            }
        }
        async fn find_by_id(&self, _user_id: &str, _id: &str) -> Result<Option<McpServerRow>, roseui_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(&self, _user_id: &str, _name: &str) -> Result<Option<McpServerRow>, roseui_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(
            ...
            user_id: &str,
            ids: &[String],
        ) -> Result<Vec<McpServerRow>, roseui_db::DbError> {
            if self.fail {
                return Err(roseui_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
                .filter_map(|id| {
        ...
        async fn create(
            &self,
            _params: roseui_db::CreateMcpServerParams<'_>,
        ) -> Result<McpServerRow, roseui_db::DbError> {
            unimplemented!()
        }
        async fn update(
            &self,
            _user_id: &str,
            _id: &str,
            _params: roseui_db::UpdateMcpServerParams<'_>,
        ) -> Result<McpServerRow, roseui_db::DbError> {
            unimplemented!()
        }
        ...
        async fn batch_upsert(
            &self,
            _user_id: &str,
            _servers: &[roseui_db::CreateMcpServerParams<'_>],
        ) -> Result<Vec<McpServerRow>, roseui_db::DbError> {
            unimplemented!()
        }
        async fn update_status(
            ...
            _id: &str,
            _status: &str,
            _last_connected: Option<roseui_common::TimestampMs>,
