...
use roseui_api_types::{AcpBuildExtra, AgentMetadata};
...
fn append_runtime_env(command_spec: &mut CommandSpec, runtime_env: &[(String, String)]) {
    for (name, value) in runtime_env {
        command_spec.env.push(roseui_common::EnvVar {
            name: name.clone(),
            value: value.clone(),
        });
    }
    ...
    fn apply_acp_launch_policy_adds_runtime_env_and_codex_full_access_config() {
        ...
        apply_acp_launch_policy(
            ...
            AcpLaunchPolicyInput {
                ...
                runtime_env: &[("ROSEUI_CONVERSATION_ID".into(), "conv-1".into())],
            },
        );
        ...
        assert!(
            command_spec
                .env
                .iter()
                .any(|entry| entry.name == "ROSEUI_CONVERSATION_ID" && entry.value == "conv-1")
        );
    }
