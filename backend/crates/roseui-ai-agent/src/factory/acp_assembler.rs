...
///
/// Layout: `[wiki?, team?, ...user_mcp_servers]`. Built-in MCP servers are
/// prepended; the user's own enabled MCP servers are always appended on top
/// so a session still gets the operator's tools.
fn resolve_mcp_servers(config: &AcpBuildExtra, user_mcp_servers: Vec<McpServer>) -> Vec<McpServer> {
    ...
    if let Some(cfg) = config.wiki_mcp_stdio_config.as_ref() {
        servers.push(wiki_mcp_server(cfg));
    }
