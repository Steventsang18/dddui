...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<roseui_api_types::AgentModeResponse, AgentError> {
...
pub enum AgentInstance {
    ...
    /// In-process roseui agent engine (built-in Rupoo engine, roseui-branded).
    /// Default kernel for all conversations; mirrors `Aionrs` routing.
    Rupoo(Arc<crate::manager::rupoo::RupooAgentManager>),
    /// clean-slate direct-CLI session model (claude/codex only). Wraps an
    /// `roseui_session::SessionBackend` via [`SessionAgentTask`]. Every other
...
impl AgentInstance {
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::Rupoo(m) => m.kill_and_wait(reason),
    ...
    pub fn get_confirmations(&self) -> Vec<roseui_common::Confirmation> {
        match self {
            ...
            Self::Rupoo(m) => m.get_confirmations(),
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::Rupoo(m) => m.confirm(msg_id, call_id, data, always_allow),
    ...
    pub fn check_approval(&self, action: &str, command_type: Option<&str>) -> bool {
        match self {
            ...
            Self::Rupoo(m) => m.check_approval(action, command_type),
    ...
    pub fn get_session_key(&self) -> Option<String> {
        match self {
            Self::Acp(_) | Self::Aionrs(_) | Self::Rupoo(_) | Self::Session(_) => None,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_mode(&self) -> Result<roseui_api_types::AgentModeResponse, AgentError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::Aionrs(m) => m.mode().await,
            Self::Rupoo(m) => m.mode().await,
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AgentError> {
        match self {
            ...
            Self::Rupoo(_) => Ok(GetModelInfoResponse { model_info: None }),
    ...
    pub async fn get_config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        match self {
            ...
            Self::Rupoo(m) => m.config_options().await,
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Rupoo(m) => m.set_config_option(option_id, value).await,
    ...
    /// [`roseui_common::normalize_keys_to_snake_case`] so keys land as
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AgentError> {
        match self {
            ...
            Self::Rupoo(m) => m.get_slash_commands().await,
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Rupoo(_) => Ok(SideQuestionResponse {
                status: "unsupported".into(),
                answer: None,
            }),
...
mod aionrs_config_option_tests {
    ...
    use roseui_api_types::ConfigOptionConfirmation;
