    ...
    pub fn apply_conversation_runtime_context(
        ...
        self.context.runtime_env.retain(|(key, _)| {
            !matches!(
                key.as_str(),
                ROSEUI_USER_ID_ENV
                    | ROSEUI_CONVERSATION_ID_ENV
                    | ROSEUI_HELPER_BIN_ENV
                    | ROSEUI_BASE_URL_ENV
                    | ROSEUI_RUNTIME_TOKEN_ENV
            )
        });
        self.context
            .runtime_env
            .push((ROSEUI_USER_ID_ENV.to_owned(), user_id.to_owned()));
        self.context
            .runtime_env
            .push((ROSEUI_CONVERSATION_ID_ENV.to_owned(), conversation_id.to_owned()));
        if let Some(helper_bin) = helper_bin {
            self.context
                .runtime_env
                .push((ROSEUI_HELPER_BIN_ENV.to_owned(), helper_bin.to_owned()));
        }
        if let Some(base_url) = base_url {
            self.context
                .runtime_env
                .push((ROSEUI_BASE_URL_ENV.to_owned(), base_url.to_owned()));
        }
        if let Some(runtime_token) = runtime_token {
            self.context
                .runtime_env
                .push((ROSEUI_RUNTIME_TOKEN_ENV.to_owned(), runtime_token.to_owned()));
        }
        self.runtime_capabilities.conversation_runtime_context_version = Some(CONVERSATION_RUNTIME_CONTEXT_VERSION);
    }
}

pub const ROSEUI_USER_ID_ENV: &str = "ROSEUI_USER_ID";
pub const ROSEUI_CONVERSATION_ID_ENV: &str = "ROSEUI_CONVERSATION_ID";
pub const ROSEUI_HELPER_BIN_ENV: &str = "ROSEUI_HELPER_BIN";
pub const ROSEUI_BASE_URL_ENV: &str = "ROSEUI_BASE_URL";
pub const ROSEUI_RUNTIME_TOKEN_ENV: &str = "ROSEUI_RUNTIME_TOKEN";
...
pub struct AionrsResolvedConfig {
    ...
    /// Normal RoseUi conversations leave this unset.
...
mod tests {
    use super::*;
    use roseui_api_types::{AcpBuildExtra, AcpModelInfo, AionrsBuildExtra, SlashCommandItem};
    use serde_json::json;
    ...
    fn build_task_options_applies_conversation_runtime_context_once() {
        ...
        use roseui_common::{AgentType, ProviderWithModel};
        ...
        let context = AgentSessionContext {
            ...
            skills: vec![],
            runtime_env: vec![
                (ROSEUI_USER_ID_ENV.into(), "old-user".into()),
                (ROSEUI_CONVERSATION_ID_ENV.into(), "old-conv".into()),
                (ROSEUI_RUNTIME_TOKEN_ENV.into(), "old-token".into()),
        ...
        options.apply_conversation_runtime_context(
            "user-1",
            "conv-1",
            Some("/Applications/RoseUi/dodiddoneui"),
        ...
        assert_eq!(
            options
                ...
                .runtime_env
                .iter()
                .filter(|(key, _)| key == ROSEUI_USER_ID_ENV)
                .count(),
            1
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(ROSEUI_USER_ID_ENV.to_owned(), "user-1".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(ROSEUI_CONVERSATION_ID_ENV.to_owned(), "conv-1".to_owned()))
        );
        ...
        assert!(
            options
                .context
                .runtime_env
                .contains(&(ROSEUI_BASE_URL_ENV.to_owned(), "http://127.0.0.1:25808".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(ROSEUI_RUNTIME_TOKEN_ENV.to_owned(), "runtime-token-1".to_owned()))
        );
        assert_eq!(
            options
                .context
                .runtime_env
                .iter()
                .filter(|(key, _)| key == ROSEUI_RUNTIME_TOKEN_ENV)
                .count(),
            1
        );
