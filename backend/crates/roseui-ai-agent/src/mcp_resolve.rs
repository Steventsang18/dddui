...
//! module emits the NEUTRAL `roseui_api_types::SessionMcpServer` so the app
//! boundary (`roseui-app`) can convert it once into the crate-local
//! `roseui_session::McpServerSpec`, and each backend serializes that into its own
...
use roseui_runtime::ensure_runtime_command;
