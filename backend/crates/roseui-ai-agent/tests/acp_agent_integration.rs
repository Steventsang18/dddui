...
use roseui_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use roseui_ai_agent::manager::acp::AcpAgentManager;
use roseui_ai_agent::registry::AgentRegistry;
use roseui_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use roseui_common::ConversationStatus;
use roseui_db::{SqliteAgentMetadataRepository, init_database_memory};
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = roseui_ai_agent::AcpBuildExtra {
        agent_id: None,
    ...
    let skill_paths = std::sync::Arc::new(roseui_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = roseui_ai_agent::AcpSkillManager::new(skill_paths);
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: roseui_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), roseui_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(roseui_common::AgentKillReason::IdleTimeout)).unwrap();
