...
use roseui_ai_agent::{AgentRegistry, AgentService};
use roseui_api_types::{AgentManagementStatus, AgentSnapshotCheckKind, AgentSnapshotCheckStatus};
use roseui_db::{
...
async fn management_rows_derive_missing_available_and_unavailable_statuses() {
    ...
    repo.upsert(&custom_params(
        "agent-missing",
        "Missing Agent",
        "roseui-missing-agent-binary",
        r#"{"binary_name":"roseui-missing-agent-binary"}"#,
    ))
    .await
    .unwrap();
    repo.upsert(&custom_params(
...
async fn manual_health_check_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        ...
        "roseui-definitely-missing-health-check-target",
        r#"{"binary_name":"roseui-definitely-missing-health-check-target"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_enabled_toggle_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-toggle",
        "Target Toggle Agent",
        "roseui-target-toggle-command",
        r#"{"binary_name":"roseui-target-toggle-command"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_delete_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-delete",
        "Target Delete Agent",
        "roseui-target-delete-command",
        r#"{"binary_name":"roseui-target-delete-command"}"#,
    ))
    .await
    .unwrap();
