...
use roseui_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use roseui_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use roseui_ai_agent::manager::acp::{AcpSession, SessionNewPreludeHook};
use roseui_ai_agent::registry::AgentRegistry;
use roseui_ai_agent::shared_kernel::ModelId;
use roseui_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(roseui_extension::resolve_skill_paths(tmp.path(), tmp.path()));
