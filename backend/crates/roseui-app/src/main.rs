...
fn run_main() -> Result<ExitCode, MainError> {
    ...
    // parse a uniform CLI, but bypass `roseui_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `roseui_runtime::init`
    // before performing managed-runtime and PATH probing.
    let needs_runtime = cli.command.as_ref().is_none_or(Command::need_runtime);
    if needs_runtime {
        roseui_runtime::set_managed_resources_mode(cli.managed_resources_mode.into());
        roseui_runtime::init(&cli.data_dir);
    }
    ...
    let merged_path = unsafe { roseui_runtime::enhance_process_path() };
...
async fn async_main(merged_path: String, cli: Cli) -> Result<ExitCode, MainError> {
    ...
    match cli.command {
        ...
        Some(Command::McpWikiStdio) => Ok(commands::run_wiki_stdio().await),
        ...
        Some(Command::Migrate(ref args)) => Ok(commands::run_migrate(args, &cli).await?),
        None => {
            let mut env = bootstrap::init_environment(&cli, &merged_path)?;

            // Acquire the data-dir process-level guard before binding a port or
            // touching the DB, so a second dodiddoneui yields structurally rather
            ...
            let db_path = env.config.resolve_database_path();
            let _instance_guard = match roseui_db::DataDirInstanceGuard::try_acquire(&db_path) {
                Ok(Some(guard)) => {
                    ...
                    match bootstrap::wait_for_instance_guard(&db_path) {
                        ...
                        Ok(None) => {
                            tracing::info!(
                                ...
                                "another dodiddoneui owns the data directory; yielding"
                            );
                            return Err(MainError::Bootstrap(bootstrap::BootstrapError::new(
                                ...
                                "another dodiddoneui already owns this data directory",
                            )));
                        }
                        Err(error) => {
                            tracing::warn!(
