//! CLI argument definitions for the `dodiddoneui` binary.
//!
...
#[derive(Parser)]
#[command(name = "dodiddoneui", about = "DoDidDoneUi Backend Server", version)]
pub(crate) struct Cli {
    /// Host address to listen on.
    #[arg(long, default_value_t = String::from(roseui_common::constants::DEFAULT_HOST))]
    pub host: String,

    /// Port number to listen on.
    #[arg(long, default_value_t = roseui_common::constants::DEFAULT_PORT)]
    pub port: u16,
    ...
    /// Falls back to ROSEUI_WORK_DIR env, then to data-dir.
    #[arg(long)]
    ...
    /// Identity source mode. Owner (default) skips login for single-operator
    /// local use. WebUi requires username/password. RosePro requires
    /// AIONCORE_BOOTSTRAP_SECRET.
    #[arg(long, value_enum, default_value_t = IdentityModeArg::Owner)]
    ...
    /// Log level filter (e.g. "info", "debug", "info,roseui_mcp=trace").
    #[arg(long)]
    ...
    /// Serve frontend assets from this directory instead of the embedded build.
    /// Useful for local development against a live `vite build` output.
    #[arg(long)]
...
impl From<IdentityModeArg> for roseui_app::IdentityMode {
    fn from(value: IdentityModeArg) -> Self {
        match value {
            IdentityModeArg::Local => Self::Local,
            IdentityModeArg::Owner => Self::Owner,
            IdentityModeArg::Webui => Self::WebUi,
            IdentityModeArg::Aionpro => Self::RosePro,
        }
    }
}

impl From<ManagedResourcesModeArg> for roseui_runtime::ManagedResourcesMode {
...
pub(crate) enum Command {
    ...
    Capabilities,
    /// Agent-facing automation CLI for DoDidDoneUi configuration.
    Config(ConfigArgs),
    /// Agent-facing read-only troubleshooting CLI for DoDidDoneUi diagnosis.
    ...
    McpTeamStdio,
    /// MCP stdio server for wiki tools (spawned by the ACP agent CLI).
    McpWikiStdio,
    ...
    /// Migrate an Electron-era roseui.db to the DoDidDoneUi schema.
    Migrate(MigrateArgs),
}
...
impl Command {
    pub(crate) fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::McpWikiStdio => "mcp-wiki-stdio",
            Self::Doctor => "doctor",
...
pub(crate) struct MigrateArgs {
    /// Path to the Electron-era roseui.db file.
    #[arg(long)]
    pub from_electron: PathBuf,

    /// Validate the migration without modifying any files.
    #[arg(long)]
    pub dry_run: bool,

    /// Directory to store a timestamped backup of the source database.
    /// Defaults to the directory containing the source database.
    #[arg(long)]
...
mod tests {
    ...
    fn long_version_flag_uses_workspace_package_version() {
        let result = Cli::try_parse_from(["dodiddoneui", "--version"]);
    ...
    fn short_version_flag_uses_workspace_package_version() {
        let result = Cli::try_parse_from(["dodiddoneui", "-V"]);
        let err = match result {
    ...
    fn prepare_managed_resources_accepts_bundle_out() {
        let cli = Cli::parse_from([
            ...
            "--bundle-out",
            "/tmp/dodiddoneui-bundle",
        ]);

        match cli.command {
            Some(Command::PrepareManagedResources(args)) => {
                assert_eq!(args.bundle_out, std::path::Path::new("/tmp/dodiddoneui-bundle"));
            }
    ...
    fn managed_resources_mode_defaults_to_download() {
        let cli = Cli::parse_from(["dodiddoneui"]);
        assert_eq!(cli.managed_resources_mode, ManagedResourcesModeArg::Download);
    }

    #[test]
    fn managed_resources_mode_accepts_download() {
        let cli = Cli::parse_from(["dodiddoneui", "--managed-resources-mode", "download"]);
        assert_eq!(cli.managed_resources_mode, ManagedResourcesModeArg::Download);
    }

    #[test]
    fn parent_pid_accepts_positive_integer() {
        let cli = Cli::parse_from(["dodiddoneui", "--parent-pid", "4242"]);
    ...
    fn dump_prompts_accepts_flag() {
        let cli = Cli::parse_from(["dodiddoneui", "--dump-prompts"]);
        assert!(cli.dump_prompts);
    }
    ...
    fn recover_corrupted_database_flag_is_accepted() {
        let cli = Cli::parse_from(["dodiddoneui", "--recover-corrupted-database"]);
        assert!(cli.recover_corrupted_database);
    }
    ...
    fn command_as_str_returns_clap_subcommand_names() {
        let prepare_args = PrepareManagedResourcesArgs {
            bundle_out: PathBuf::from("/tmp/dodiddoneui-bundle"),
        };
        let migrate_args = MigrateArgs {
            from_electron: PathBuf::from("/tmp/roseui.db"),
            dry_run: false,
            backup: None,
        };
        ...
        let cases = [
            ...
            (Command::McpWikiStdio, "mcp-wiki-stdio"),
            (Command::Doctor, "doctor"),
            (
                Command::PrepareManagedResources(prepare_args),
                "prepare-managed-resources",
            ),
            (Command::Migrate(migrate_args), "migrate"),
        ];
    ...
    fn config_cli_accepts_agent_facing_design_command_paths() {
        let commands: &[&[&str]] = &[
            &["dodiddoneui", "config", "capabilities"],
            &["dodiddoneui", "config", "context"],
            &["dodiddoneui", "config", "conversation", "rename"],
            &["dodiddoneui", "config", "assistants", "list"],
            &["dodiddoneui", "config", "assistants", "get"],
            &["dodiddoneui", "config", "assistants", "create"],
            &["dodiddoneui", "config", "assistants", "update"],
            &["dodiddoneui", "config", "assistants", "delete"],
            &["dodiddoneui", "config", "assistants", "import"],
            &["dodiddoneui", "config", "assistants", "state"],
            &["dodiddoneui", "config", "assistants", "rule", "read"],
            &["dodiddoneui", "config", "assistants", "rule", "write"],
            &["dodiddoneui", "config", "assistants", "rule", "delete"],
            &["dodiddoneui", "config", "assistants", "skill", "read"],
            &["dodiddoneui", "config", "assistants", "skill", "write"],
            &["dodiddoneui", "config", "assistants", "skill", "delete"],
            &["dodiddoneui", "config", "skills", "list"],
            &["dodiddoneui", "config", "skills", "info"],
            &["dodiddoneui", "config", "skills", "paths"],
            &["dodiddoneui", "config", "skills", "import"],
            &["dodiddoneui", "config", "skills", "delete"],
            &["dodiddoneui", "config", "skills", "scan"],
            &["dodiddoneui", "config", "mcp", "servers", "list"],
            &["dodiddoneui", "config", "mcp", "servers", "get"],
            &["dodiddoneui", "config", "mcp", "servers", "create"],
            &["dodiddoneui", "config", "mcp", "servers", "update"],
            &["dodiddoneui", "config", "mcp", "servers", "delete"],
            &["dodiddoneui", "config", "mcp", "servers", "toggle"],
            &["dodiddoneui", "config", "mcp", "servers", "import"],
            &["dodiddoneui", "config", "mcp", "test-connection"],
            &["dodiddoneui", "config", "mcp", "agent-configs"],
            &["dodiddoneui", "config", "mcp", "oauth", "check-status"],
            &["dodiddoneui", "config", "mcp", "oauth", "login"],
            &["dodiddoneui", "config", "mcp", "oauth", "logout"],
            &["dodiddoneui", "config", "mcp", "oauth", "authenticated"],
            &["dodiddoneui", "config", "providers", "list"],
            &["dodiddoneui", "config", "providers", "create"],
            &["dodiddoneui", "config", "providers", "update"],
            &["dodiddoneui", "config", "providers", "delete"],
            &["dodiddoneui", "config", "providers", "detect-protocol"],
            &["dodiddoneui", "config", "providers", "fetch-models"],
            &["dodiddoneui", "config", "providers", "models", "fetch"],
            &["dodiddoneui", "config", "providers", "health-check"],
            &["dodiddoneui", "config", "settings", "get"],
            &["dodiddoneui", "config", "settings", "patch"],
            &["dodiddoneui", "config", "settings", "client", "get"],
            &["dodiddoneui", "config", "settings", "client", "put"],
            &["dodiddoneui", "config", "agents", "list"],
            &["dodiddoneui", "config", "agents", "enable"],
            &["dodiddoneui", "config", "agents", "overrides", "get"],
            &["dodiddoneui", "config", "agents", "overrides", "set"],
            &["dodiddoneui", "config", "agents", "custom", "create"],
            &["dodiddoneui", "config", "agents", "custom", "update"],
            &["dodiddoneui", "config", "agents", "custom", "delete"],
            &["dodiddoneui", "config", "agents", "custom", "try-connect"],
            &["dodiddoneui", "config", "cron", "jobs", "list"],
            &["dodiddoneui", "config", "cron", "jobs", "get"],
            &["dodiddoneui", "config", "cron", "jobs", "create"],
            &["dodiddoneui", "config", "cron", "jobs", "update"],
            &["dodiddoneui", "config", "cron", "jobs", "delete"],
            &["dodiddoneui", "config", "cron", "jobs", "run"],
            &["dodiddoneui", "config", "cron", "jobs", "skill", "get"],
            &["dodiddoneui", "config", "cron", "jobs", "skill", "save"],
            &["dodiddoneui", "config", "cron", "jobs", "skill", "delete"],
            &["dodiddoneui", "config", "skills", "external-paths", "list"],
            &["dodiddoneui", "config", "skills", "external-paths", "add"],
            &["dodiddoneui", "config", "skills", "external-paths", "remove"],
            &["dodiddoneui", "config", "skills", "market", "enable"],
            &["dodiddoneui", "config", "skills", "market", "disable"],
        ];
    ...
    fn team_cli_accepts_agent_facing_command_paths() {
        let commands: &[&[&str]] = &[
            &["dodiddoneui", "team", "capabilities"],
            &["dodiddoneui", "team", "help"],
            &["dodiddoneui", "team", "context"],
            &["dodiddoneui", "team", "members"],
            &["dodiddoneui", "team", "send-message"],
            &["dodiddoneui", "team", "task", "create"],
            &["dodiddoneui", "team", "task", "update"],
            &["dodiddoneui", "team", "task", "list"],
            &["dodiddoneui", "team", "list-assistants"],
            &["dodiddoneui", "team", "describe-assistant"],
            &["dodiddoneui", "team", "spawn-agent"],
            &["dodiddoneui", "team", "rename-agent"],
            &["dodiddoneui", "team", "shutdown-agent"],
        ];
    ...
    fn prepare_managed_resources_requires_bundle_out() {
        let err = match Cli::try_parse_from(["dodiddoneui", "prepare-managed-resources"]) {
