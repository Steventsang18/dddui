use roseui_api_types::{TEAM_TOOLS_SCHEMA_VERSION, team_tool_descriptors};
...
pub(crate) fn data() -> Value {
    ...
    json!({
        ...
        "commands": {
            ...
            "context": { "runtime_env_required": ["ROSEUI_BASE_URL", "ROSEUI_USER_ID", "ROSEUI_CONVERSATION_ID", "ROSEUI_RUNTIME_TOKEN"] },
            "tool_call": { "runtime_env_required": ["ROSEUI_BASE_URL", "ROSEUI_USER_ID", "ROSEUI_CONVERSATION_ID", "ROSEUI_RUNTIME_TOKEN"] }
        },
        "output_envelope": {
            "success": "boolean",
...
pub(crate) fn help_markdown() -> String {
    let mut text = String::from("# DoDidDoneUi Team CLI\n\nUse `dodiddoneui team capabilities` for exact schemas.\n\n");
    for tool in team_tool_descriptors() {
        ...
        text.push_str(&format!(
            "- `dodiddoneui team {command}` -> `{}` ({permission}): {}\n",
