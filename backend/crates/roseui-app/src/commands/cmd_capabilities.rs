//! Top-level agent-readable capability index for the `dodiddoneui` binary.
...
fn data() -> Value {
    json!({
        "schema_version": 1,
        "contract": "agent-facing-dodiddoneui-cli",
        "stability": "stable",
        "entrypoint": "dodiddoneui capabilities",
        "purpose": "Top-level index for agent-facing DoDidDoneUi CLI domains.",
        "output": {
        ...
        "runtime_context": {
            "primary": "ROSEUI_CONVERSATION_ID",
            "environment": RUNTIME_ENV,
            "selectors": {
                "conversation_id": {
                    "current": "resolve from ROSEUI_CONVERSATION_ID"
                },
                "assistant_id": {
                    "current": "resolve via current conversation"
                },
                "user_id": {
                    "current": "resolve from ROSEUI_USER_ID"
                }
            }
        },
        "input": {
        ...
        "domains": [
            {
                "name": "config",
                "mode": "read-write",
                "description": "Manage RoseUi configuration: assistants, assistant rules, skills, MCP servers, providers, settings, agents, and scheduled tasks.",
                "contract": "agent-facing-config-cli",
                "contract_command": "config capabilities",
                "invocation": "dodiddoneui config capabilities",
                "runtime_required": ["ROSEUI_BASE_URL", "ROSEUI_CONVERSATION_ID", "ROSEUI_USER_ID"],
                "safety": {
                    "can_write": true,
            ...
            },
            {
                "name": "diagnose",
                "mode": "read-only",
                "description": "Diagnose a running RoseUi installation: backend health, conversations, provider health, MCP, cron, teams, logs, and controlled GET reads.",
                "contract": "agent-facing-diagnose-cli",
                "contract_command": "diagnose capabilities",
                "invocation": "dodiddoneui diagnose capabilities",
                "runtime_required": ["ROSEUI_BASE_URL", "ROSEUI_CONVERSATION_ID", "ROSEUI_USER_ID"],
                "optional_runtime": ["ROSEUI_LOG_DIR"],
                "safety": {
                    "can_write": false,
                    "read_only": true,
                    "redacted_by_default": true,
            ...
            },
            {
                ...
                "invocation": "dodiddoneui team capabilities",
                "runtime_required": ["ROSEUI_BASE_URL", "ROSEUI_CONVERSATION_ID", "ROSEUI_USER_ID", "ROSEUI_RUNTIME_TOKEN"],
