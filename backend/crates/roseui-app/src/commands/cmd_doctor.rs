//! `dodiddoneui doctor` subcommand: agent CLI detection self-check.
//!
...
//! `roseui_runtime::init` + `enhance_process_path` for `Doctor` as it
//! does for the server, so managed runtimes and CLI commands resolve
//! through the same paths the server uses.
//!
//! Writes to stdout (not the rolling dodiddoneui.log) — the user
...
pub async fn run_doctor(cli: &Cli, merged_path: &str) -> Result<ExitCode, CliBoundaryError> {
    ...
    // Fail-fast guard for the wiki full-text-search feature (migration 034).
    check_fts5(database.pool()).await?;
...
/// Fail-fast guard for the wiki full-text-search feature (migration 034).
///
/// `wiki_fts` is an FTS5 virtual table; if the linked SQLite was built without
...
async fn check_fts5(pool: &SqlitePool) -> Result<(), CliBoundaryError> {
    ...
    if has_fts5 {
        ...
        Err(CliBoundaryError::new(
            CliBoundaryCode::CliDoctorFts5Missing,
            SUBCOMMAND,
            "SQLite was built without FTS5; wiki full-text search (migration 034) will fail",
        ))
    }
}
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    println!("RoseUi backend doctor — agent CLI detection self-check");
...
fn print_snapshot(snapshot: &[(roseui_api_types::AgentMetadata, Option<UnavailableReason>)]) {
...
mod tests {
    ...
    fn doctor_database_error_uses_stable_code_without_raw_path() {
        ...
        assert!(!err.stderr_line().contains("/Users/secret/dodiddoneui-backend.db"));
    }
    ...
    #[tokio::test]
    async fn check_fts5_passes_on_bundled_sqlite() {
        let db = roseui_db::init_database_memory().await.unwrap();
        let result = check_fts5(db.pool()).await;
        assert!(result.is_ok(), "test sqlite must be built with FTS5");
        db.close().await;
    }
}
