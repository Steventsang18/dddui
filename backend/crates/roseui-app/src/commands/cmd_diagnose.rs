//! `dodiddoneui diagnose` subcommand: agent-facing read-only troubleshooting CLI.
...
const ENV_BASE_URL: &str = "ROSEUI_BASE_URL";
const ENV_CONVERSATION_ID: &str = "ROSEUI_CONVERSATION_ID";
const ENV_USER_ID: &str = "ROSEUI_USER_ID";
const ENV_RUNTIME_TOKEN: &str = "ROSEUI_RUNTIME_TOKEN";
const ENV_LOG_DIR: &str = "ROSEUI_LOG_DIR";
    ...
    let log_dir = optional_string_field(&payload, "log_dir")
        ...
        .ok_or_else(|| {
            DiagnoseError::new(
                ...
                command,
                "missing log directory; set ROSEUI_LOG_DIR or pass log_dir in stdin JSON",
            )
...
struct DiagnoseEnv {
    ...
    /// backend rejects tokenless requests in RosePro mode.
...
async fn request_json(
    ...
    let mut request = client
        ...
        .header("x-roseui-conversation-id", &env.conversation_id)
        .header("x-roseui-user-id", &env.user_id);
    if let Some(runtime_token) = &env.runtime_token {
        request = request.header("x-roseui-runtime-token", runtime_token);
    }
    let response = request.send().await.map_err(|_| {
        DiagnoseError::new(
            DiagnoseErrorCode::HttpRequestFailed,
            command,
            "failed to call RoseUi backend",
        )
        .field("path", path)
    })?;
    ...
    let text = response.text().await.map_err(|_| {
        DiagnoseError::new(
            ...
            command,
            "failed to read RoseUi backend response",
        )
        .field("path", path)
    })?;
    ...
    if !status.is_success() {
        return Err(DiagnoseError::new(
            ...
            command,
            "RoseUi backend returned an error status",
        )
        .field("path", path)
    ...
    let value: Value = serde_json::from_str(&text).map_err(|_| {
        DiagnoseError::new(
            ...
            command,
            "RoseUi backend returned invalid JSON",
        )
        .field("path", path)
    })?;
...
fn extract_api_data(value: Value, command: &str) -> Result<Value, DiagnoseError> {
    ...
    Err(DiagnoseError::new(
        DiagnoseErrorCode::HttpStatusError,
        command,
        "RoseUi backend returned an unsuccessful response",
    ))
}
...
fn tail_latest_log(
    ...
    collect_dodiddoneui_logs(log_dir, 0, &mut files).map_err(|_| {
        DiagnoseError::new(
...
fn collect_dodiddoneui_logs(dir: &Path, depth: usize, out: &mut Vec<PathBuf>) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            collect_dodiddoneui_logs(&path, depth + 1, out)?;
        } else if path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name.ends_with(".dodiddoneui.log"))
        {
            out.push(path);
        }
    }
...
mod tests {
    ...
    fn diagnose_error_renders_quoted_stable_contract() {
        ...
        assert_eq!(
            error.stderr_line(),
            "DIAGNOSE_ENV_MISSING command=\"diagnose context\" field=\"ROSEUI_CONVERSATION_ID\": missing required environment variable"
        );
    }
