//! `dodiddoneui` (no subcommand): the main HTTP server.
...
use roseui_api_types::{RuntimeStatusScope, RuntimeStatusScopeKind};
use roseui_app::{AppConfig, AppServices, RouterBuildError, create_router_with_runtime};
use roseui_system::RuntimePrepareService;
use roseui_team::TeamIdleCleanupCoordinator;
...
pub(crate) async fn run_server(
    ...
    // 首启自动打开浏览器（本地单二进制体验），除非 --no-open。
    if !services.no_open {
        let open_url = services.local_base_url.clone();
        if let Err(error) = open::that(open_url.clone()) {
            warn!(url = %open_url, error = %error, "startup: failed to open browser automatically");
        } else {
            info!(url = %open_url, "startup: opened default browser");
        }
    }
    ...
    // 10 min / 30 min / 60 s and are overridable via ROSEUI_IDLE_TIMEOUT_SECS,
    // ROSEUI_TEAM_IDLE_TIMEOUT_SECS, and ROSEUI_IDLE_SCAN_INTERVAL_SECS. The
    ...
    let idle_cleanup_coordinator: Arc<dyn roseui_ai_agent::IdleCleanupCoordinator> =
        Arc::new(TeamIdleCleanupCoordinator::new(
            router_runtime.team_service.clone(),
            services.active_lease_registry.clone(),
        ));
    let (solo_timeout_secs, team_timeout_secs, scan_interval_secs) = roseui_ai_agent::resolve_idle_config_from_env();
    let idle_scanner_handle = roseui_ai_agent::start_idle_scanner_with_coordinator(
