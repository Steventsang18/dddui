...
/// the same binary. When `ROSEUI_BUILTIN_SKILLS_PATH` is set, skip
...
pub(super) async fn materialize_builtin_skills(data_dir: &Path) -> Result<()> {
    let skip = std::env::var(roseui_extension::BUILTIN_SKILLS_ENV_VAR)
        .map(|v| !v.is_empty())
        .unwrap_or(false);
    if skip {
        return Ok(());
    ...
    let corpus = roseui_extension::builtin_skills_corpus();
    let marker = roseui_extension::builtin_skills_materialize_marker(corpus, env!("CARGO_PKG_VERSION"));

    roseui_extension::materialize_if_needed(data_dir, corpus, &marker)
        .await
...
mod tests {
    ...
    fn materialize_marker_includes_builtin_skill_corpus_fingerprint() {
        let marker = roseui_extension::builtin_skills_materialize_marker(
            roseui_extension::builtin_skills_corpus(),
