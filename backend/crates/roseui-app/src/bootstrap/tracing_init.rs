...
const NOISE_SUPPRESSIONS: &[&str] = &[
    ...
    // out of default dev logs; roseui_ai_agent::protocol::acp emits sanitized
...
pub fn init_tracing(log_dir: &Path, log_level: Option<&str>) -> Result<LogGuards, BootstrapError> {
    ...
    let file_appender = DailyDatedLogWriter::new(log_dir.to_path_buf(), "dodiddoneui.log");
...
mod tests {
    ...
    fn env_filter_suppresses_raw_acp_sdk_jsonrpc_debug_even_when_debug_enabled() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            assert!(
                tracing::enabled!(target: "roseui_ai_agent::protocol::acp", Level::DEBUG),
                "RoseUi ACP sanitized debug summaries should still be available"
            );
        });
    }
    ...
    fn env_filter_suppresses_raw_aionrs_provider_debug_even_when_debug_enabled() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            assert!(
                tracing::enabled!(target: "roseui_ai_agent::manager::aionrs::agent", Level::DEBUG),
                "RoseUi aionrs lifecycle debug logs should still be available"
            );
        });
    }
    ...
    fn dated_log_dir_appends_date_partition() {
        let root = Path::new("/tmp/roseui-logs");
    ...
    fn dated_file_writer_moves_new_day_files_into_matching_day_directory() {
        ...
        let first_path = tmp.path().join("2026/07/02/2026-07-02.dodiddoneui.log");
        let second_path = tmp.path().join("2026/07/03/2026-07-03.dodiddoneui.log");
        ...
        assert!(!tmp.path().join("2026/07/02/2026-07-03.dodiddoneui.log").exists());
    }
}
