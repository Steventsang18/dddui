...
use roseui_app::{AppConfig, IdentityMode};
use roseui_db::Database;
...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment, BootstrapError> {
    ...
    unsafe {
        std::env::set_var("ROSEUI_WORK_DIR", &work_dir);
    }
...
fn validate_identity_environment(
    ...
    if identity_mode == IdentityMode::RosePro && bootstrap_secret.is_none() {
        return Err(BootstrapError::new(
            BootstrapErrorCode::ConfigInvalid,
            "config.identity_mode",
            "RosePro identity mode requires AIONCORE_BOOTSTRAP_SECRET",
        ));
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    roseui_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
    ...
    let database = roseui_db::init_database_staged_with_options(
        &db_path,
        roseui_db::DatabaseInitOptions {
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = roseui_db::DatabaseInitError::new(
            "database.migration",
            roseui_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = roseui_db::DatabaseInitError::new(
            ...
            roseui_db::DbError::Init("repair failed".into()),
        );
    ...
    fn database_recoverable_corruption_stage_comes_from_db_boundary_error() {
        let err = roseui_db::DatabaseInitError::new(
            ...
            roseui_db::DbError::Migration(sqlx::migrate::MigrateError::ExecuteMigration(
    ...
    fn aionpro_identity_requires_bootstrap_secret() {
        let err = validate_identity_environment(IdentityMode::RosePro, None)
            .expect_err("RosePro startup must require bootstrap secret");
    ...
    fn aionpro_identity_accepts_bootstrap_secret() {
        validate_identity_environment(IdentityMode::RosePro, Some("secret"))
            .expect("RosePro startup should accept configured bootstrap secret");
    }
