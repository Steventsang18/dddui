...
use roseui_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<roseui_extension::SkillPaths>,
    /// User skill metadata and import history repository.
    pub skill_repo: Arc<dyn ISkillRepository>,
    runtime_helper_bin: String,
    runtime_base_url: String,
    /// Optional external static directory overriding the embedded frontend.
    pub static_dir: Option<PathBuf>,
    /// When `true`, do not open the default browser after startup.
    pub no_open: bool,
    /// Local base URL (`http://host:port`) used to open the browser on startup.
...
impl AppServices {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        let skill_paths = Arc::new(roseui_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        if identity_mode.is_local() {
            roseui_extension::sync_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
                .await
                .map_err(|e| anyhow::anyhow!("Failed to synchronize skill catalog: {e}"))?;
        } else {
            // RosePro: never ingest the legacy shared skill directory — its
            // files carry no account attribution and would only create rows
            // for the never-logged-in local default user.
            roseui_extension::sync_builtin_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
                .await
                .map_err(|e| anyhow::anyhow!("Failed to synchronize skill catalog: {e}"))?;
        }
        ...
        let backend_binary_path =
            Arc::new(std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("dodiddoneui")));
        ...
        let process_registry = Arc::new(roseui_process::FileRegistryStore::new(&data_dir));
        let machine_id = roseui_process::local_machine_id(&data_dir);
        let session_spawner: Arc<dyn roseui_process::Spawner> = Arc::new(roseui_process::RealSpawner::new(
            process_registry,
            uuid::Uuid::now_v7(),
            machine_id,
        ));
