...
use roseui_api_types::ErrorResponse;
use roseui_assets::{AssetRouterState, asset_routes};
use roseui_assistant::assistant_routes;
use roseui_auth::{
...
use roseui_channel::channel_routes;
#[cfg(feature = "weixin")]
use roseui_channel::weixin_login_route;
use roseui_common::ApiErrorLogContext;
use roseui_conversation::{conversation_ops_routes, conversation_routes};
use roseui_cron::cron_routes;
use roseui_extension::{extension_routes, hub_routes, skill_routes};
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let auth_state = AuthRouterState {
        ...
        aionpro_mode: services.identity_mode == crate::config::IdentityMode::RosePro,
    };
    ...
    // Wiki routes protected by auth middleware
    let wiki_authenticated =
        wiki_routes(states.wiki.clone()).route_layer(from_fn_with_state(auth_mw_state.clone(), auth_middleware));
    ...
    .merge(office_proxy)
    .merge(public_assets)
    .merge(webhost_routes(services.static_dir.clone()))
    ...
    let router = router.layer(DefaultBodyLimit::max(roseui_common::constants::BODY_LIMIT));
...
/// as the conversation-helper credential channel (roseui-auth cannot depend on
/// roseui-ai-agent, so the binding happens here in the composition layer).
...
fn auth_identity_mode(identity_mode: crate::config::IdentityMode) -> AuthIdentityMode {
    match identity_mode {
        crate::config::IdentityMode::Local | crate::config::IdentityMode::Owner => AuthIdentityMode::Local,
        crate::config::IdentityMode::WebUi => AuthIdentityMode::UserSession,
        crate::config::IdentityMode::RosePro => AuthIdentityMode::RosePro,
    }
}
