...
use roseui_api_types::{AssistantConversationRequest, CreateConversationRequest, GetConfigOptionsResponse};
...
use roseui_db::models::{AgentMetadataRow, MessageRow};
use roseui_db::{IAgentMetadataRepository, IConversationRepository};
use roseui_team::{
...
impl NativeSlashCommandPort for TeamConversationAdapters {
    ...
    /// the SHARED grammar owner (`roseui_session::slash_command_name`, same
    ...
    async fn recognize(&self, conversation_id: &str, content: &str) -> SlashCommandRecognition {
        let Some(name) = roseui_session::slash_command_name(content) else {
...
impl TeamConversationAdapters {
    ...
    async fn resolve_slash_catalog(&self, conversation_id: &str) -> Option<(Vec<String>, SlashCatalogSource)> {
        ...
        {
            let names = roseui_session::codex_capabilities()
                .slash_commands
                .into_iter()
