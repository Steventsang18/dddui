---
name: roseui-webui-public
description: >-
  Expose the user's local DoDidDoneUi WebUI to the public internet with a near-zero-effort flow. Detects whether the WebUI is running, guides the user to switch it on if needed (the only manual step), self-installs cloudflared cross-platform, opens a Cloudflare quick tunnel, verifies the public URL actually works, then explains the limitations honestly (temporary/random URL, must stay running, password is the only protection, traffic transits Cloudflare). Use whenever the user wants to reach their DoDidDoneUi from outside the LAN, over the internet, or share a public link. Distinct from roseui-webui-setup (which covers manual LAN / Tailscale / server config through the settings UI): this skill produces a one-click public link via an automatic tunnel.
---
...
# DoDidDoneUi WebUI Public Access Guide

You help a user turn their local DoDidDoneUi WebUI (LAN-only at best) into a public internet URL, with the user doing almost nothing. You have a shell (Bash) and run on the same machine as DoDidDoneUi, so you do the work yourself. The user only does what you architecturally cannot: flip the WebUI toggle in the desktop UI.

## Core facts (verified, do not re-derive)

- DoDidDoneUi WebUI is a local HTTP server on port 25808 (the dodiddoneui default; override with the `--port` CLI flag — so don't hardcode it, confirm with the curl probe below). It has built-in user+password / JWT auth, which is the *only* thing protecting it once it's on the public internet — so the password is load-bearing, not a formality (see Step 6 risks).
...
```bash
mkdir -p ~/.roseui/tools && cd ~/.roseui/tools
...
(Use the path to the binary you installed, e.g. `~/.roseui/tools/cloudflared`.)
...
Retry 2-3 times with a few seconds between; a freshly created tunnel can return 530/000 for the first few seconds before the edge is ready. Consider it good only when you get 200. For extra confidence, confirm it is really DoDidDoneUi:

```bash
curl -s --max-time 20 "<public-url>/" | grep -i "<title>DoDidDoneUi</title>"
```
...
between the open internet and the user's DoDidDoneUi. **Before** you give them the
...
> These `/api/webui/*` credential endpoints only work when dodiddoneui runs in
> **local mode** (the normal desktop/Electron case, which is exactly the scenario
> this skill targets). If dodiddoneui is deployed as a standalone server, they
