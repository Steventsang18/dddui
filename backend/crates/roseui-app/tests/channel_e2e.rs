...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn roseui_db::IChannelRepository> =
        std::sync::Arc::new(roseui_db::SqliteChannelRepository::new(pool));
    let pairing_svc = roseui_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
