...
use roseui_db::{IConversationRepository, MessagePageDirection, MessagePageParams};
...
async fn ensure_default_team_agent_installed(services: &roseui_app::AppServices) {
...
async fn mark_claude_backend_team_mcp_stdio_capable(services: &roseui_app::AppServices) {
...
async fn tc6c_create_team_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("roseui-team-missing-{}", roseui_common::generate_short_id()));
