//! RosePro-mode gate: machine startup must not mint local-default-user rows.
//!
...
//! account that never logs in on an RosePro machine. This e2e boots the full
//! router in RosePro mode and then sweeps EVERY ownership column in the live
...
async fn aionpro_startup_writes_no_system_default_user_rows() {
    ...
    let _router = roseui_app::create_router(&services).await.expect("build router");
    ...
    assert!(
        offenders.is_empty(),
        "RosePro startup minted local-default-user rows — a machine-level \
