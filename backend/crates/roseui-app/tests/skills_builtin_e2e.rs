...
//! `roseui_app::create_router_with_states` against an in-memory DB.
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("ROSEUI_BUILTIN_SKILLS_PATH");
    }
    ...
    roseui_extension::materialize_if_needed(&data_dir, roseui_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.roseui/`,
    ...
    let skill_repo = Arc::new(roseui_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    roseui_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        names.contains(&"roseui-config"),
        "roseui-config should be shipped as an auto-inject builtin skill: {names:?}",
    );
    assert!(
        !names.contains(&"roseui-skills"),
        "roseui-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
