...
use roseui_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
...
async fn upsert_visible_agent_metadata(services: &roseui_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
...
async fn agent_logos_endpoint_returns_backend_to_logo_catalog() {
    ...
    assert_eq!(logo_for("aionrs").as_deref(), Some("/api/assets/logos/brand/app.png"));
...
async fn agent_logos_endpoint_includes_disabled_and_missing_rows() {
    ...
    services
        ...
        .upsert(&UpsertAgentMetadataParams {
            id: "logo-only-row",
            icon: Some("/api/assets/logos/brand/app.png"),
            name: "Logo Only",
            name_i18n: None,
            description: None,
            description_i18n: None,
    ...
    assert_eq!(entry.unwrap()["logo"], "/api/assets/logos/brand/app.png");
}
