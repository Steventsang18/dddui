...
//! mirroring `roseui-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(roseui_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = roseui_system::ClientPrefService::new(repo);
    let mut req = roseui_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the roseui-shell integration
