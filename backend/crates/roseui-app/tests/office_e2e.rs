...
//! integration level in `roseui-office/tests/`.
...
use roseui_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use roseui_office::{ConversionService, OfficeRouterState, OfficecliWatchManager, ProxyService, SnapshotService};
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use roseui_office::error::OfficeError;
    use roseui_office::types::DocType;
    use roseui_office::{ProcessHandle, ProcessSpawner};
    ...
    impl roseui_realtime::EventBroadcaster for NoopBroadcaster {
        fn broadcast(&self, _msg: roseui_api_types::WebSocketMessage<serde_json::Value>) {}
    }

    let spawner: Arc<dyn ProcessSpawner> = Arc::new(NoopSpawner);
    let bc: Arc<dyn roseui_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
