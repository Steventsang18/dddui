...
//! against a real `roseui_app::create_router_with_states` instance backed by
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(
        &self,
        _user_id: &str,
    ) -> Result<Vec<AgentManagementRow>, roseui_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.roseui/` for the state store and
    ...
    // `~/.roseui/` for user data — neither is appropriate for tests.
    ...
    provider_repo
        .create(roseui_db::CreateProviderParams {
...
async fn rule_and_skill_files_are_isolated_by_current_user() {
    ...
    assert_eq!(
        std::fs::read_to_string(
            rule_root
                .join("users")
                .join(roseui_common::user_dir_name(&user_b.id).unwrap())
                .join("shared-assistant.md")
        )
        .unwrap(),
        "rule-b"
...
async fn avatars_of_two_users_are_physically_isolated() {
    ...
    let dir_b = roseui_common::user_dir_name(&user_b.id).unwrap();
