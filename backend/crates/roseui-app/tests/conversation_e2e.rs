...
async fn t1_5c_create_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("roseui-conv-missing-{}", roseui_common::generate_short_id()));
...
async fn t2_4_list_source_filter() {
    ...
    // Create 2 roseui + 1 telegram
    for _ in 0..2 {
        let req = json_with_token("POST", "/api/conversations", create_body("Roseui Conv"), &token, &csrf);
        app.clone().oneshot(req).await.unwrap();
    }
...
async fn auto_workspaces_of_two_users_live_under_distinct_user_roots() {
    ...
    let user_b = services
        ...
        .unwrap()
        .expect("bob exists");
    let dir_a = roseui_common::user_dir_name(&user_a.id).unwrap();
