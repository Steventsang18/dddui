...
async fn insert_message(
    services: &roseui_app::AppServices,
    conv_id: &str,
    msg_id: &str,
    content: &str,
    created_at: i64,
) {
    let repo = roseui_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
    let msg = roseui_db::models::MessageRow {
        id: msg_id.into(),
    ...
    roseui_db::IConversationRepository::insert_message(&repo, &user_id, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &roseui_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = roseui_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
...
async fn upsert_artifact(services: &roseui_app::AppServices, artifact: roseui_db::ConversationArtifactRow) {
    let repo = roseui_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(&artifact.conversation_id).await.unwrap().unwrap();
    roseui_db::IConversationRepository::upsert_artifact(&repo, &user_id, &artifact)
        .await
        .unwrap();
}
