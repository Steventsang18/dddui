//! E2E coverage for the agent-facing `dodiddoneui config` CLI.
...
fn config_command() -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_dodiddoneui"));
    command.arg("config");
    command
}
...
async fn fake_conversation_cron_list(
    ...
    if capture.is_none() {
        *capture = Some(Capture {
            conversation_id: header(&headers, "x-roseui-conversation-id"),
            user_id: header(&headers, "x-roseui-user-id"),
            job_id: None,
            resource_id: None,
            payload: None,
        });
...
async fn config_mcp_oauth_logout_reads_status_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-mcp")
        .env("ROSEUI_USER_ID", "user-mcp")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_capabilities_prints_agent_readable_contract_without_runtime_env() {
    let output = config_command()
        .arg("capabilities")
        .env_remove("ROSEUI_BASE_URL")
        .env_remove("ROSEUI_CONVERSATION_ID")
        .env_remove("ROSEUI_USER_ID")
        .output()
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        "resolve via ROSEUI_CONVERSATION_ID"
    );
...
async fn config_provider_update_reads_collection_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-provider")
        .env("ROSEUI_USER_ID", "user-provider")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_conversation_rename_patches_name_and_reads_resource_before_and_after() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-current")
        .env("ROSEUI_USER_ID", "user-rename")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_external_paths_add_reads_collection_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-skill")
        .env("ROSEUI_USER_ID", "user-skill")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_payload_commands_resolve_current_conversation_and_user_selectors() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-selector")
        .env("ROSEUI_USER_ID", "user-selector")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_agent_custom_update_reads_agent_id_from_stdin() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-agent")
        .env("ROSEUI_USER_ID", "user-agent")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_cron_job_skill_save_reads_job_id_from_stdin() {
    ...
    let mut child = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-cron")
        .env("ROSEUI_USER_ID", "user-cron")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_context_resolves_current_conversation_assistant() {
    ...
    let output = config_command()
        ...
        .env("ROSEUI_BASE_URL", &base_url)
        .env("ROSEUI_CONVERSATION_ID", "conv-current")
        .env("ROSEUI_USER_ID", "user-current")
        .output()
        .await
        .unwrap();
...
async fn config_context_fails_with_stable_error_when_conversation_env_missing() {
    ...
    assert!(stderr.contains(
        "CONFIG_ENV_MISSING command=\"config context\" field=\"ROSEUI_CONVERSATION_ID\": missing required environment variable"
    ));
}
...
fn builtin_config_skills_use_config_cli_not_python_or_cron_helper() {
    ...
    let roseui_config = std::fs::read_to_string(root.join("roseui-config/SKILL.md")).unwrap();
    let cron = std::fs::read_to_string(root.join("cron/SKILL.md")).unwrap();

    for forbidden in ["python3", "roseui_api.py", "lsof", "netstat", "curl"] {
        assert!(
            !roseui_config.contains(forbidden),
            "roseui-config skill must not mention {forbidden}"
        );
    }
    assert!(roseui_config.contains("\"$ROSEUI_HELPER_BIN\" config context"));
    assert!(roseui_config.contains("\"$ROSEUI_HELPER_BIN\" config capabilities"));
    assert!(roseui_config.contains("assistant_id\": \"current"));
    for command in [
        "\"$ROSEUI_HELPER_BIN\" config mcp servers",
        "\"$ROSEUI_HELPER_BIN\" config providers",
        "\"$ROSEUI_HELPER_BIN\" config settings",
        "\"$ROSEUI_HELPER_BIN\" config agents",
        "\"$ROSEUI_HELPER_BIN\" config cron jobs",
        "\"$ROSEUI_HELPER_BIN\" config skills external-paths",
    ] {
        assert!(
            roseui_config.contains(command),
            "roseui-config skill must document {command}"
        );
    }

    assert!(!cron.contains("cron-helper"));
    assert!(cron.contains("\"$ROSEUI_HELPER_BIN\" config cron current list"));
