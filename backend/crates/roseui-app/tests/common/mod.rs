//! Shared test helpers for roseui-app E2E tests.
#![allow(dead_code)]
...
use roseui_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use roseui_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use roseui_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use roseui_file::FileService;
use roseui_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_repo: std::sync::Arc::new(roseui_db::SqliteSkillRepository::new(services.database.pool().clone())),
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths, username: &str) {
    let repo = roseui_db::SqliteSkillRepository::new(services.database.pool().clone());
    let user = services
        .user_repo
    ...
    roseui_extension::sync_skill_catalog_into_repo_for_user(paths, &repo, &user.id)
        .await
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                roseui_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, roseui_ai_agent::AgentError>>
            + Send
            + Sync,
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> roseui_common::AgentType {
        roseui_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<roseui_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> roseui_common::TimestampMs {
        roseui_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<roseui_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: roseui_ai_agent::types::SendMessageData,
    ) -> Result<(), roseui_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), roseui_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<roseui_common::AgentKillReason>) -> Result<(), roseui_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<roseui_api_types::GetConfigOptionsResponse, roseui_ai_agent::AgentError> {
        Ok(roseui_api_types::GetConfigOptionsResponse {
            config_options: vec![roseui_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![roseui_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<roseui_api_types::SetConfigOptionResponse, roseui_ai_agent::AgentError> {
        Ok(roseui_api_types::SetConfigOptionResponse {
            confirmation: roseui_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![roseui_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("roseui-csrf-token="))
        .map(|s| {
            s.strip_prefix("roseui-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("roseui-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = roseui_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
