...
//! at the service integration level in `roseui-cron/tests/service_integration.rs`.
...
async fn ensure_conversation(services: &roseui_app::AppServices, user_id: &str, conversation_id: &str, title: &str) {
...
fn cron_skill_does_not_instruct_agents_to_write_payload_files() {
    ...
    assert!(!skill.contains("/tmp/roseui-cron"));
    assert!(!skill.contains("python3"));
    assert!(!skill.contains("roseui_cron.py"));
    assert!(skill.contains("$ROSEUI_HELPER_BIN"));
...
async fn cj3b_create_accepts_workspace_with_whitespace_segment() {
    ...
    let (token, csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ensure_default_assistant(&mut app, &token, &csrf).await;
    let dir = std::env::temp_dir().join(format!("roseui-cron-test-{}", roseui_common::generate_short_id()));
...
async fn cj5b_run_now_legacy_workspace_with_whitespace_succeeds() {
    ...
    cron_repo
        .insert(&roseui_db::models::CronJobRow {
