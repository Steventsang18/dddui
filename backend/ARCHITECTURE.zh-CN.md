...
RoseCore 是 RoseUi 的后端服务，使用 Rust 构建（Axum + Tokio + SQLite）。
它通过 HTTP REST API 和 WebSocket 实时事件为 RoseUi 桌面客户端提供服务。
...
依赖方向严格向下流动。领域 crate 不可依赖 roseui-app，
roseui-common 没有任何内部依赖。
...
| Crate | 职责 |
|-------|------|
| `roseui-common` | 共享错误类型（ApiError）、枚举、ID 生成、加密工具、时间戳、分页 |
| `roseui-api-types` | 所有 HTTP/WebSocket 的请求和响应类型，是 API 契约的唯一定义处 |
| `roseui-db` | SQLite 数据库层，定义 Repository trait 和实现 |
| `roseui-assets` | 嵌入式静态资源（Agent 元数据、提示词） |
| `roseui-runtime` | 托管 Node、子进程管理、PATH 增强 |
...
| Crate | 职责 |
|-------|------|
| `roseui-auth` | JWT 认证、密码哈希、CSRF 保护、Cookie 管理、认证中间件 |
| `roseui-realtime` | WebSocket 连接管理、事件广播（BroadcastEventBus）、消息路由 |
...
| Crate | 职责 |
|-------|------|
| `roseui-conversation` | 对话管理、消息收发、确认机制、流式响应 |
| `roseui-channel` | 多渠道集成（微信、钉钉、飞书）、插件系统、配对会话 |
| `roseui-team` | 团队协作、任务调度、邮箱系统 |
| `roseui-cron` | 定时任务执行、Cron 表达式、事件触发 |
| `roseui-file` | 文件操作、监听、快照、Git 操作、压缩 |
| `roseui-office` | Office 文档处理（Excel、PPT、Word）、预览、转换 |
| `roseui-system` | 系统设置、提供商管理、版本检查、模型获取 |
| `roseui-mcp` | MCP 协议集成、OAuth、多平台适配器 |
| `roseui-ai-agent` | Agent 生命周期管理、Worker 任务队列、ACP/辅助技能 |
| `roseui-extension` | 扩展注册中心、Hub 管理、技能发现与安装 |
| `roseui-shell` | Shell 命令执行、语音转文字 |
| `roseui-assistant` | Assistant 配置与管理 |

### 组装层（Composition）

| Crate | 职责 |
|-------|------|
| `roseui-app` | 顶层二进制入口，组装所有 crate 为完整的 Axum 服务 |
...
每个领域 crate 遵循统一的内部组织模式，以 roseui-conversation 为参考示例：
...
所有响应类型定义在 `roseui-api-types` 中，这是 API 契约的唯一来源。
...
ACP Agent 的工具调用事件在 `roseui-ai-agent` 翻译层进入统一
...
所有数据库访问通过 trait 抽象，定义在 `roseui-db` 中：
...
| 类型 | 位置 | 用途 | 示例 |
|------|------|------|------|
| Row 模型 | `roseui-db/src/models/` | 数据库行映射 | `ConversationRow` |
| Params 对象 | `roseui-db/src/repository/` | 数据库写入参数 | `UpdateConversationParams` |
| 请求/响应类型 | `roseui-api-types` | API 契约与共享 DTO | `CreateConversationRequest`、`ConversationResponse` |

**Service 层可以直接使用 `roseui-api-types` 中的类型。** 该 crate 是纯数据结构定义，
不依赖任何 HTTP 框架，本质上是共享 DTO 层。

⚠️ **关键约束：`roseui-api-types` 禁止依赖 axum、tower 等 HTTP 框架，
...
- **Repository（roseui-db）**：纯数据库操作，不包含业务逻辑
...
- 迁移文件位于 `crates/roseui-db/migrations/`
...
```
DbError（数据库层）
  ↓ From trait 实现（roseui-db/src/error.rs）
ApiError（统一错误类型）
  ↓ IntoResponse 实现
...
`roseui-app` 中定义 `AppServices`，集中持有所有共享依赖：
...
- **领域 crate 不负责构建自己的依赖**——只定义需要什么（RouterState），由 `roseui-app` 负责组装
...
- Token 提取优先级：`Authorization: Bearer` 头 → `roseui-session` Cookie
- 支持 Token 黑名单（SHA-256 哈希，DashMap 存储）

### CSRF 保护

采用 Double Submit Cookie 模式：
- Cookie 名：`roseui-csrf-token`（非 HttpOnly，JavaScript 需读取）
- 请求头：`x-csrf-token`
...
|--------|----------|--------|----------|---------|
| `roseui-session` | ✅ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
| `roseui-csrf-token` | ❌ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
...
| E2E 测试 | `crates/roseui-app/tests/` | 内存 SQLite | 完整 HTTP 请求链路验证 |
...
`roseui-app/tests/common/mod.rs` 提供共享的测试工具：
...
以添加 `roseui-my-feature` 为例：

**第一步：创建 crate 并注册到 workspace**

1. 创建目录 `crates/roseui-my-feature/`
...
**第三步：如需数据库，在 roseui-db 中添加**

1. 在 `models/` 添加 Row 模型
2. 在 `repository/` 定义 Repository trait（`I` 前缀）和 Sqlite 实现
3. 在 `migrations/` 添加迁移文件（`NNN_descriptive_name.sql`）

**第四步：如需 API 类型，在 roseui-api-types 中添加**

在 `roseui-api-types` 中定义请求/响应类型，保持 API 契约集中管理。

**第五步：接入 roseui-app**

1. 在 `roseui-app/Cargo.toml` 添加依赖：
   ```toml
...
- [ ] Repository trait 定义在 roseui-db，实现使用 Sqlite 前缀
- [ ] API 类型定义在 roseui-api-types
...
内置 ACP adapter 通过 `crates/roseui-runtime/src/node_runtime/` 中的托管
...
`roseui_runtime::enhance_process_path()`，使后续所有
...
新的子进程启动点应通过
`roseui_runtime::Builder::agent(program)`（长期运行的 Agent CLI，
调用者拥有 stdio）或 `roseui_runtime::Builder::clean_cli(program)`
