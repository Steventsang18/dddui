// Vendored external engine (MIT, Steventsang18). Lint discipline for this crate
// is owned upstream; blanket-allow all warnings so the host project's `-D warnings`
// gate only covers first-party crates.
#![allow(warnings)]
