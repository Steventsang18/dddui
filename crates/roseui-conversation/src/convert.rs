...
use roseui_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
use roseui_common::{
    AgentType, ConversationSource, ConversationStatus, MessagePosition, MessageStatus, MessageType, ProviderWithModel,
};
use roseui_db::MessageSearchRow;
use roseui_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// RoseUi stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use roseui_common::{AgentType, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    #[test]
    fn parse_provider_with_model_roseui_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("roseui"),
            None,
            r#"{"workspace":"/srv/roseui-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/roseui-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("roseui"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/roseui-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("roseui".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::Roseui));
