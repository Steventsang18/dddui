//! `roseui-process` — self-contained subprocess mechanism (feature 001).
//!
...
//! existing `CliAgentProcess` / process registry in `roseui-ai-agent`.
//!
//! "Bytes not semantics": it never parses agent output, holds no session
//! state, and never mutates `std::env`. It depends only on `roseui-common`
//! and `roseui-runtime`.
//!
//! ## Isolation contract (why two mechanisms coexist without conflict)
//! All shared resources are namespaced under `{data_dir}/runtime/roseui-process/`
