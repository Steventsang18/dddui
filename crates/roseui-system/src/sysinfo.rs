use roseui_api_types::SystemInfoResponse;
...
///
/// Priority: `ROSEUI_CACHE_DIR` env → `dirs::cache_dir()/roseui`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("ROSEUI_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("roseui").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for AionUI.
///
/// Priority: `ROSEUI_WORK_DIR` env → `dirs::data_dir()/roseui`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("ROSEUI_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
///
/// Priority: `ROSEUI_LOG_DIR` env →
///   macOS: `~/Library/Logs/roseui`
///   Linux: `dirs::state_dir()/roseui/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/roseui/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("ROSEUI_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/roseui").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("roseui/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/roseui/logs
    dirs::data_dir()
        .map(|p| p.join("roseui/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "roseui".
        let dir = resolve_cache_dir();
        assert!(dir.contains("roseui"), "cache_dir should contain 'roseui': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("roseui"), "work_dir should contain 'roseui': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("roseui"), "log_dir should contain 'roseui': {dir}");
    }
}
