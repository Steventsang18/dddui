...
use roseui_file::{FileError, path_safety::validate_path_with_extra_root};
...
) -> Result<Json<ApiResponse<roseui_api_types::DocumentConversionResponse>>, ApiError> {
    let Json(req) = body.map_err(ApiError::from)?;
    let validated_path = validate_office_path(&state, &req.file_path, req.workspace.as_deref())?;
    let resp = state
        .conversion_service
