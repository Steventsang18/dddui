...
mod tests {
    ...
    const CHILD_MARKER: &str = "ROSEUI_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("ROSEUI_CURRENT_ONLY", "from-current")
                .env("ROSEUI_OVERLAY", "from-current")
        ...
        assert_eq!(value("ROSEUI_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("ROSEUI_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("ROSEUI_OVERLAY").as_deref(), Some("from-shell"));
