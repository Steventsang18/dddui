...
use roseui_db::{IUserRepository, SqliteUserRepository, UserStatus, UserType, init_database_memory};
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", format!("roseui-csrf-token={token}"))
...
async fn t12_2_post_with_mismatched_csrf_tokens_rejected() {
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", "roseui-csrf-token=token_a")
...
async fn auth_middleware_aionpro_rejects_local_user_token() {
    ...
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteUserRepository::new(db.pool().clone()));
    let app = protected_auth_app_with_mode(jwt_service, repo as Arc<dyn IUserRepository>, AuthIdentityMode::RosePro);
...
async fn t12_2_csrf_cookie_set_on_first_request() {
    ...
    assert!(set_cookie.contains("roseui-csrf-token="));
...
fn t13_1_authorization_header_takes_priority() {
    ...
    headers.insert(header::COOKIE, "roseui-session=cookie_tok".parse().unwrap());
    assert_eq!(
        roseui_auth::extract_token_from_headers(&headers),
...
fn t13_2_cookie_fallback() {
    let mut headers = axum::http::HeaderMap::new();
    headers.insert(header::COOKIE, "roseui-session=fallback_tok".parse().unwrap());
    assert_eq!(
        roseui_auth::extract_token_from_headers(&headers),
...
fn t13_3_no_token_returns_none() {
    ...
    assert_eq!(roseui_auth::extract_token_from_headers(&headers), None);
}
...
fn helper_request(token: Option<&str>, user_id: &str, conversation_id: &str) -> Request<Body> {
    let mut builder = Request::get("/whoami")
        .header("x-roseui-user-id", user_id)
        .header("x-roseui-conversation-id", conversation_id);
    if let Some(token) = token {
        builder = builder.header("x-roseui-runtime-token", token);
    }
...
async fn runtime_token_channel_aionpro_authenticates_external_user() {
    ...
    let user = repo
        .ensure_external_user(
            UserType::Aionpro,
            "ext-user-1",
            roseui_db::ExternalUserProjection {
    ...
    let resp = app
        .oneshot(
            Request::post("/api/thing")
                .header("x-roseui-runtime-token", "tok-1")
                .body(Body::empty())
                .unwrap(),
        )
        .await
