...
//! - 否则回退到仓库相对路径
//!   `upstream-AionUi/packages/desktop/src/renderer/dist`
...
fn main() {
    ...
    let dist_dir: Option<PathBuf> = if let Ok(env_dist) = std::env::var("ROSEUI_FRONTEND_DIST") {
        ...
        let repo_root = manifest_dir
            ...
            .to_path_buf();
        let default_dist = repo_root.join("upstream-AionUi/packages/desktop/src/renderer/dist");
    ...
    match dist_dir {
        ...
        None => {
            ...
            if assets_has_content {
                ...
                panic!(
                    "No frontend dist found and assets/ is empty. Run `cd upstream-AionUi && npm run build` first, \
