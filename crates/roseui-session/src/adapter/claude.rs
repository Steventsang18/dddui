...
use roseui_process::{BoxedStdin, ProcessError, Spawner};
    ...
    fn parse_control_request(v: &Value) -> Vec<SessionEvent> {
        ...
        match (subtype, request_id.is_empty()) {
            ("can_use_tool", false) => {
                ...
                // approves blind (RoseUi issue #3779). This is display-to-the-approver,
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let proc = spawner.spawn(spec, &[], "roseui-session").await?;
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via ROSEUI_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("ROSEUI_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: roseui_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    /// (RoseUi issue #3779: with input dropped the card read "命令: Bash" and the
