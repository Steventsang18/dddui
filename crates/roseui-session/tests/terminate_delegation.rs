...
use roseui_session::testing::CountingTerminateIo;
use roseui_session::{AcpSessionBackend, ClaudeSessionBackend, CodexSessionBackend, SessionBackend};
...
/// covered in `roseui-process`; it cannot be unit-constructed here without a
/// real OS process).
#[tokio::test]
async fn fake_agent_io_terminate_is_default_noop() {
    use roseui_session::AgentIo;
    let io = roseui_session::testing::FakeAgentIo::never_exits(Vec::new());
