...
//!
//! These tests exercise the public API surface of `roseui_extension::skill_service`
//! and `roseui_extension::external_paths` against the functional requirements in
...
use roseui_extension::external_paths::ExternalPathsManager;
...
///
/// Production mode: no `ROSEUI_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn sm11_get_skill_paths() {
    ...
    let data_dir = Path::new("/home/user/.roseui");
...
async fn cp4_enable_skills_market() {
    ...
    let paths = mgr.get_custom_external_paths().await;
    assert_eq!(paths.len(), 1);
    assert_eq!(paths[0].name, "roseui-skills");
}
