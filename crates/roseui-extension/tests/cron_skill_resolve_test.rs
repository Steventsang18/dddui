use roseui_db::{ISkillRepository, IUserRepository, SqliteSkillRepository, SqliteUserRepository, UpsertSkillParams};
use roseui_extension::{resolve_skill_paths, skill_service};
use std::sync::Arc;
...
fn unique_temp_dir(label: &str) -> std::path::PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    std::env::temp_dir().join(format!("roseui-extension-{label}-{}-{nanos}", std::process::id()))
}
