...
use roseui_db::{Database, IProjectStore, SqliteProjectStore, init_database_memory};
use roseui_project::ProjectService;
use roseui_project::canonical::{canonicalize, to_file_uri};
use roseui_project::types::{AttachInput, FileOp, ReferenceInput};
...
async fn create_standard_missing_dir_is_folder_not_found() {
    let (svc, _db) = service().await;
    let missing = uri_of(std::path::Path::new("/nonexistent-roseui-xyz-8f3a2b1c"));
