...
/// MCP Agent adapter for Roseui itself.
///
/// Unlike CLI-based adapters, this adapter reads/writes directly to the
/// local database. It is always "installed" since RoseUi is the host
/// application.
///
/// # Behavior
///
...
impl McpAgentAdapter for RoseuiAdapter {
    ...
    async fn install_server(&self, _name: &str, _transport: &McpServerTransport) -> Result<(), McpError> {
        ...
        // Roseui the server is already in the DB.
        Ok(())
    }
...
mod tests {
    ...
    fn source_is_roseui() {
        ...
        let adapter = RoseuiAdapter::new(repo);
        assert_eq!(adapter.source(), McpSource::Roseui);
    }
    ...
    async fn trait_is_object_safe() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter: Arc<dyn McpAgentAdapter> = Arc::new(RoseuiAdapter::new(repo));
        assert_eq!(adapter.source(), McpSource::Roseui);
        assert!(adapter.is_installed().await.unwrap());
    }
}
