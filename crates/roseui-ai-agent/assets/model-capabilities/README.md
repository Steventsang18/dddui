...
`image_input_models.json` is a hand-maintained Dodiddoneui asset. It is embedded at compile time and is never downloaded or refreshed at runtime. Its API roots mirror the fixed `base_url` presets in RoseUi's `modelPlatforms.ts`; model IDs are maintained independently from `models.dev`.

The catalog is intentionally a positive allowlist:

- Match both the provider API root and the exact model ID.
- Add a model only when the provider's own documentation confirms image input on the API protocol used by Dodiddoneui.
...
- Keep an empty `models` array when the preset endpoint is known but no stable model ID can be positively verified for that endpoint. This still records the RoseUi preset without claiming image support.
- Aggregator entries may be refreshed manually from that aggregator's own catalog. The reviewed result must be committed as a static snapshot; Dodiddoneui never fetches it at runtime.

Poe bot names and Ctyun deployment model IDs are account- or deployment-specific, so they intentionally have no static model entries. DeepSeek does not currently expose a positively verified image-input chat model on the corresponding RoseUi preset endpoint.
...
- RoseUi provider presets: https://github.com/iOfficeAI/RoseUi/blob/main/packages/desktop/src/renderer/utils/model/modelPlatforms.ts
