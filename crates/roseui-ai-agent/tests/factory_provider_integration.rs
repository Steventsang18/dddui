...
use roseui_ai_agent::AcpSessionSyncService;
use roseui_ai_agent::AcpSkillManager;
use roseui_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
...
use roseui_api_types::AionrsBuildExtra;
use roseui_common::{AgentType, ProviderWithModel, encrypt_string};
use roseui_db::{
...
fn make_factory(
...
) -> roseui_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(roseui_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    let process_registry = Arc::new(roseui_process::FileRegistryStore::new(tmp.path()));
    let session_spawner: Arc<dyn roseui_process::Spawner> = Arc::new(roseui_process::RealSpawner::new(
        process_registry,
        uuid::Uuid::now_v7(),
        roseui_process::local_machine_id(tmp.path()),
    ));
