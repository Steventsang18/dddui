...
pub struct SkillDefinition {
    ...
    pub source: roseui_extension::SkillSource,
...
pub struct AcpSkillManager {
    ...
    #[allow(dead_code)]
    paths: Arc<roseui_extension::SkillPaths>,
...
impl AcpSkillManager {
    pub fn new(paths: Arc<roseui_extension::SkillPaths>) -> Arc<Self> {
        Arc::new(Self {
    ...
    pub fn new_with_repo(paths: Arc<roseui_extension::SkillPaths>, skill_repo: Arc<dyn ISkillRepository>) -> Arc<Self> {
        Arc::new(Self {
            cache: RwLock::new(HashMap::new()),
            discovered: RwLock::new(false),
            paths,
    ...
    /// Discover skills via `roseui_extension::list_available_skills`.
    ///
    /// Filtering rules:
    ...
    pub async fn discover_skills_for_user(
        ...
        for item in items {
            ...
            let keep = match item.source {
                roseui_extension::SkillSource::Builtin => {
                    if is_auto_inject {
                ...
                roseui_extension::SkillSource::Custom
                | roseui_extension::SkillSource::Cron
                | roseui_extension::SkillSource::Extension => {
                    enabled_skills.is_some_and(|en| en.iter().any(|n| n == &item.name))
                }
            };
            if !keep {
    ...
    async fn list_available_skills_for_user(
        &self,
        user_id: &str,
    ) -> Result<Vec<roseui_extension::SkillListItem>, roseui_extension::ExtensionError> {
        if let Some(repo) = &self.skill_repo {
            roseui_extension::list_available_skills_with_repo_for_user(&self.paths, repo.as_ref(), user_id).await
        } else {
            ...
            roseui_extension::list_available_skills(&self.paths).await
        }
    }
    ...
    /// - `Builtin` → `roseui_extension::read_builtin_skill(&paths, relative)`
        ...
        let content = match def.source {
            roseui_extension::SkillSource::Builtin => {
                if let Some(rel) = def.relative_location.as_deref() {
                    match roseui_extension::read_builtin_skill(&self.paths, rel).await {
                        Ok(s) => s,
                        Err(e) => {
...
mod tests {
    ...
    async fn new_accepts_skill_paths() {
        ...
        let paths = std::sync::Arc::new(roseui_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    fn skill_definition_has_source_and_relative_location() {
        let def = SkillDefinition {
            ...
            source: roseui_extension::SkillSource::Builtin,
            relative_location: Some("auto-inject/x/SKILL.md".into()),
            body: None,
        };
        assert_eq!(def.source, roseui_extension::SkillSource::Builtin);
        assert_eq!(def.relative_location.as_deref(), Some("auto-inject/x/SKILL.md"));
    }

    // Frontmatter parsing tests live in roseui-extension (covers
    ...
    // because they now need `roseui_extension::BUILTIN_SKILLS_ENV_VAR` to
    ...
    async fn get_skill_unknown_returns_none() {
        let tmp = TempDir::new().unwrap();
        let mgr = AcpSkillManager::new(std::sync::Arc::new(roseui_extension::resolve_skill_paths(
            tmp.path(),
            tmp.path(),
        )));
