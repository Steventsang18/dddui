...
use roseui_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, ErrorChain, TimestampMs, now_ms};
...
fn resolve_roseui_config(cli_args: &CliArgs) -> Result<Config, AgentError> {
    let mut config =
        Config::resolve(cli_args).map_err(|e| AgentError::internal(format!("Config resolve failed: {e}")))?;

    // RoseUi owns the embedded runtime policy. Standalone aionrs max-token
...
impl AionrsAgentManager {
    pub async fn new(
        ...
        let mut config = resolve_roseui_config(&cli_args)?;
