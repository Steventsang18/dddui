...
pub(crate) async fn load_stt_config(
    ...
    user_id: &str,
) -> Result<SpeechToTextConfig, roseui_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: roseui_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(roseui_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = roseui_system::ClientPrefService::new(repo);
