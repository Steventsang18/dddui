...
use roseui_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_id, now_ms};
...
impl TeamSessionService {
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                roseui_api_types::TeamRunStatus::Accepted
                    | roseui_api_types::TeamRunStatus::Running
                    | roseui_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = roseui_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
...
mod tests {
    ...
    use roseui_api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use roseui_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use roseui_db::{IConversationRepository, ITeamRepository};
    ...
    impl IAgentTask for ModeSettingAgent {
        ...
        fn workspace(&self) -> &str {
            "/tmp/dodiddoneui-team-mode-test"
        }
    ...
    fn two_agent_team_request(name: &str) -> roseui_api_types::CreateTeamRequest {
        roseui_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                roseui_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_aionrs_worker_request(name: &str) -> roseui_api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(roseui_api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
