...
use roseui_api_types::TeamToolTransport;

mod wake_summary;

pub use roseui_team_prompts::AvailableAssistant;

use crate::types::{MailboxMessage, MailboxMessageType, TeamAgent, TeamTask};

fn to_prompt_role(role: crate::types::TeammateRole) -> roseui_team_prompts::TeamPromptRole {
    match role {
        crate::types::TeammateRole::Lead => roseui_team_prompts::TeamPromptRole::Lead,
        crate::types::TeammateRole::Teammate => roseui_team_prompts::TeamPromptRole::Teammate,
    }
}

fn to_prompt_agent(agent: &TeamAgent) -> roseui_team_prompts::TeamPromptAgent {
    roseui_team_prompts::TeamPromptAgent {
...
///
/// Delegates to `roseui-team-prompts`, the canonical Team role prompt crate.
...
pub fn build_lead_prompt_for_transport(
    ...
    let body = roseui_team_prompts::build_lead_prompt(&roseui_team_prompts::LeadPromptParams {
        agent: &prompt_agent,
        team_name,
...
pub fn build_teammate_prompt_for_transport(
    ...
    let leader = prompt_members
        .iter()
        .find(|candidate| candidate.role == roseui_team_prompts::TeamPromptRole::Lead)
        .cloned()
        .unwrap_or_else(|| roseui_team_prompts::TeamPromptAgent {
            slot_id: "lead".to_owned(),
            name: "Lead".to_owned(),
            role: roseui_team_prompts::TeamPromptRole::Lead,
            backend: agent.backend.clone(),
            model: agent.model.clone(),
            status: None,
        });
    ...
    roseui_team_prompts::build_teammate_prompt(&roseui_team_prompts::TeammatePromptParams {
        agent: &prompt_agent,
        team_name,
        leader: &leader,
        teammates: &teammates,
