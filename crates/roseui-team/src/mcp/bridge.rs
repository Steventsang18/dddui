...
pub use roseui_api_types::{TEAM_MCP_SERVER_NAME, TeamMcpStdioConfig};

/// Stdio MCP server description ready to be handed to `session/new`.
///
/// Field shapes:
/// - `name` = `"roseui-team"` (fixed; team routing is done via `port` + `token`,
...
impl TeamMcpStdioServerSpec {
    ...
    ///
    /// `backend_binary_path` is the absolute path to the `dodiddoneui`
...
mod tests {
    ...
    fn from_config_fills_all_fields() {
        let spec = TeamMcpStdioServerSpec::from_config("/usr/bin/dodiddoneui", &sample_cfg());

        assert_eq!(spec.name, TEAM_MCP_SERVER_NAME);
        assert_eq!(spec.command, "/usr/bin/dodiddoneui");
    ...
    fn into_sdk_serializes_as_stdio_variant() {
        let spec = TeamMcpStdioServerSpec::from_config("/bin/dodiddoneui", &sample_cfg());
        ...
        assert_eq!(json["command"], "/bin/dodiddoneui");
