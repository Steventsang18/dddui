...
pub fn all_tool_descriptors_for_role(caller_role: TeammateRole) -> Vec<ToolDescriptor> {
    ...
    roseui_api_types::team_tool_descriptors_for_role(role)
}
...
pub fn authorize_tool(caller_role: TeammateRole, tool_name: &str) -> Result<(), String> {
    let Some(spec) = roseui_api_types::team_tool_descriptor(tool_name) else {
...
pub fn is_builtin_mcp_backend(backend: &str) -> bool {
    backend == roseui_common::constants::AIONRS_RUNTIME_BACKEND
}
...
mod tests {
    ...
    #[test]
    fn team_spawn_agent_description_is_roseui_original() {
        ...
        assert!(
            ...
            "description must be the full RoseUi original, not the legacy one-liner"
        );
        assert!(
