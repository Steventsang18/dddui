...
**双保险：**
1. `crates/roseui-team/src/guide/server.rs:156` — send_message 追加 `[SYSTEM NOTE]` 明确指示直接 spawn
2. `crates/roseui-team/src/prompts/prompt_templates/lead.txt` — 规则 9/10/88 增加 SYSTEM NOTE 例外
...
- `crates/roseui-team/src/service.rs` — create_team 成功后 broadcast `team.created` 事件
- 同时在 adopt 旧 conversation 时广播 `conversation.listChanged(updated)` 让前端移除旧对话

### 前端待做

前端需监听 `team.created` + `conversation.listChanged` 实现自动跳转。已提 issue: https://github.com/iOfficeAI/RoseUi/issues/2734
...
- `crates/roseui-team/src/service.rs` — `rebuild_agent_processes` 的 patch 加 `session_mode`
- `crates/roseui-team/src/session.rs` — `attach_spawned_agent_process_bg` 的 patch 加 `session_mode`
...
`crates/roseui-team/src/mcp/server.rs` — `exec_shutdown_agent` 写入 mailbox 后调用 `svc.wake_agent_in_session(team_id, &target_slot_id)` 唤醒目标。
...
`crates/roseui-team/src/service.rs` — 新增 `ensure_session_locks: DashMap<String, Mutex>` 字段，`ensure_session` 加 per-team mutex + double-check locking。
...
`crates/roseui-app/src/guide_stdio.rs` — `forward_tool` 加最多 3 次重试（间隔 500ms/1s/1.5s）。
...
```
crates/roseui-team/src/service.rs          — wake_lock 修复、StreamRelay、ensure_session 锁、team.created 事件、conversation.listChanged、session_mode
crates/roseui-team/src/session.rs          — try_wake 加 StreamRelay、移除 leader mirror 跳过、session_mode、msg_id 去重
crates/roseui-team/src/guide/server.rs     — send_message 加 [SYSTEM NOTE]
crates/roseui-team/src/prompts/prompt_templates/lead.txt — SYSTEM NOTE 例外规则
crates/roseui-team/src/mcp/server.rs       — shutdown_agent 加 wake
crates/roseui-team/docs/phase2/bugfix.md   — 本文档
crates/roseui-conversation/src/service.rs  — pub fn repo() accessor
crates/roseui-app/src/guide_stdio.rs       — forward_tool 重试逻辑
```
...
1. build release 后必须执行 `pkill -f roseui-backend` 杀掉旧进程，再重启前端
2. 当前 release binary 已包含所有修复：`/Users/zhuqingyu/project/roseui-backend/target/release/roseui-backend`
...
- Backend binary: `/Users/zhuqingyu/project/roseui-backend/target/release/roseui-backend`
- Symlink: `~/.local/bin/roseui-backend` → 上述 binary
- Frontend: `/Users/zhuqingyu/project/RoseUi` branch `feat/backend-migration`
- Backend fix branch: `fix/team-communication-bugs`
- Log file: `/Users/zhuqingyu/Library/Logs/RoseUi-Dev/2026-05-02.backend.log`
- Frontend issue: https://github.com/iOfficeAI/RoseUi/issues/2734
