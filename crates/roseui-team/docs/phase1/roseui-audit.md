# RoseUi Team 模块后端能力审计（Phase 1）

> **目的**：对照 RoseUi 前端 main 分支（`/Volumes/Macintosh HD/Users/zhuqingyu/project/RoseUi/`，commit `ed8a6bcd3`），列出 roseui-backend 后端需要复刻的所有 team 能力。
>
> **范围约束**：
...
>
> **RoseUi 源码路径索引**（下文中所有路径均相对前端根目录 `/Volumes/Macintosh HD/Users/zhuqingyu/project/RoseUi/`）：
>
> | 模块 | 路径 | 行数 |
> |------|------|------|
...
| `StdioMcpConfig` 形状 | `{ name: 'roseui-team-<teamId>', command: 'node', args: [scriptPath], env: [{name:'TEAM_MCP_PORT',value},{name:'TEAM_MCP_TOKEN',value},{name:'TEAM_AGENT_SLOT_ID',value}] }` | `TeamMcpServer.ts:44-49, 101-118` |
...
> RoseUi 走 ipcBridge；后端要包掉这些 team 逻辑意味着提供等价 HTTP/WebSocket 端点。具体 endpoint 命名/路径由后端决定，此处只列能力。
...
这些是 RoseUi 代码里写死的、前后端分离必须保留的行为契约：
...
- RoseUi 没有显式 REST 端点做 "aion_create_team"——它完全依赖 MCP + ipcBridge。后端要暴露 REST 需要后端自己设计。
...
- RoseUi 源码 commit：`ed8a6bcd3 fix(bundled-bun): add baseline variant for linux-x64 to support non-AVX2 CPUs (#2654)`（main 分支，2026-04-29 拉取）
- roseui-backend 所在 worktree：`/Users/zhuqingyu/.superset/worktrees/roseui-backend/repeated-algebra`，分支 `docs/api-for-frontend`
