...
use roseui_app::{AppConfig, AppServices, build_module_states, create_router_with_states, derive_encryption_key};
use roseui_common::{decrypt_string, now_ms};
...
async fn eq19_channel_status_merges_extension_meta_for_persisted_row() {
    ...
    repo.upsert_plugin(
        &owner_user_id,
        &roseui_db::models::ChannelPluginRow {
...
async fn skill_batch_import_reports_partial_failures_without_rolling_back_successes() {
    ...
    let skill_repo = roseui_db::SqliteSkillRepository::new(services.database.pool().clone());
    let alpha_row = roseui_db::ISkillRepository::find_by_name_for_user(&skill_repo, &user.id, "sample-alpha")
        .await
        .unwrap()
    ...
    assert!(
        roseui_db::ISkillRepository::find_by_name_for_user(&skill_repo, &user.id, "sample-beta")
            .await
            .unwrap()
            .is_none()
    );
