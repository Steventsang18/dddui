...
//! RoseUi → RosePro adoption moves the default user's files to the adopter.
//!
//! Boots the real AppServices (RosePro mode) so the assertions run against the
//! production filesystem layout, not hand-built paths.

use std::path::Path;

use roseui_extension::skill_service::{
...
async fn aionpro_services(data_dir: &Path) -> roseui_app::AppServices {
    let db = roseui_db::init_database_memory().await.unwrap();
    let config = roseui_app::AppConfig {
        identity_mode: roseui_app::IdentityMode::RosePro,
        bootstrap_secret: Some("bootstrap-secret".to_string()),
        data_dir: data_dir.to_path_buf(),
        work_dir: data_dir.to_path_buf(),
        ..Default::default()
    };
    roseui_app::AppServices::from_config(db, &config).await.unwrap()
}

async fn seed_user(services: &roseui_app::AppServices, username: &str) -> String {
    use roseui_db::IUserRepository;
    roseui_db::SqliteUserRepository::new(services.database.pool().clone())
...
async fn same_named_skill_is_physically_isolated_per_user() {
    ...
    let dir_a = roseui_common::user_dir_name(&user_a).unwrap();
    let dir_b = roseui_common::user_dir_name(&user_b).unwrap();
...
async fn provisioning_first_aionpro_user_adopts_default_user_files_on_disk() {
    let data = tempfile::tempdir().unwrap();
    let services = aionpro_services(data.path()).await;
    let (app, _runtime) = roseui_app::create_router_with_runtime(&services).await.unwrap();
    ...
    // Provision the first external (RosePro) user via the bootstrap-secret API
    ...
    let resp = app
        .oneshot(
            axum::http::Request::builder()
                ...
                .header("x-dodiddoneui-bootstrap-secret", "bootstrap-secret")
    ...
    let adopter_dir = roseui_common::user_dir_name(adopter).unwrap();
