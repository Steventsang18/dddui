//! RosePro-mode auth for the in-conversation helper CLI (`dodiddoneui config`).
//!
...
use roseui_ai_agent::{RuntimeTokenScope, TEAM_RUNTIME_TOKEN_SESSION_GENERATION};

const CONVERSATION_ID: &str = "conv-helper-auth";

async fn build_aionpro_app() -> (axum::Router, roseui_app::AppServices, String) {
    ...
    let resp = app
        .oneshot(
            Request::builder()
                ...
                .header("x-roseui-user-id", &user_id)
                .header("x-roseui-conversation-id", CONVERSATION_ID)
                .header("x-roseui-runtime-token", &issue.token)
                .body(Body::from("{}"))
                .unwrap(),
        )
        .await
