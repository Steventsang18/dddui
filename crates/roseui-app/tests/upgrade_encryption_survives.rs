...
use roseui_db::SqliteProviderRepository;
...
async fn upgrade_from_pre_user_scope_keeps_encryption_key() {
    ...
    // ── Phase 1: a genuine pre-user-scope (≤028) database ────────────────
    {
        ...
        let mut migrator: Migrator = sqlx::migrate!("../roseui-db/migrations");
        ...
        let key = roseui_app::derive_encryption_key("legacy-secret-0123456789");
        let enc = roseui_common::encrypt_string("sk-upgrade-SECRET", &key).unwrap();
        sqlx::query(
    ...
    let db = roseui_db::init_database(&db_path).await.unwrap();
    ...
    let key = roseui_app::derive_encryption_key(&services.jwt_secret_raw);
    let repo = Arc::new(SqliteProviderRepository::new(services.database.pool().clone()));
    let svc = roseui_system::ProviderService::new(repo, key);
    let list = svc
