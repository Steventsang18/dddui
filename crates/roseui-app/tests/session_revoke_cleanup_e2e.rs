...
//! services — revokes an RosePro session over HTTP, and asserts observable
...
fn bootstrap_json(method: &str, uri: &str, body: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("x-dodiddoneui-bootstrap-secret", BOOTSTRAP)
...
async fn http_revoke_runs_the_real_cleanup_hook_end_to_end() {
    ...
    let app = roseui_app::create_router(&services).await.expect("build router");

    // Provision an RosePro user + session over the internal API.
    let resp = app
        .clone()
