...
**Вариант B: Запуск RoseUi WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом RoseUi
...
- **Требования к настройке**: Запуск сервиса RoseUi WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите RoseUi**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице RoseUi и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим RoseUi WebUI?»
...
4. **Вариант B: Запуск RoseUi WebUI в удалённом режиме**
   - **Обязательно использовать навык roseui-webui-setup**: Обратиться к `references/roseui-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек RoseUi**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу RoseUi»
          - «В меню настроек нажмите на опцию \*\*'WebUI'»
          - «Войдите в интерфейс конфигурации WebUI»
        - **Шаги настройки**: Следуйте документации `roseui-webui-setup` навыка `references/roseui-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@roseui/webui` или подобные npm-пакеты**: WebUI — это встроенная функция RoseUi, а не отдельный пакет
...
### Навык roseui-webui-setup

**Основная документация**: `references/roseui-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/roseui-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу RoseUi)
...
3. **Выбрал WebUI**: Использовать навык `roseui-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `roseui-webui-setup`** (напрямую обратиться к `references/roseui-webui.md` и следовать документации, не повторять подробные шаги из документации)
