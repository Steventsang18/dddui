---
name: roseui-troubleshooting
description: >-
  Diagnose a running DoDidDoneUi installation: inspect stuck or errored conversations, read provider health, scheduled task state, MCP server health, team member state, backend health, and dodiddoneui logs. Use when the user reports DoDidDoneUi is misbehaving, a conversation is stuck, an LLM/provider call is failing, a scheduled task did not run, an MCP server has no tools, a team member is hung, or they ask to troubleshoot DoDidDoneUi.
---

# DoDidDoneUi Troubleshooting

Use the bundled `dodiddoneui diagnose` CLI for read-only troubleshooting. It uses
...
1. Use only `"$ROSEUI_HELPER_BIN" diagnose ...`.
...
4. Treat every command as read-only. To change DoDidDoneUi configuration, use the
   separate `roseui-config` skill.
...
```bash
"$ROSEUI_HELPER_BIN" diagnose capabilities
```
...
The CLI resolves `"current"` from `ROSEUI_CONVERSATION_ID`. Commands that read
the backend also use `ROSEUI_BASE_URL` and `ROSEUI_USER_ID` from the same
runtime context.

## Start Wide

For a vague "DoDidDoneUi is broken" report, run:

```bash
"$ROSEUI_HELPER_BIN" diagnose overview
```
...
```bash
"$ROSEUI_HELPER_BIN" diagnose conversations get <<'JSON'
{
  "conversation_id": "current"
}
JSON
...
```bash
"$ROSEUI_HELPER_BIN" diagnose conversations messages <<'JSON'
{
  "conversation_id": "current",
  "limit": 30,
  "errors_only": true
...
```bash
"$ROSEUI_HELPER_BIN" diagnose providers summary
```
...
```bash
"$ROSEUI_HELPER_BIN" diagnose cron summary
```
...
```bash
"$ROSEUI_HELPER_BIN" diagnose mcp summary
```
...
```bash
"$ROSEUI_HELPER_BIN" diagnose teams summary
```
...
```bash
"$ROSEUI_HELPER_BIN" diagnose health
```
...
Tail logs when the runtime provides `ROSEUI_LOG_DIR`:

```bash
"$ROSEUI_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "lines": 100,
  "errors_only": true,
  "conversation_id": "current"
...
If `ROSEUI_LOG_DIR` is unavailable and the user gives a log directory, pass it
explicitly:

```bash
"$ROSEUI_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "log_dir": "/Users/alex/Library/Logs/DoDidDoneUi",
  "lines": 100,
  "errors_only": true,
  "conversation_id": "conv_123"
}
...
```bash
"$ROSEUI_HELPER_BIN" diagnose http get <<'JSON'
{
  "path": "/api/teams",
...
- For configuration changes, switch to `roseui-config`.
- For scheduled task creation or updates, use the `cron` skill or
  `roseui-config` cron commands.
