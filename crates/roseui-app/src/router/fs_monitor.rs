...
//!
//! Adapts the transport-agnostic [`FsMonitorActor`] (in `roseui-project`) to the
...
use roseui_project::monitor::{FsInbound, FsMonitorActor, FsWirePush};
use roseui_realtime::{ConnectionId, MessageRouter, NoopMessageRouter, WebSocketManager};
...
#[cfg(test)]
mod tests {
    use roseui_realtime::{PER_CONNECTION_BUFFER, WsOutbound};
    use serde_json::json;
    use tokio::sync::mpsc;
