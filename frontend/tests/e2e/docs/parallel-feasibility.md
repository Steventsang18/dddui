...
**Critical bottleneck**: `workers: 1` enforced + shared `roseui.db` → parallel execution impossible without refactoring.
...
1. Extend `ROSEUI_E2E_TEST` to include worker ID:
   ```typescript
   ROSEUI_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.ROSEUI_E2E_TEST_WORKER_ID || '0';
   return `RoseUi-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/RoseUi-E2E-Worker-0/config/roseui.db`
   - `~/Library/Application Support/RoseUi-E2E-Worker-1/config/roseui.db`
...
**Complexity**: High (requires main process to read `ROSEUI_DATABASE_PATH`, conflicts with userData convention)
