 ...
 *
 * Covers the four user-visible preview flows that run against rosecore
...
function makeTempFile(ext: string, body: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-preview-e2e-'));
...
function makeExternalWorkspaceFile(ext: string, body: string): { filePath: string; workspace: string } {
  const dir = fs.mkdtempSync(path.join(EXTERNAL_WORKSPACE_ROOT, 'roseui-preview-e2e-'));
  const file = path.join(dir, `sample.${ext}`);
  fs.writeFileSync(file, body);
