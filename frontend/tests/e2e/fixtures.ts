 ...
 *      (e.g. out/linux-unpacked/roseui, out/mac-arm64/RoseUi.app, out/win-unpacked/RoseUi.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-e2e-state-'));
const e2eStateFile = path.join(e2eStateSandboxDir, 'extension-states.json');
// Disposable userData root so RoseCore migrates a fresh DB per run instead of
...
const e2eUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-e2e-userdata-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/RoseUi.exe  or  out/win-x64-unpacked/RoseUi.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'RoseUi.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/RoseUi.app/Contents/MacOS/RoseUi  or  out/mac/RoseUi.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'RoseUi');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/roseui  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['roseui', 'RoseUi']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    ROSEUI_EXTENSIONS_PATH: process.env.ROSEUI_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    ROSEUI_EXTENSION_STATES_FILE: process.env.ROSEUI_EXTENSION_STATES_FILE || e2eStateFile,
    ROSEUI_DISABLE_AUTO_UPDATE: '1',
    ROSEUI_DISABLE_DEVTOOLS: '1',
    ROSEUI_E2E_TEST: '1',
    ROSEUI_E2E_USER_DATA_DIR: process.env.ROSEUI_E2E_USER_DATA_DIR || e2eUserDataDir,
    ROSEUI_CDP_PORT: '0',
  };
