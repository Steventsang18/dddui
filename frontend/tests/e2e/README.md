...
### 2. Ensure `rosecore` is on PATH

The Electron main process spawns the `rosecore` binary during startup and
exposes its port to the renderer via `window.__backendPort`. The binary is
located via `which rosecore`, so it must be reachable from the `PATH`
...
```bash
# Install the backend binary (builds to ~/.cargo/bin/rosecore)
cd ../RoseCore && cargo install --path crates/roseui-app
...
| ---------------------------- | ----- | ------------------------ |
| `ROSEUI_E2E_TEST`            | `1`   | App recognizes test mode |
| `ROSEUI_DISABLE_AUTO_UPDATE` | `1`   | No update checks         |
| `ROSEUI_DISABLE_DEVTOOLS`    | `1`   | No DevTools windows      |
| `ROSEUI_CDP_PORT`            | `0`   | CDP disabled             |
...
```bash
# Clean via database
sqlite3 "~/Library/Application Support/RoseUi-Dev/roseui/roseui.db" \
