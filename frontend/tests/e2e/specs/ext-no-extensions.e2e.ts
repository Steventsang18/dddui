...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      ROSEUI_EXTENSIONS_PATH: emptyExtensionsDir,
      ROSEUI_EXTENSION_STATES_FILE: extensionStatesFile,
