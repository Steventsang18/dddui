...
test('error bubble butler chip pre-fills a diagnosis prompt in the home chat', async ({ page }) => {
  ...
  try {
    ...
    await page.waitForFunction(
      (id) =>
        Boolean(
          (window as unknown as { __ROSEUI_E2E_MESSAGE_STREAM__?: StreamRegistry }).__ROSEUI_E2E_MESSAGE_STREAM__
            ?.controllers[id]
        ),
      conversation.id,
      { timeout: 15_000 }
    ...
    await page.evaluate(
      async ({ id, text }) => {
        const registry = (window as unknown as { __ROSEUI_E2E_MESSAGE_STREAM__?: StreamRegistry })
          .__ROSEUI_E2E_MESSAGE_STREAM__;
