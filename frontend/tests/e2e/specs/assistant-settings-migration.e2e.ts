 ...
 * 1. seed a legacy `roseui.db` using the pre-unification assistants schema
...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    ...
    path.join(projectRoot, '../aionCore/target/debug/rosecore'),
  ...
  throw new Error('rosecore binary not found for migration e2e');
}

function schemaPath(): string {
  return path.join(process.cwd(), '../aionCore/crates/roseui-db/migrations/001_initial_schema.sql');
}

function querySqliteValue(dataDir: string, sql: string): string {
  const dbPath = path.join(dataDir, 'roseui-backend.db');
  return execFileSync('sqlite3', ['-readonly', dbPath, sql], { encoding: 'utf8' }).trim();
}

function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'roseui.db');
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        ROSEUI_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'roseui-assistant-migration-'));
