...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'roseui-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData RoseUi directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'RoseUi.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'RoseUi.exe' }]);
