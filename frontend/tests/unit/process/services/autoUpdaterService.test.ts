...
describe('AutoUpdaterService', () => {
  ...
  beforeEach(() => {
    ...
    appMock.getPath.mockImplementation(() => '/tmp/roseui-test');
    delete (autoUpdaterMock as { updateInfoAndProvider?: unknown }).updateInfoAndProvider;
    appMock.isPackaged = false;
    delete process.env.ROSEUI_FORCE_DEV_AUTO_UPDATE;
    delete process.env.ROSEUI_DEBUG_AUTO_UPDATE_CURRENT_VERSION;
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'RoseUi-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
        path: 'RoseUi-2.1.14-mac-arm64.dmg',
        sha512: 'sha512-value',
  ...
  it('overrides the updater current version only for forced unpacked dev checks', async () => {
    process.env.ROSEUI_FORCE_DEV_AUTO_UPDATE = '1';
    process.env.ROSEUI_DEBUG_AUTO_UPDATE_CURRENT_VERSION = '2.1.12';
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'RoseUi-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'RoseUi-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
      url: new URL('https://static.roseui.com/releases/2.1.14/RoseUi-2.1.14-mac.zip'),
      info: { url: 'RoseUi-2.1.14-mac.zip', sha512: 'sha512-value' },
    };
    const cachedUpdatePath = path.join('/cache/pending', 'RoseUi-2.1.14-mac.zip');
  ...
  it('moves the process cwd to temp before the Windows updater handoff', async () => {
    setPlatform('win32');
    const tempRoot = path.join(process.env.TEMP || process.cwd(), `roseui-updater-cwd-test-${process.pid}`);
    const expectedCwd = path.join(tempRoot, 'roseui-updater-cwd');
    const chdir = vi.spyOn(process, 'chdir').mockImplementation(() => undefined);
    appMock.getPath.mockImplementation((name: string) => (name === 'temp' ? tempRoot : '/tmp/roseui-test'));
