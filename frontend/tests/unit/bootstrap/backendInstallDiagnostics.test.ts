...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ['C:\\RoseUi\\resources', { mtimeMs: 1000, size: 0 }],
      ['C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
          content: JSON.stringify({
            ...
            sourceType: 'download',
            files: ['rosecore.exe', 'managed-resources/'],
          }),
        },
      ],
    ]);
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        resourcesPath: 'C:\\RoseUi\\resources',
        checkedBundledPath: 'C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\rosecore.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
        execPath: 'C:\\RoseUi\\RoseUi.exe',
        isPackaged: true,
        platform: 'win32',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'rosecore.exe',
      binaryPath: 'C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\rosecore.exe',
      bundledDirPath: 'C:\\RoseUi\\resources\\bundled-rosecore',
      execPath: 'C:\\RoseUi\\RoseUi.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['rosecore.exe', 'managed-resources/'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\RoseUi\\resources\\bundled-rosecore\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});
