...
describe('classifyBackendStartupFailure', () => {
  it('classifies missing GLIBC symbols as an incompatible backend runtime', () => {
    ...
    error.details = {
      stage: 'early_exit',
      stderrTail:
        "/opt/RoseUi/resources/bundled-rosecore/linux-x64/rosecore.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.34' not found\n" +
        "/opt/RoseUi/resources/bundled-rosecore/linux-x64/rosecore.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.32' not found",
    };
  ...
  it('classifies missing startup directory preparation as a startup directory failure', () => {
    const error = new Error('rosecore startup directory preparation failed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn',
  ...
  it('does not classify post-resolution binary spawn ENOENT as a startup directory failure', () => {
    const error = new Error('rosecore process emitted an error before startup') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn_error',
      binaryPath: 'D:\\apps\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\rosecore.exe',
      causeMessage: 'spawn D:\\apps\\RoseUi\\resources\\bundled-rosecore\\win32-x64\\rosecore.exe ENOENT',
    };
  ...
  it('classifies recoverable database corruption boundary failures separately from data migration failures', () => {
    ...
    error.details = {
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.recoverable_corruption databasePath=/db/roseui-backend.db: failed to initialize application data',
    };
  ...
  it('classifies packaged app resources missing from installation as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      missingPwaDir: true,
      missingResources: ['bundled-rosecore/', 'bundled-rosecore/win32-x64/'],
  ...
  it('classifies packaged runtime directories without the backend binary as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      missingPwaDir: false,
      missingResources: ['bundled-rosecore/win32-x64/managed-resources/', 'bundled-rosecore/win32-x64/rosecore.exe'],
      missingRuntimeDir: false,
    });
  });

  it('classifies packaged macOS architecture mismatches separately from generic startup failures', () => {
    const error = new Error('RoseUi package architecture does not match this Mac') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
