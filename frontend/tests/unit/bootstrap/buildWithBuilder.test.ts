...
function readInstallerErrorDefinitions(): Array<{ defineName: string; code: string }> {
  ...
  return Array.from(source.matchAll(/!define\s+(ROSEUI_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
}
...
describe('build-with-builder', () => {
  it('rejects skip-vite when renderer output is only a source html shell', () => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'roseui-build-skip-vite-test-'));
...
`,
  ...
  it('releases the NSIS output directory before any update repair or uninstall work', () => {
    ...
    const preInit = script.match(/!macro ROSEUI_INSTALLER_PREINIT([\s\S]*?)!macroend/)?.[1];
    const releaseMacro = script.match(/!macro ROSEUI_RELEASE_INSTALL_DIR_OUTDIR([\s\S]*?)!macroend/)?.[1];
    ...
    expect(preInit).toContain('!insertmacro ROSEUI_RELEASE_INSTALL_DIR_OUTDIR');
    expect(preInit!.indexOf('ROSEUI_RELEASE_INSTALL_DIR_OUTDIR')).toBeLessThan(
      preInit!.indexOf('ROSEUI_SESSION_BEGIN')
    );
  });
  ...
  it('uses install-directory ownership checks in the shared Windows NSIS include', () => {
    ...
    expect(script).not.toContain("Name -ieq '${ROSEUI_APP_EXECUTABLE_FILENAME}'");
  });
  ...
  it('records installer self-lock diagnostics when Restart Manager finds no locking process', () => {
    ...
    const captureMacro = script.match(/!macro ROSEUI_CAPTURE_FAILED_PATH_LOCKERS[\s\S]*?!macroend/)?.[0];

    expect(script).toContain('roseui-query-lockers.ps1');
    expect(captureMacro).toContain('ROSEUI_QUERY_LOCKERS');
    expect(captureMacro).not.toContain('ROSEUI_QUERY_LOCKERS_INLINE_LEGACY');
    ...
    expect(queryScript).toContain('currentOutDir');
    expect(queryScript).toContain("name = 'RoseUi installer'");
  });
  ...
  it('continues with the bundled uninstaller when installed-uninstaller repair remains locked', () => {
    ...
    const retryFailureBranch = script.match(
      /\$\{If\} \$\{Errors\}\s+([\s\S]*?)\$\{Else\}\s+!insertmacro ROSEUI_LOG_UNINSTALLER_REPAIR "after-copy-retry"/
    )?.[1];

    expect(retryFailureBranch).toBeTruthy();
    expect(retryFailureBranch).toContain('copy-failed-using-bundled');
    expect(retryFailureBranch).toContain('$RoseUiBundledUninstaller');
    expect(retryFailureBranch).not.toContain('MessageBox');
    expect(retryFailureBranch).not.toContain('ROSEUI_MSG_UNINSTALLER_LOCKED');
  ...
  it('keeps coded Windows installer failures on the unified reportable failure path', () => {
    ...
    for (const file of files) {
      ...
      source.split(/\r?\n/).forEach((line, index) => {
        if (line.includes('!macro ROSEUI_FAIL ')) {
          offenders.push(`${file}:${index + 1}: defines non-reportable coded failure macro`);
        }
        if (line.includes('!insertmacro ROSEUI_FAIL ')) {
  ...
  it('allows raw Windows installer MessageBox calls only for unified reporting or non-terminal prompts', () => {
    ...
    const allowedMessageBoxes = new Map<string, RegExp[]>([
      ...
      [
        ...
        [/ROSEUI_MSG_FILE_OR_FOLDER_IN_USE_ZH/, /\$\(appRunning\)/, /ROSEUI_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH/],
      ],
    ]);
  ...
  it('routes app-cannot-be-closed cancellation through E1003 instead of quitting silently', () => {
    ...
    const cannotCloseBranch = script.match(
      /ROSEUI_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH[\s\S]*?IDRETRY roseui_wait_for_close([\s\S]*?)\$\{Else\}/
    )?.[1];

    expect(cannotCloseBranch).toBeTruthy();
    expect(cannotCloseBranch).toContain('ROSEUI_E_INSTALL_DIR_REMOVE_OR_LOCKED');
    expect(cannotCloseBranch).toContain('ROSEUI_FAIL_REPORTABLE_BILINGUAL_DIAGNOSTICS');
  ...
  ])('prepares bundled RoseCore for $expectedArch with args $args', ({ args, expectedArch }) => {
    const tempDir = mkdtempSync(join(tmpdir(), 'roseui-build-test-'));
    const hookPath = join(tempDir, 'hook.cjs');
...
function recordPrepareCall(options) {
  const callsPath = process.env.ROSEUI_PREPARE_CALLS_FILE;
  const calls = fs.existsSync(callsPath) ? JSON.parse(fs.readFileSync(callsPath, 'utf8')) : [];
  calls.push(options ?? null);
  fs.writeFileSync(callsPath, JSON.stringify(calls));
  return { prepared: true, dir: 'mock-bundled-rosecore', sourceType: 'mock' };
}
...
Module._load = function patchedLoad(request, parent, isMain) {
  ...
  if (request.endsWith('packages/shared-scripts/src/prepare-rosecore.js')) {
...
`,
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ['scripts/build-with-builder.js', ...args], {
        ...
        env: {
          ...process.env,
          ROSEUI_PREPARE_CALLS_FILE: callsPath,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, `--require=${hookPath}`].filter(Boolean).join(' '),
        },
      });
      ...
      expect(readFileSync(resolve(repoRoot, 'resources/windows/support/_sentry-dsn.generated.nsh'), 'utf8')).toBe(
        '!define ROSEUI_SENTRY_DSN ""\n'
      );

      if (args.includes('--win')) {
        const installUtil = readFileSync(resolveAppBuilderInstallUtil(), 'utf8');
        expect(installUtil).toContain('RoseUi-bundled-uninstaller override source');
        expect(installUtil).toContain('$PLUGINSDIR\\RoseUi-fixed-uninstaller.exe');
        expect(installUtil.match(/RoseUi-bundled-uninstaller override source/g)).toHaveLength(1);
      }
