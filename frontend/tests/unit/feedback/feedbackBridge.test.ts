...
vi.mock('electron', () => ({
  ...
  app: {
    getPath: vi.fn(() => '/tmp/roseui-test-logs-nonexistent'),
...
describe('feedback logs', () => {
  it('collects top-level frontend logs and nested backend logs through the IPC handler', async () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'roseui-feedback-bridge-'));
    try {
  ...
  it('collects the same recent three log days used by user feedback reports', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'roseui-feedback-logs-'));
    try {
      writeFileSync(path.join(logsDir, '2026-05-25.log'), 'today frontend\n');
      writeFileSync(path.join(logsDir, '2026-05-25.rosecore.log'), 'today backend\n');
  ...
  it('collects recent logs from dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'roseui-feedback-dated-logs-'));
    try {
      ...
      writeFileSync(path.join(recentDir, '2026-07-02.rosecore.log'), 'today backend nested\n');
      ...
      expect(content).toContain('2026/07/02/2026-07-02.rosecore.log');
    } finally {
      rmSync(logsDir, { recursive: true, force: true });
    }
  });
