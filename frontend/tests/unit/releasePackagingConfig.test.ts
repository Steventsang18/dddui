...
describe('release packaging configuration', () => {
  ...
  it('uploads mac zip artifacts without a stale Windows zip glob', () => {
    ...
    expect(workflow).toContain('out/RoseUi-*-mac-*.zip');
    expect(workflow).not.toContain('out/RoseUi-*-win32-*.zip');
  });
  ...
  itWithBash('fails release asset preparation when a mac zip is missing', () => {
    const tempDir = mkdtempSync(resolve(tmpdir(), 'roseui-release-assets-'));
    ...
    try {
      ...
      rmSync(resolve(artifactsDir, 'macos-build-arm64', 'RoseUi-1.0.0-mac-arm64.zip'), { force: true });
