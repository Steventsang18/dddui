...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string) => (key === 'cron.status.defaultPrompt' ? 'Create with /cron in RoseUi' : key),
  }),
}));
...
describe('create scheduled task conversation action', () => {
  ...
  it('prefills the current editable conversation without navigating', () => {
    ...
    expect(requestPrefillMock).toHaveBeenCalledWith('current-conversation', 'Create with /cron in RoseUi');
  ...
  it('queues the target-scoped prefill before navigating to a background conversation', () => {
    ...
    expect(requestPrefillMock).toHaveBeenCalledWith('background-conversation', 'Create with /cron in RoseUi');
  ...
  it.each(['openclaw-gateway', 'nanobot', 'remote', 'gemini', 'codex'])(
    ...
    (type) => {
      ...
      expect(navigateMock).toHaveBeenCalledWith('/guid', {
        state: {
          prefillPrompt: 'Create with /cron in RoseUi',
