...
describe('selectRecentLogFiles', () => {
  it('returns every file from the N most recent non-empty days', () => {
    const files = [
      ...
      { path: '/a/2026-05-22.rosecore.log', mtime: Date.UTC(2026, 4, 22, 11), size: 200 },
...
describe('listLogFilesSync', () => {
  it('finds log files under dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'roseui-sentry-logs-'));
    try {
      const datedDir = path.join(logsDir, '2026', '07', '02');
      mkdirSync(datedDir, { recursive: true });
      writeFileSync(path.join(datedDir, '2026-07-02.rosecore.log'), 'backend\n');
      ...
      expect(relative).toEqual([
        '2026-07-02.log',
        '2026/07/02/2026-07-02.rosecore.log',
...
describe('captureBackendStartupFailure', () => {
  it('captures and flushes a dedicated backend startup failure with diagnostics', async () => {
    ...
    const error = new Error('rosecore failed to start within timeout') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'health_timeout',
      binaryPath: '/abs/path/rosecore',
      port: 33334,
  ...
  it('sets flattened incomplete-installation tags for update-related missing directory resources', async () => {
    ...
    try {
      const error = new Error('rosecore startup failed while resolving backend binary') as Error & {
        details?: Record<string, unknown>;
      };
      error.details = {
        ...
        binaryName: 'rosecore.exe',
        resourcesPath: 'C:\\Users\\alice\\AppData\\Local\\Programs\\RoseUi\\resources',
      ...
      expect(scopeSetTag).toHaveBeenCalledWith(
        'roseui.backend_startup.incomplete_installation_kind',
        'missing_directory_resources'
      );
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.missing_bundled_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.missing_runtime_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.missing_binary', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.missing_hub_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.last_update_status', 'quit-and-install');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.seconds_since_quit_and_install', '46');
      expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.install_path_kind', 'user_local_programs');
      expect(scopeSetContext).toHaveBeenCalledWith(
        'rosecore_startup_classification',
        expect.objectContaining({
  ...
  it('sets bucketed health polling tags for backend startup timeouts', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.health_polling_delayed', 'true');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.health_attempts_bucket', '1');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.health_attempt_deficit_bucket', '76-150');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.health_timeout_overrun_bucket', 'over_60s');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.health_max_attempt_gap_bucket', '0ms');
  });

  it('sets backend data migration reason and boundary tags', async () => {
    scopeSetTag.mockClear();
    const error = new Error('rosecore exited before health check passed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'early_exit',
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/roseui-backend.db: failed to initialize application data',
    };

    await captureBackendStartupFailure(error);

    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.reason', 'backend_data_migration_failed');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.boundary_code', 'BOOTSTRAP_DATA_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.boundary_stage', 'database.migration');
  });
  ...
  it('sets local data repair reason and issue-kind tags', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.reason', 'backend_local_data_repair_failed');
    expect(scopeSetTag).toHaveBeenCalledWith(
      'roseui.backend_startup.local_data_issue_kind',
      'agent_metadata_invalid_utf8'
    );
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.boundary_code', 'BOOTSTRAP_SERVICE_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('roseui.backend_startup.boundary_stage', 'services.init');
  });
});
...
describe('initSentry beforeSend', () => {
  ...
  it('drops backend-port secondary errors after backend startup already failed', () => {
    ...
    const event = {
      exception: {
        values: [
          {
            value: '[WebUI] Cannot start: rosecore is not running (globalThis.__backendPort unset)',
          },
        ],
      },
    };
  ...
  it('keeps the primary backend startup failure even when its details contain secondary text', () => {
    ...
    const event = {
      tags: {
        'roseui.failure': 'backend_startup',
      },
      exception: {
        values: [
          {
  ...
  it('keeps user feedback reports even when diagnostics contain backend secondary text', () => {
    ...
    const event = {
      tags: {
        type: 'user-feedback',
        'roseui.installation_integrity.user_report': 'true',
      },
      extra: {
