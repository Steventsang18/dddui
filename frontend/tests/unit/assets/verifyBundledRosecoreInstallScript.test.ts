...
const scriptPath = 'resources/windows/support/verify-bundled-rosecore-install.ps1';
...
describe('Windows bundled rosecore install verifier', () => {
  ...
  runOnWindows('fails an old-version-only Codex CLI install directory', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'roseui-install-verify-'));
    const installDir = join(tmp, 'install');
    const managedRoot = join(installDir, 'resources', 'bundled-rosecore', 'win32-x64', 'managed-resources');
    const logPath = join(tmp, 'verify.log');
    const codexTriple = 'x86_64-pc-windows-msvc';

    try {
      writeFile(join(installDir, 'resources', 'bundled-rosecore', 'win32-x64', 'rosecore.exe'), 'x');
      writeJson(join(installDir, 'resources', 'bundled-rosecore', 'win32-x64', 'manifest.json'), {
        platform: 'win32',
        arch: 'x64',
      });
