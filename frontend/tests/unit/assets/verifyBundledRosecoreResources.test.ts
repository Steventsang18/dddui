...
} = require('../../../packages/shared-scripts/src/verify-bundled-rosecore-resources');
...
function seedRuntimeKey(
  ...
  const managedResourcesDir = join(resourcesDir, 'bundled-rosecore', runtimeKey, 'managed-resources');
  mkdirSync(join(resourcesDir, 'bundled-rosecore', runtimeKey), { recursive: true });
  writeFile(join(resourcesDir, 'bundled-rosecore', runtimeKey, platform === 'win32' ? 'rosecore.exe' : 'rosecore'));
  writeJson(join(resourcesDir, 'bundled-rosecore', runtimeKey, 'manifest.json'), { platform, arch });
...
describe('verifyBundledAioncoreResources', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'roseui-bundled-resources-'));
  ...
  it('fails when managed resources contract is missing', () => {
    ...
    expect(result.missing).toContain('bundled-rosecore/win32-x64/managed-resources/manifest.json');
  ...
  it('reports bundle manifest platform and architecture mismatches', () => {
    writeJson(join(resourcesDir, 'bundled-rosecore', 'win32-x64', 'manifest.json'), {
      platform: 'darwin',
      arch: 'arm64',
    });
    ...
    expect(result.missing).toContain('bundled-rosecore/win32-x64/manifest.json<platform:win32>');
    expect(result.missing).toContain('bundled-rosecore/win32-x64/manifest.json<arch:x64>');
    expect(result.failures).toContainEqual(
      expect.objectContaining({
  ...
  it('passes with the Windows arm64 CLI layout', () => {
    ...
    expect(result.checked).toContain(
      'bundled-rosecore/win32-arm64/managed-resources/cli/codex/0.144.6/win32-arm64/vendor/aarch64-pc-windows-msvc/bin/codex.exe'
    );
  });
  ...
  it('passes for non-Windows node runtime layout', () => {
    ...
    expect(result.failures).toEqual([]);
    expect(result.checked).toContain(
      'bundled-rosecore/darwin-arm64/managed-resources/node/node-v24.11.0-darwin-arm64/bin/node'
    );
    expect(result.checked).toContain(
      'bundled-rosecore/darwin-arm64/managed-resources/cli/claude/2.1.215/darwin-arm64/claude'
    );
  });
  ...
  it('reports missing non-Windows managed node runtime executable', () => {
    ...
    expect(result.missing).toContain(
      'bundled-rosecore/linux-x64/managed-resources/node/node-v24.11.0-linux-x64/bin/node'
    );
    expect(result.failures).toContainEqual(
      expect.objectContaining({
  ...
  it('fails when the pinned codex version directory is absent', () => {
    ...
    expect(result.missing).toContain(
      'bundled-rosecore/win32-x64/managed-resources/cli/codex/0.144.6/win32-x64/vendor/x86_64-pc-windows-msvc/bin/codex.exe'
    );
  });
  ...
  it('fails when contract node root points to the required version but only a wrong node directory exists', () => {
    ...
    expect(result.missing).toContain(
      'bundled-rosecore/win32-x64/managed-resources/node/node-v24.11.0-win-x64/node.exe'
    );
  });
  ...
  it('ignores unknown contract fields but rejects duplicate cli names', () => {
    ...
    expect(result.missing).toContain('bundled-rosecore/win32-x64/managed-resources/manifest.json<contract_failure>');
  });
