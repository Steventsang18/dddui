...
describe('resolveImageGenerationMcpEnv', () => {
  ...
  it('matches legacy env by platform, base URL, and model when provider id is absent', () => {
    const result = resolveImageGenerationMcpEnv(undefined, [geminiProvider], {
      ROSEUI_IMG_PLATFORM: 'gemini',
      ROSEUI_IMG_BASE_URL: 'https://generativelanguage.googleapis.com/',
      ROSEUI_IMG_MODEL: 'gemini-3-pro-image-preview',
      ROSEUI_IMG_API_KEY: 'stale-key',
    });

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.source).toBe('field-match');
    expect(result.env.ROSEUI_IMG_PROVIDER_ID).toBe('03c8482c');
    expect(result.env.ROSEUI_IMG_API_KEY).toBe('provider-key');
  });
