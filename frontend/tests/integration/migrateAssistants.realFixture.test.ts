 ...
 * rosecore binary using the user-provided fixtures
 * (`/Users/zhoukai/Downloads/roseui-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `roseui-config.txt` exactly as ConfigStorage would.
 *   2. Stage `Archive/*.md` as `<userData>/config/assistants/<id>.<locale>.md`.
 *   3. Spawn a real rosecore bound to a throw-away data-dir.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/roseui-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.ROSEUI_BACKEND_BINARY,
    path.join(os.homedir(), '.cargo', 'bin', 'rosecore'),
    path.resolve(__dirname, '../../../RoseCore/target/debug/rosecore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('rosecore binary not found (set ROSEUI_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `roseui-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'roseui-migrate-fixture-'));
