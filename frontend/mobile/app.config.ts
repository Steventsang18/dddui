...
export default ({ config }: ConfigContext): ExpoConfig => {
  return {
    ...
    scheme: 'roseui-mobile',
    userInterfaceStyle: 'automatic',
    ios: {
      supportsTablet: true,
      bundleIdentifier: 'ai.resopod.roseui',
      buildNumber: String(VERSION.buildNumber),
      infoPlist: {
        ITSAppUsesNonExemptEncryption: false,
        NSCameraUsageDescription: 'RoseUi needs camera access to scan QR codes for server login.',
      },
    },
    android: {
      adaptiveIcon: {
