...
export INSTALL_DIR="/tmp/roseui-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/roseui-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/roseui-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/roseui-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/roseui-web"
    exit 1
fi
...
VERSION_OUTPUT=$(roseui-web version 2>&1 || echo "")
