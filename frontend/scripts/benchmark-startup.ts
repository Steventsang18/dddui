 ...
 * parsing the electron-log file for [RoseUi:ready] / [RoseUi:init] /
 * [RoseUi:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [RoseUi:ready] marks
  ...
  // Parsed from [RoseUi:init] marks
  initTotalMs: number;
  // Parsed from [RoseUi:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'RoseUi-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'RoseUi', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'RoseUi', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'RoseUi', 'logs', `${today}.log`));
  }
...
// Matches: [RoseUi:ready] <label> +<ms>ms
// Matches: [RoseUi:init]  <label> +<ms>ms
// Matches: [RoseUi:process] <label> +<ms>ms
const MARK_REGEX = /\[RoseUi:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[RoseUi] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[RoseUi] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[RoseUi] Showing main window')) marks.logs.showingMainWindow = true;
  }
