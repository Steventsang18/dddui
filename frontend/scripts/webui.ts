 ...
 *
 * Env vars:
 *   ROSEUI_PORT           : static server port (default 33000)
 *   ROSEUI_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   ROSEUI_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   ROSEUI_DATA_DIR       : override userData path (default Electron-compatible)
 *   ROSEUI_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   ROSEUI_STATIC_DIR     : override static dir (default out/renderer)
 *   ROSEUI_BACKEND_BIN    : absolute path to rosecore binary (else PATH lookup)
 *   ROSEUI_BACKEND_BUNDLED_DIR : dir containing bundled-rosecore/<plat-arch>/binary
 *   ROSEUI_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
...
import { startWebHost } from '@roseui/web-host';
...
const DEFAULT_PORT = (() => {
  ...
  if (process.env.ROSEUI_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
const BACKEND_BINARY = process.platform === 'win32' ? 'rosecore.exe' : 'rosecore';
...
/**
 * Resolve the directory where rosecore persists its SQLite DB.
 *
 * `bun run webui` runs **independently of the Electron desktop app** — it must
 * work on hosts that never installed RoseUi.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $ROSEUI_DATA_DIR        env override (same effect)
 *   otherwise               ~/.roseui-web         (production)
 *                           ~/.roseui-web-dev     (dev, default)
 *                           ~/.roseui-web-dev-2   (dev + ROSEUI_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.roseui[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.roseui-dev` as a **symlink** to
 * `~/Library/Application Support/RoseUi-Dev/roseui` so CLI tools (claude,
 ...
 * `.roseui-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.roseui-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.ROSEUI_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.ROSEUI_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.roseui-web${suffix}`);
...
function resolvePort(): number {
  ...
  const env = process.env.ROSEUI_PORT ?? process.env.PORT;
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.ROSEUI_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.ROSEUI_ALLOW_REMOTE ?? process.env.ROSEUI_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.ROSEUI_STATIC_DIR) return process.env.ROSEUI_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set ROSEUI_STATIC_DIR.`);
}
 ...
 *   $ROSEUI_NO_BUILD=1        : env-level opt-out
 *   $ROSEUI_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.ROSEUI_NO_BUILD)) return;
  if (process.env.ROSEUI_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  if (process.env.ROSEUI_BACKEND_BIN) return process.env.ROSEUI_BACKEND_BIN;

  const bundledBase = process.env.ROSEUI_BACKEND_BUNDLED_DIR ?? path.join(repoRoot, 'resources', 'bundled-rosecore');
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set ROSEUI_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without RoseUi.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.ROSEUI_LOG_DIR ?? path.join(workDir, 'logs');
  ...
  console.log('');
  console.log('RoseUi WebUI is ready');
