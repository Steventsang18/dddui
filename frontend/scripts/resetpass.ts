 ...
 *   2. No webui running → spawn a short-lived rosecore against the same
 ...
 *   ROSEUI_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@roseui/web-host';
 ...
 * See the comment there for why the default is `~/.roseui-web*` (not `~/.roseui*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `ROSEUI_PORT` to match
 ...
 * pass --port / ROSEUI_PORT if they launched webui on a non-default one.
 */
