...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('RoseUi.exe');
    await killProcessByName('RoseUi');
  ...
  const env = {
    ...process.env,
    ROSEUI_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] ROSEUI_EXTENSIONS_PATH: ${env.ROSEUI_EXTENSIONS_PATH}`);
