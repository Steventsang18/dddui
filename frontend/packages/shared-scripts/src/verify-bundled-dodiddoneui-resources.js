...
function bundledPath(runtimeKey, ...parts) {
  return normalize(path.join('bundled-dodiddoneui', runtimeKey, ...parts));
}
...
function requireRelativePath(baseDir, runtimeKey, parts, checked, missing, failures) {
  ...
  if (!isFile(path.join(baseDir, ...parts))) {
    const failure = { component: 'dodiddoneui', reason: 'missing_file', path: relativePath };
    failures.push(failure);
...
function verifyBundledDodiddoneuiResources({ resourcesDir, electronPlatformName, targetArch }) {
  const runtimeKey = `${electronPlatformName}-${targetArch}`;
  const baseDir = path.join(resourcesDir, 'bundled-dodiddoneui', runtimeKey);
  const checked = [];
  const missing = [];
  const failures = [];
...
module.exports = {
  verifyBundledDodiddoneuiResources,
};
