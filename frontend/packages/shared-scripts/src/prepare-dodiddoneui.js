/**
 * Prepare dodiddoneui binary for packaging.
 *
 * Resolution order:
 *  1. GitHub Actions artifact download when ROSEUI_BACKEND_RUN_ID is set
 *  2. GitHub release download (requires version or defaults to "latest")
 *  3. Complete local bundle from ROSEUI_BACKEND_LOCAL_BUNDLE_DIR
 *  4. Local binary fallback from ROSEUI_BACKEND_LOCAL_BINARY
 *
 * Output: {projectRoot}/resources/bundled-dodiddoneui/{platform}-{arch}/
 *   - dodiddoneui[.exe]
 *   - manifest.json
 *   - managed-resources/...
 *
 * @module prepare-dodiddoneui
 */
...
const { verifyBundledDodiddoneuiResources } = require('./verify-bundled-dodiddoneui-resources');

const GITHUB_OWNER = 'DoDidDoneUi' // TODO: replace with the real GitHub org before publishing releases;
const GITHUB_REPO = 'dodiddoneui' // TODO: replace with the real GitHub repo before publishing releases;

const ACTIONS_ARTIFACT_TARGETS = {
  'darwin-arm64': {
    artifactName: 'dodiddoneui-manual-macos-arm64',
    manualPlatform: 'macos-arm64',
  },
  'darwin-x64': {
    artifactName: 'dodiddoneui-manual-macos-x64',
    manualPlatform: 'macos-x64',
  },
  'linux-arm64': {
    artifactName: 'dodiddoneui-manual-linux-arm64',
    manualPlatform: 'linux-arm64',
  },
  'linux-x64': {
    artifactName: 'dodiddoneui-manual-linux-x64',
    manualPlatform: 'linux-x64',
  },
  'win32-arm64': {
    artifactName: 'dodiddoneui-manual-windows-arm64',
    manualPlatform: 'windows-arm64',
  },
  'win32-x64': {
    artifactName: 'dodiddoneui-manual-windows-x64',
...
function getBinaryName(platform) {
  return platform === 'win32' ? 'dodiddoneui.exe' : 'dodiddoneui';
}
...
function getActionsArtifactMissingMessage({ runId, platform, arch, expectedArtifactName, availableArtifactNames }) {
  const available =
    Array.isArray(availableArtifactNames) && availableArtifactNames.length > 0
      ...
      : '(none)';
  return [
    `Dodiddoneui run ${runId} does not contain artifact [ ${expectedArtifactName} ] required for [ ${platform}-${arch} ].`,
    `Available artifacts: ${available}.`,
    `Re-run Dodiddoneui Manual Build with platform [ ${getActionsManualPlatform(platform, arch)} ] or all.`,
  ].join(' ');
}
...
function prepareManagedResources(binaryPath, targetDir) {
  ...
  execFileSync(binaryPath, ['--data-dir', dataDir, 'prepare-managed-resources', '--bundle-out', bundleOut], {
    ...
    env: {
      ...process.env,
      ROSEUI_BUNDLED_MANAGED_RESOURCES: '',
    },
  });
...
function verifyPreparedDodiddoneuiBundle(projectRoot, platform, arch) {
  const result = verifyBundledDodiddoneuiResources({
  ...
  if (result.missing.length > 0 || result.failures.length > 0) {
    ...
    throw new Error(`Prepared dodiddoneui bundle is missing required bundled resource(s): ${summary}`);
  }
  return result;
}
 ...
 *   dodiddoneui-v0.1.0-aarch64-apple-darwin.tar.gz
 */
function getAssetName(platform, arch, tag) {
  ...
  return `dodiddoneui-${tag}-${normalizedArch}-${normalizedPlatform}${ext}`;
}
...
function downloadFile(url, outputPath) {
  console.log(`  Downloading dodiddoneui from ${url}`);
...
function findDodiddoneuiArchiveInDir(dir) {
  const entries = fs.readdirSync(dir, { withFileTypes: true });
  for (const entry of entries) {
    const fullPath = path.join(dir, entry.name);
    if (
      entry.isFile() &&
      entry.name.startsWith('dodiddoneui-') &&
      (entry.name.endsWith('.zip') || entry.name.endsWith('.tar.gz'))
    ) {
      return fullPath;
    }
    if (entry.isDirectory()) {
      const found = findDodiddoneuiArchiveInDir(fullPath);
      if (found) return found;
    }
  }
  return null;
...
function downloadAndExtractActionsArtifact(platform, arch, runId) {
  ...
  if (!expectedArtifactName) {
    throw new Error(`Unsupported Dodiddoneui Actions artifact target: ${platform}-${arch}`);
  }
  ...
  const tempDir = path.join(os.tmpdir(), 'dodiddoneui-prepare-actions', runId, `${platform}-${arch}`);
  ...
  console.log(`  Downloading dodiddoneui from Dodiddoneui run ${runId} artifact ${expectedArtifactName}`);
  downloadFileWithAuth(downloadUrl, artifactZipPath);
  extractArchive(artifactZipPath, artifactExtractDir, platform);

  const archivePath = findDodiddoneuiArchiveInDir(artifactExtractDir);
  if (!archivePath) {
    throw new Error(`Dodiddoneui artifact ${expectedArtifactName} from run ${runId} does not contain an dodiddoneui archive`);
  }
  ...
  if (!binaryPath) {
    throw new Error(`Binary ${binaryName} not found in Dodiddoneui artifact ${expectedArtifactName} from run ${runId}`);
  }
...
function downloadAndExtract(platform, arch, tag) {
  ...
  if (!assetName) {
    throw new Error(`Unsupported dodiddoneui target: ${platform}-${arch}`);
  }

  const url = getDownloadUrl(assetName, tag);
  const tempDir = path.join(os.tmpdir(), 'dodiddoneui-prepare', tag, `${platform}-${arch}`);
 ...
 */
function prepareDodiddoneui(options) {
  const { projectRoot, platform, arch, version = 'latest' } = options;
  const runtimeKey = `${platform}-${arch}`;
  const actionsRunId = (process.env.ROSEUI_BACKEND_RUN_ID || '').trim();
  ...
  if (!actionsRunId) {
    ...
    if (version === 'latest') {
      ...
      if (!resolved) {
        throw new Error('Failed to resolve latest dodiddoneui release tag from GitHub API');
      }
      tag = resolved;
      console.log(`Resolved dodiddoneui "latest" → ${tag}`);
    } else {
      tag = version.startsWith('v') ? version : `v${version}`;
    }
  }

  const targetDir = path.join(projectRoot, 'resources', 'bundled-dodiddoneui', runtimeKey);
  const binaryName = getBinaryName(platform);
  const targetBinaryPath = path.join(targetDir, binaryName);

  console.log(
    `Preparing dodiddoneui for ${runtimeKey} (${actionsRunId ? `actions run: ${actionsRunId}` : `version: ${tag}`})`
  );

  removeDirectorySafe(targetDir);
  ensureDirectory(targetDir);

  const localBundleDir = (process.env.ROSEUI_BACKEND_LOCAL_BUNDLE_DIR || '').trim();
  if (localBundleDir) {
    ...
    if (
      ...
      verifyPreparedDodiddoneuiBundle(projectRoot, platform, arch);
      console.log(`  Using local dodiddoneui bundle: ${resolvedLocalBundleDir}`);
      return { prepared: true, dir: targetDir, sourceType: 'local-bundle' };
    }
    console.warn(`  Local dodiddoneui bundle is incomplete or missing: ${resolvedLocalBundleDir}`);
  }
  ...
  if (!sourcePath) {
    const localBinary = (process.env.ROSEUI_BACKEND_LOCAL_BINARY || '').trim();
    if (localBinary) {
      ...
      if (fs.existsSync(resolvedLocalBinary) && fs.statSync(resolvedLocalBinary).isFile()) {
        ...
        console.log(`  Using local dodiddoneui binary: ${resolvedLocalBinary}`);
      } else {
        console.warn(`  Local dodiddoneui binary not found: ${resolvedLocalBinary}`);
      }
    }
  }
  ...
  if (sourcePath) {
    ...
    // The release tag is the authoritative version — the dodiddoneui
    ...
    verifyPreparedDodiddoneuiBundle(projectRoot, platform, arch);
    console.log(
      `  Bundled dodiddoneui prepared: resources/bundled-dodiddoneui/${runtimeKey}/${binaryName} [source=${sourceType}]`
    );
  ...
  throw new Error(`dodiddoneui binary not found for ${runtimeKey} (tag: ${tag})`);
}
...
module.exports = {
  ...
  prepareDodiddoneui,
  verifyPreparedDodiddoneuiBundle,
};
