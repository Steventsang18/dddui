...
/** Backend manifest id of the built-in DoDidDoneUi Butler assistant. */
const BUTLER_ASSISTANT_ID = 'roseui-assistant';
 ...
 * select the DoDidDoneUi Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the DoDidDoneUi Butler for you' })
            );
          }
        }
      } catch (error) {
