...
import { ROSEUI_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理 RoseUi 时间戳后缀，返回原始文件名
export function cleanRoseUiTimestamp(file_name: string): string {
  return file_name.replace(ROSEUI_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanRoseUiTimestamp(file_name);
}
...
class FileServiceClass {
   ...
   * directory (`temp_dir/roseui/...`). Passing the raw device path (the old
