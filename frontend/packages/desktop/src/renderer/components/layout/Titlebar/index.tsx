...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const appTitle = useMemo(() => 'DoDidDoneUi', []);
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('roseui:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('roseui:last-non-settings-path');
      if (stored) {
