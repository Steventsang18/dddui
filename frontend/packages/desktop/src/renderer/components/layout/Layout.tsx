...
const Layout: React.FC<{
  ...
  // The "DoDidDoneUi" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('roseui:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('roseui-open-update-modal', { detail: { source: 'tray' } }));
    };
          ...
          <ArcoLayout className={'size-full layout flex-1 min-h-0'}>
            ...
            >
              ...
              >
                <div
                  className={classNames(
                    'shrink-0 size-36px relative rd-2px bg-white flex items-center justify-center p-3px shadow-sm',
                    {
                      '!size-28px !p-2px': collapsed,
                    }
                  )}
                  onClick={onClick}
                >
                ...
                {isSettingsRoute ? (
                  <Tooltip content={t('common.back', { defaultValue: 'Back to Chat' })} position='bottom'>
                    ...
                    >
                      <div className='text-16px text-t-primary font-semibold leading-tight'>DDDUi</div>
                      <div className='text-10px text-t-tertiary leading-tight'>DoDidDoneUi</div>
                    </div>
                  </Tooltip>
                ) : (
