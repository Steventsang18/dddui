...
const ToolsModalContent: React.FC = () => {
  ...
  return (
    <div className='flex flex-col h-full w-full'>
      ...
      {/* Content Area */}
      <RoseScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
                ...
                {imageGenerationModelList.length > 0 ? (
                  ...
                  >
                    {imageGenerationModelList.map(({ models, ...platform }) => (
                      <RoseSelect.OptGroup label={platform.name} key={platform.id}>
                        {models.map((modelName) => (
                          <RoseSelect.Option key={platform.id + modelName} value={platform.id + '|' + modelName}>
                            {modelName}
                          </RoseSelect.Option>
                        ))}
                      </RoseSelect.OptGroup>
                    ...
                    <Tooltip
                      content={
                        <div>
                          ...
                          <a
                            href='https://github.com/iOfficeAI/RoseUi/wiki/RoseUi-Image-Generation-Tool-Model-Configuration-Guide'
                            target='_blank'
