/**
 * @license
 * Copyright 2026 DoDidDoneUi (dodiddoneui.com)
...
export const UPDATE_AVAILABLE_EVENT = 'roseui-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/iOfficeAI/RoseUi/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('roseui-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('roseui-open-update-modal', handleWindowOpen);
