...
/** RoseModal 组件 Props */
export interface RoseModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'roseui-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'roseui-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <RoseModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </RoseModal>
 *
 * // 预设尺寸 / Preset size
 * <RoseModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </RoseModal>
 *
...
const RoseModal: React.FC<RoseModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'roseui-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('roseui-modal', isStandard && 'roseui-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('roseui-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'roseui-modal-body-content',
...
RoseModal.displayName = 'RoseModal';
