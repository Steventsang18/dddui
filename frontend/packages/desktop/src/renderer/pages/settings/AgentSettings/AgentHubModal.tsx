...
// Upstream marketplace repository; display name rebranded to RoseHub.
const ROSE_HUB_REPO_URL = 'https://github.com/iOfficeAI/AionHub';
...
export const AgentHubModal: React.FC<AgentHubModalProps> = ({ visible, onCancel }) => {
  ...
  const openRoseHubRepo = () => {
    void openExternalUrl(ROSE_HUB_REPO_URL).catch(console.error);
  };
    ...
    >
      <div>
        <div className='mb-12px flex flex-wrap items-center justify-start gap-x-6px gap-y-2px text-left'>
          ...
          <Link className='text-12px leading-18px' onClick={openRoseHubRepo}>
            {t('settings.agentManagement.marketContributionAction', {
              defaultValue: 'Open a PR on RoseHub',
            })}
          </Link>
        </div>
