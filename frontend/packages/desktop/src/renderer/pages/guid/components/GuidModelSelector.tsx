  ...
  if (isGeminiMode) {
    ...
    const addModelItem = (
      ...
      >
        ...
        {t('settings.goConfigureModel')}
      </Menu.Item>
    );

    // 无可用模型时的友好引导：说明应配置国产大模型，并提供直达设置入口。
    const noModelsGuide = (
      <Menu.Item
        key='no-models-guide'
        className='px-12px py-10px max-w-240px text-t-secondary text-12px leading-relaxed'
        disabled
      >
        {t('settings.noAvailableModelsGuide')}
      </Menu.Item>
    );
