...
const ENABLED_CONVERSATION_KEY = 'roseui:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __ROSEUI_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__ROSEUI_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__ROSEUI_E2E_MESSAGE_STREAM__) {
        delete window.__ROSEUI_E2E_MESSAGE_STREAM__.controllers[conversationId];
      }
    };
