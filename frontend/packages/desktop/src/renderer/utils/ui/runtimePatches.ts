...
declare global {
  interface Window {
    __RoseSafeResizeObserver__?: boolean;
    __RoseResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__RoseSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__RoseSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__RoseResizeObserverPatched__) {
    ...
    window.__RoseResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__RoseResizeObserverPatched__) {
    ...
    console.__RoseResizeObserverPatched__ = true;
  }
};
