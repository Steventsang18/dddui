 ...
 * calls routed to dodiddoneui. Electron-native operations (window controls,
...
/**
 * Web-build-safe stand-in for Electron's `OpenDialogOptions` subset.
 * Electron was removed during the pure-Web migration, so we must not
 * `import type { ... } from 'electron'` (the type package no longer
 * exists and breaks the Vite/tsc build). Only the two fields actually
 * consumed by `ShowOpenOptions` are modeled.
 */
...
export const application = {
  ...
  // The backend reads ROSEUI_*_DIR env vars on boot, so it does not own this config.
...
export type ShowOpenOptions =
  | { defaultPath?: string; properties?: ElectronOpenDialogOptions['properties']; filters?: ElectronOpenDialogOptions['filters'] }
  | undefined;
...
export const wiki = {
  health: httpGet<{ status: string; module: string }, void>('/api/wiki/health'),
  listPages: httpGet<WikiPage[], { limit?: number; offset?: number }>(
    (p) => `/api/wiki/pages?limit=${p.limit ?? 50}&offset=${p.offset ?? 0}`
  ),
  createPage: httpPost<WikiPage, CreateWikiPageRequest>('/api/wiki/pages'),
  getPage: httpGet<WikiPage, { page_id: string }>((p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}`),
  updatePage: httpPut<WikiPage, { page_id: string; updates: UpdateWikiPageRequest }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}`,
    (p) => p.updates
  ),
  deletePage: httpDelete<void, { page_id: string }>((p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}`),
  setTags: httpPut<void, { page_id: string; tags: string[] }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/tags`,
    (p) => p.tags
  ),
  getBacklinks: httpGet<WikiPage[], { page_id: string }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/backlinks`
  ),
  search: httpGet<WikiSearchResponse, { q: string; limit?: number; category?: string; doc_type?: string }>(
    (p) =>
      `/api/wiki/search?q=${encodeURIComponent(p.q)}` +
      `${p.limit ? `&limit=${p.limit}` : ''}` +
      `${p.category ? `&category=${encodeURIComponent(p.category)}` : ''}` +
      `${p.doc_type ? `&doc_type=${encodeURIComponent(p.doc_type)}` : ''}`
  ),
  listTags: httpGet<WikiTag[], void>('/api/wiki/tags'),
  // Phase 2: typed edges + link graph
  listEdges: httpGet<WikiEdge[], { page_id: string }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/edges`
  ),
  putEdge: httpPut<void, { page_id: string; to_page_id: string; edge_type: WikiEdgeType }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/edges`,
    (p) => ({ to_page_id: p.to_page_id, edge_type: p.edge_type })
  ),
  deleteEdge: httpDelete<void, { page_id: string; to_page_id: string; edge_type: WikiEdgeType }>(
    (p) =>
      `/api/wiki/pages/${encodeURIComponent(p.page_id)}/edges?to_page_id=${encodeURIComponent(
        p.to_page_id
      )}&edge_type=${encodeURIComponent(p.edge_type)}`
  ),
  getLinkGraph: httpGet<WikiGraphNode[], { page_id: string }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/graph`
  ),
  cite: httpGet<string, { page_id: string; anchor?: string; style?: string }>(
    (p) =>
      `/api/wiki/pages/${encodeURIComponent(p.page_id)}/cite` +
      `${p.anchor ? `?anchor=${encodeURIComponent(p.anchor)}` : ''}` +
      `${p.style ? `${p.anchor ? '&' : '?'}style=${encodeURIComponent(p.style)}` : ''}`
  ),
  // Phase 3: rename safety + Obsidian-style usability
  unlinked: httpGet<string[], { page_id: string }>(
    (p) => `/api/wiki/pages/${encodeURIComponent(p.page_id)}/unlinked`
  ),
  titleSearch: httpGet<string[], { q: string; limit?: number }>(
    (p) => `/api/wiki/title-search?q=${encodeURIComponent(p.q)}${p.limit ? `&limit=${p.limit}` : ''}`
  ),
  listTemplates: httpGet<{ templates: string[] }, void>('/api/wiki/templates'),
  initTemplate: httpPost<number, { template: string }>('/api/wiki/init-template'),
  // --- Raw-source ingest (read-only wiki/raw/ folder) -------------------
  listRaw: httpGet<RawEntryView[], void>('/api/wiki/raw'),
  ingestRaw: httpPost<IngestResult, { raw_path: string }>('/api/wiki/ingest'),
  deleteRaw: httpDelete<void, { raw_path: string }>(
    (p) => `/api/wiki/raw/${encodeURIComponent(p.raw_path)}`
  ),
  // Upload a dropped document into the read-only raw folder (multipart).
  uploadRawFile: {
    provider: () => ({}),
    invoke: async (file: File, relativePath?: string) => {
      ...
      return (await resp.json()) as { success: boolean; data: RawEntryView };
    },
  },
};
