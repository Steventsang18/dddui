---
name: bump-version
description: Use when bumping the RoseUi version: query RoseCore release, verify artifacts, update package.json, generate CHANGELOG, branch, commit, push, create PR, auto-merge, tag release.
---

# Bump Version

Automate the RoseUi release preparation: query RoseCore release → verify artifacts → update versions → generate CHANGELOG → branch → PR → tag.

**Usage:** `/bump-version [version] [flags]`

- `/bump-version` — auto patch + latest RoseCore
- `/bump-version 2.2.0` — explicit RoseUi version + latest RoseCore
- `/bump-version 2.2.0 --core v0.1.12` — explicit both versions
- `/bump-version --skip-core` — pure frontend release (don't touch rosecoreVersion)
...
### Step 3: Determine RoseUi Target Version
...
Display: "Bumping RoseUi: {current} → {target}"

### Step 4: Query RoseCore Latest Release

**Skip entirely if `--skip-core` is set.**

```bash
gh release view --repo iOfficeAI/RoseCore --json tagName,body
```

- If `--core <version>` provided → use that tag instead of latest
- Display the RoseCore version and ask user to confirm before continuing
- Also read current `rosecoreVersion` from `package.json` — if it already matches the queried version, warn the user and ask whether to proceed or use `--skip-core`

### Step 5: Verify RoseCore Artifacts

**Skip if `--skip-core`.**

```bash
gh release view <tag> --repo iOfficeAI/RoseCore --json assets --jq '.assets[].name'
```

Verify all 7 expected assets exist:

- `rosecore-<tag>-x86_64-unknown-linux-gnu.tar.gz`
- `rosecore-<tag>-aarch64-unknown-linux-gnu.tar.gz`
- `rosecore-<tag>-x86_64-apple-darwin.tar.gz`
- `rosecore-<tag>-aarch64-apple-darwin.tar.gz`
- `rosecore-<tag>-x86_64-pc-windows-msvc.zip`
- `rosecore-<tag>-aarch64-pc-windows-msvc.zip`
- `rosecore-checksums.txt`

Missing → Stop: "RoseCore {tag} is missing artifacts: {list}. Wait for CI to complete or check for build failures."
...
- `"rosecoreVersion": "{old}"` → `"rosecoreVersion": "{new core tag}"` (skip if `--skip-core`)
...
## [{target}](https://github.com/iOfficeAI/RoseUi/compare/v{previous}...v{target}) ({date YYYY-MM-DD})
...
### Core ([{core tag}](https://github.com/iOfficeAI/RoseCore/releases/tag/{core tag}))
...
git commit -m "chore: bump version to {target} and rosecore to {core tag}"
...
```
 ...
 2. git pull --rebase
 3. Determine RoseUi target version (patch+1 or explicit)
 4. Query RoseCore latest release (or --core / --skip-core)
 5. Verify RoseCore artifacts (7 files)
 6. Edit package.json (version + rosecoreVersion)
 7. Generate CHANGELOG entry (frontend commits + RoseCore release body)
 8. lint + format + tsc
 9. vitest run
