# RoseUi WebUI Mode - Startup Guide

RoseUi supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts RoseUi with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use RoseUi from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\RoseUi\RoseUi.exe" --webui

# Or if RoseUi is in your PATH
RoseUi.exe --webui
```
...
```bash
# Using full path
/Applications/RoseUi.app/Contents/MacOS/RoseUi --webui

# Or using open command
open -a RoseUi --args --webui
```

### Method 2: Create Shell Script

Create `start-roseui-webui.sh`:
...
```bash
chmod +x start-roseui-webui.sh
./start-roseui-webui.sh
```
...
chmod +x RoseUi-*.AppImage

# Run with --webui flag
./RoseUi-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/roseui-webui.desktop`:

```ini
[Desktop Entry]
Name=RoseUi WebUI
Comment=Start RoseUi in WebUI mode
Exec=/opt/RoseUi/roseui --webui
Icon=roseui
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/roseui-webui.desktop
```
...
Create `~/bin/start-roseui-webui.sh`:
...
```bash
chmod +x ~/bin/start-roseui-webui.sh
```
...
Create `/etc/systemd/system/roseui-webui.service`:

```ini
[Unit]
Description=RoseUi WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/RoseUi/roseui --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable roseui-webui.service
sudo systemctl start roseui-webui.service

# Check status
sudo systemctl status roseui-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run RoseUi in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making RoseUi accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running RoseUi WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/RoseUi/issues/217)
...
#### 4. Download and Install RoseUi

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/RoseUi/releases
wget https://github.com/iOfficeAI/RoseUi/releases/download/vVERSION/RoseUi_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/RoseUi/releases/download/vVERSION/RoseUi_VERSION_arm64.deb

# Install the package
apt install -y ./RoseUi_*.deb

# Verify installation
which RoseUi
```

#### 5. Launch RoseUi WebUI

```bash
# Start RoseUi in WebUI mode with no-sandbox flag
RoseUi --no-sandbox --webui
```
...
To access RoseUi from other devices on your local network:

```bash
# Start with --remote flag
RoseUi --no-sandbox --webui --remote
...
RoseUi --no-sandbox --webui --port 8080
```
...
chmod +x /opt/RoseUi/roseui
```

#### Out of Memory

RoseUi requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if RoseUi is running: look for "Server started" message
...
4. **Keep device charged** - Running RoseUi consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **RoseUi Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch RoseUi quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-roseui.sh << 'EOF'
#!/bin/bash
echo "Starting RoseUi WebUI..."
RoseUi --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-roseui.sh
...
```bash
proot-distro login ubuntu -- bash -c "RoseUi --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/RoseUi/issues/217)
...
```cmd
RoseUi.exe --webui --remote
```

### macOS

```bash
/Applications/RoseUi.app/Contents/MacOS/RoseUi --webui --remote
```
...
netsh advfirewall firewall add rule name="RoseUi WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add RoseUi
...
which roseui
# or
find /opt -name "roseui" 2>/dev/null
```
...
```powershell
& "C:\Program Files\RoseUi\RoseUi.exe" --webui 2>&1 | Tee-Object -FilePath roseui.log
```

**macOS/Linux:**

```bash
/path/to/roseui --webui 2>&1 | tee roseui.log
```
...
export ROSEUI_PORT=8080

# Allow remote access without passing --remote
export ROSEUI_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as ROSEUI_ALLOW_REMOTE=true)
export ROSEUI_HOST=0.0.0.0

# Then start the application
roseui --webui

# You can also pass the port directly via CLI
roseui --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/RoseUi/webui.config.json`                     |
| macOS    | `~/Library/Application Support/RoseUi/webui.config.json` |
| Linux    | `~/.config/RoseUi/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\RoseUi\RoseUi.exe" --resetpass

# Or for a specific user
"C:\Program Files\RoseUi\RoseUi.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/RoseUi.app/Contents/MacOS/RoseUi --resetpass

# Or for a specific user
/Applications/RoseUi.app/Contents/MacOS/RoseUi --resetpass username
```
...
# Or for a specific user
roseui --resetpass username

# Or using full path
/opt/RoseUi/roseui --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/RoseUi/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/RoseUi/issues)
3. Create a [new issue](https://github.com/iOfficeAI/RoseUi/issues/new) with:
   - Your OS and version
   - RoseUi version
   - Steps to reproduce
...
**Happy using RoseUi in WebUI mode!** 🚀
